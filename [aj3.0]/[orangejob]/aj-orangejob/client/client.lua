...
RegisterNetEvent('aj-orangejob:collect', function()
    ...
    if not targeteye then
        ...
        AJFW.Functions.Progressbar('drillling stone', 'Picking Orange...', 3000, false, true, { -- Name | Label | Time | useWhileDead | canCancel
...
RegisterNetEvent('aj-orangejob:orangejuice', function()
    if not cooldown then
        cooldown = true
        AJFW.Functions.TriggerCallback('orange:ingredient', function(result, cb)
            hasBagd7 = result
            s1d7 = true
        end)
        while(not s1d7) do
        ...
        if (hasBagd7) then
            ...
            AJFW.Functions.Notify('You are missing empty bottle or oranges..', 'error')
        end
    else
