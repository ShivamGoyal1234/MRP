...
AJFW.Commands.Add('radius', 'for event', {{name='type', help='public / test'}, {name='toggle', help='true / false'}, {name='radius', help='in number'},{name='color', help='red / green / blue'}}, false, function(source, args)
    local src = source
        ...
        if zone then
            if radius ~= nil then
                if color == 'red' or color == 'green' or color == 'blue' then
                    ...
                    TriggerClientEvent("AJFW:Notify", src, "only red, green, and blue color are allowed", "error", 5000)
                end
            else
                TriggerClientEvent("AJFW:Notify", src, "You are missing radius", "error", 5000)
            end
        else
            global_radius = nil
            global_color = nil
...
AJFW.Commands.Add('radius2', 'for event', {{name='type', help='public / test'}, {name='toggle', help='true / false'}, {name='radius', help='in number'},{name='color', help='red / green / blue'}}, false, function(source, args)
    local src = source
    local type = tostring(args[1])
    local zone = args[2]
    local radius = tonumber(args[3])
...
AJFW.Commands.Add('radius3', 'for event', {{name='type', help='public / test'}, {name='toggle', help='true / false'}, {name='radius', help='in number'},{name='color', help='red / green / blue'}}, false, function(source, args)
    local src = source
    local type = tostring(args[1])
    local zone = args[2]
    local radius = tonumber(args[3])
...
AJFW.Commands.Add('eventtime', 'for event', {{name='time', help='in second'}}, false, function(source, args)
    local src = source
    ...
    if seconds then
        ...
        TriggerClientEvent("AJFW:Notify", src, "invalid usage", "error", 5000)
    end
end,'dev')

AJFW.Commands.Add('eventstash', 'for event', {}, false, function(source, args)
    TriggerClientEvent("event:stash", source)
end,'dev')

AJFW.Commands.Add('eventblip', 'eventblip', {}, false, function(source, args)
