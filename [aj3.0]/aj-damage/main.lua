...
AJFW.Commands.Add("testdecay", "Description", {}, true, function(source, args)
    local src = source
...
RegisterNetEvent('Perform:Decay:item', function(itemm, slott, amount)
    ...
    if Player ~= nil then
        ...
        for k,v in pairs(items) do
            local itemInfo = AJFW.Shared.Items[v.name:lower()]
...
function DegradeInventoryItems(sources)
	...
	if results[1] ~= nil then
        ...
        for k = 1, #results, 1 do
            ...
            local sentItems = {}
            local items = nil
            local isOnline = AJFW.Functions.GetPlayerByCitizenId(citizenid)
            if isOnline then
                ...
                for a, item in pairs(items) do
                    if item ~= nil then
                        ...
                        if item.info ~= nil and item.info.quality ~= nil then
                            local degradeAmount = AJFW.Shared.Items[item.name:lower()]["degrade"] ~= nil and AJFW.Shared.Items[item.name:lower()]["degrade"] or 0.0
    ...
    TriggerClientEvent("AJFW:Notify", src, "DoneInventory Decay", "success", 3000)
end
...
function DegradeStashItems(sources)
    ...
    TriggerClientEvent("AJFW:Notify", src, "DoneStash Decay", "success", 3000)
end
...
function DegradeGloveboxItems(sources)
    ...
    TriggerClientEvent("AJFW:Notify", src, "DoneGlovebox Decay", "success", 3000)
end
...
function DegradeTrunkItems(sources)
    ...
    TriggerClientEvent("AJFW:Notify", src, "DoneTrunk Decay", "success", 3000)
end
