...
AJFW.Commands.Add('transfer', 'Transfer your vehicle to dealer', {{ name = 'id', help = 'player id' },{ name = 'plate', help = 'vehicle plate' }}, false, function(source, args)
    local src = source
    local tsrc = tonumber(args[1])
    local plate = args[2]
    local Player = AJFW.Functions.GetPlayer(src)
    local tPlayer = AJFW.Functions.GetPlayer(tsrc)
    if Player then
        if tPlayer then
                    ...
                    if distance <= 3 then
                        for k,v in pairs(Config.Dealers) do
                            if targetid == v.cid then
                                ...
                                TriggerClientEvent('AJFW:Notify', src, 'Vehicle transfer successful', 'success')
                                TriggerClientEvent('vehiclekeys:client:SetOwner', tPlayer.PlayerData.source, plate)
                                TriggerClientEvent('AJFW:Notify', tPlayer.PlayerData.source, 'You Got the vehicle Plate: '..plate, 'success')
                                return
                            end
                        end
                        TriggerClientEvent("AJFW:Notify", src, "He/She is not a dealer", "error", 3000)
                    else
                        TriggerClientEvent("AJFW:Notify", src, "Not Near that person", "error", 3000)
                    end
                else
                    TriggerClientEvent("AJFW:Notify", src, "You are not the owner of this vehicle", "error", 3000)
                end
            else
                TriggerClientEvent("AJFW:Notify", src, "Invalid Usage", "error", 3000)
            end
        else
            TriggerClientEvent("AJFW:Notify", src, "Player Not Online", "error", 3000)
        end
    end
end)

AJFW.Commands.Add('transferdealer', 'Transfer your vehicle to dealer', {{ name = 'id', help = 'player id' },{ name = 'plate', help = 'vehicle plate' }}, false, function(source, args)
    local src = source
    local tsrc = tonumber(args[1])
    local plate = args[2]
    local Player = AJFW.Functions.GetPlayer(src)
...
RegisterNetEvent('aj-vehicleshop:server:addPlayer', function(citizenid, gameTime)
...
RegisterNetEvent('aj-vehicleshop:server:customTestDrive', function(vehicle, playerid)
    ...
    if not AJFW.Functions.GetPlayer(target) then
        TriggerClientEvent('AJFW:Notify', src, 'Invalid Player Id Supplied', 'error')
        return
    end
    if #(GetEntityCoords(GetPlayerPed(src))-GetEntityCoords(GetPlayerPed(target)))<3 then
        ...
        TriggerClientEvent('AJFW:Notify', src, 'This player is not close enough', 'error')
    end
end)
    ...
    if newBalance > 0 then
        if player and paymentAmount >= minPayment then
            ...
            elseif bank >= paymentAmount then
                ...
                TriggerClientEvent('AJFW:Notify', src, 'Not enough money', 'error')
            end
        else
            TriggerClientEvent('AJFW:Notify', src, 'Minimum payment allowed is $' ..comma_value(minPayment), 'error')
        end
    else
        TriggerClientEvent('AJFW:Notify', src, 'You overpaid', 'error')
    end
end)
...
RegisterNetEvent('aj-vehicleshop:server:financePaymentFull', function(data)
    ...
    if player and vehBalance ~= 0 then
        ...
        TriggerClientEvent('AJFW:Notify', src, 'Vehicle is already paid off', 'error')
    end
end)
...
RegisterNetEvent('aj-vehicleshop:server:buyShowroomVehicle', function(vehicle)
    ...
    if cash > vehiclePrice then
        ...
        TriggerClientEvent('AJFW:Notify', src, 'Congratulations on your purchase!', 'success')
...
RegisterNetEvent('aj-vehicleshop:server:financeVehicle', function(downPayment, paymentAmount, vehicle)
    ...
    if downPayment > vehiclePrice then return TriggerClientEvent('AJFW:Notify', src, 'Vehicle is not worth that much', 'error') end
    if downPayment < minDown then return TriggerClientEvent('AJFW:Notify', src, 'Down payment too small', 'error') end
    if paymentAmount > Config.MaximumPayments then return TriggerClientEvent('AJFW:Notify', src, 'Exceeded maximum payment amount', 'error') end
    local plate = GeneratePlate()
...
RegisterNetEvent('aj-vehicleshop:server:sellShowroomVehicle', function(data, playerid, type2)
    local src = source
    local player = AJFW.Functions.GetPlayer(src)
    ...
    if #(GetEntityCoords(GetPlayerPed(src))-GetEntityCoords(GetPlayerPed(target.PlayerData.source)))<3 then
        ...
        if player.PlayerData.job.name == 'police' then
            if target.PlayerData.job.name ~= 'police' then
                TriggerClientEvent('AJFW:Notify', src, 'This Person is not a cop', 'error')
                return
            end
        end
        if cash >= vehiclePrice then
            ...
            if player.PlayerData.job.name == 'pdm' then
                TriggerEvent('aj-bossmenu:server:addAccountMoney', 'pdm', vehiclePrice)
                TriggerEvent("aj-log:server:CreateLog", 'vehicleshop', "Vehicle Purchased", "blue", "**Source**: "..player.PlayerData.charinfo.firstname.." "..player.PlayerData.charinfo.lastname..
            ...
            elseif player.PlayerData.job.name =='edm' then
                TriggerEvent('aj-bossmenu:server:addAccountMoney', 'edm', vehiclePrice)
                TriggerEvent("aj-log:server:CreateLog", 'vehicleshop2', "Vehicle Purchased", "blue", "**Source**: "..player.PlayerData.charinfo.firstname.." "..player.PlayerData.charinfo.lastname..
                                                                                                  "\n**Target**: "..target.PlayerData.charinfo.firstname.." "..target.PlayerData.charinfo.lastname..
                                                                                                  "\n**Vehicle**: "..vehicle..
                                                                                                  "\n**plate**: "..plate..
                                                                                                  "\n**Price**: "..vehiclePrice)
            elseif player.PlayerData.job.name =='tunner' then
                TriggerEvent('aj-bossmenu:server:addAccountMoney', 'tunner', vehiclePrice)
                TriggerEvent("aj-log:server:CreateLog", 'vehicleshop3', "Vehicle Purchased", "blue", "**Source**: "..player.PlayerData.charinfo.firstname.." "..player.PlayerData.charinfo.lastname..
                                                                                                  "\n**Target**: "..target.PlayerData.charinfo.firstname.." "..target.PlayerData.charinfo.lastname..
                                                                                                  "\n**Vehicle**: "..vehicle..
                                                                                                  "\n**plate**: "..plate..
                                                                                                  "\n**Price**: "..vehiclePrice)
            end
            TriggerClientEvent('AJFW:Notify', target.PlayerData.source, 'Congratulations on your purchase!', 'success')
        elseif bank >= vehiclePrice then
            MySQL.Async.insert('INSERT INTO player_vehicles (license, citizenid, vehicle, hash, mods, plate, state, type) VALUES (?, ?, ?, ?, ?, ?, ?, ?)', {
                target.PlayerData.license,
                cid,
...
RegisterNetEvent('aj-vehicleshop:server:sellfinanceVehicle', function(downPayment, paymentAmount, vehicle, playerid, type2)
    local src = source
    local player = AJFW.Functions.GetPlayer(src)
    local target = AJFW.Functions.GetPlayer(tonumber(playerid))
    ...
    if #(GetEntityCoords(GetPlayerPed(src))-GetEntityCoords(GetPlayerPed(target.PlayerData.source)))<3 then
        ...
        if cash >= downPayment then
            ...
            if player.PlayerData.job.name == 'pdm' then
                TriggerEvent('aj-bossmenu:server:addAccountMoney', 'pdm', vehiclePrice)
                TriggerEvent("aj-log:server:CreateLog", 'vehicleshop', "Vehicle Financed", "blue", "**Source**: "..player.PlayerData.charinfo.firstname.." "..player.PlayerData.charinfo.lastname..
                                                                                                  "\n**Target**: "..target.PlayerData.charinfo.firstname.." "..target.PlayerData.charinfo.lastname..
                                                                                                  "\n**Vehicle**: "..vehicle..
                                                                                                  "\n**plate**: "..plate..
            ...
            elseif player.PlayerData.job.name =='edm' then
                TriggerEvent('aj-bossmenu:server:addAccountMoney', 'edm', vehiclePrice)
                TriggerEvent("aj-log:server:CreateLog", 'vehicleshop2', "Vehicle Financed", "blue", "**Source**: "..player.PlayerData.charinfo.firstname.." "..player.PlayerData.charinfo.lastname..
                                                                                                  "\n**Target**: "..target.PlayerData.charinfo.firstname.." "..target.PlayerData.charinfo.lastname..
                                                                                                  "\n**Vehicle**: "..vehicle..
                                                                                                  "\n**plate**: "..plate..
                                                                                                  "\n**DownPaymet**: "..downPayment..
            ...
            elseif player.PlayerData.job.name =='tunner' then
                TriggerEvent('aj-bossmenu:server:addAccountMoney', 'tunner', vehiclePrice)
                TriggerEvent("aj-log:server:CreateLog", 'vehicleshop3', "Vehicle Financed", "blue", "**Source**: "..player.PlayerData.charinfo.firstname.." "..player.PlayerData.charinfo.lastname..
                                                                                                  "\n**Target**: "..target.PlayerData.charinfo.firstname.." "..target.PlayerData.charinfo.lastname..
                                                                                                  "\n**Vehicle**: "..vehicle..
                                                                                                  "\n**plate**: "..plate..
                                                                                                  "\n**DownPaymet**: "..downPayment..
...
RegisterNetEvent('aj-vehicleshop:server:checkFinance', function()
    ...
    if paymentDue then
        TriggerClientEvent('AJFW:Notify', src, 'Your vehicle payment is due within '..Config.PaymentWarning..' minutes')
        ...
        MySQL.Sync.fetchAll('SELECT * FROM player_vehicles WHERE citizenid = ?', {player.PlayerData.citizenid}, function(vehicles)
            for k,v in pairs(vehicles) do
                if v.balance >= 1 and v.financetime < 1 then
                    ...
                    TriggerClientEvent('AJFW:Notify', src, 'Your vehicle with plate '..plate..' has been repossessed', 'error')
                end
            end
        end)
    end
...
-- AJFW.Commands.Add('transferVehicle', 'Gift or sell your vehicle', {{ name = 'amount', help = 'Sell amount' }}, false, function(source, args)
--     local src = source
--     local ped = GetPlayerPed(src)
--     local player = AJFW.Functions.GetPlayer(src)
--     local citizenid = player.PlayerData.citizenid
...
--     if vehicle == 0 then return TriggerClientEvent('AJFW:Notify', src, 'Must be in a vehicle', 'error') end
--     local driver = GetPedInVehicleSeat(vehicle, -1)
--     local passenger = GetPedInVehicleSeat(vehicle, 0)
--     local plate = AJFW.Functions.GetPlate(vehicle)
--     local isOwned = MySQL.Sync.fetchScalar('SELECT citizenid FROM player_vehicles WHERE plate = ?', {plate})
--     if isOwned ~= citizenid then return TriggerClientEvent('AJFW:Notify', src, 'You dont own this vehicle', 'error') end
--     if ped ~= driver then return TriggerClientEvent('AJFW:Notify', src, 'Must be driver', 'error') end
--     if passenger == 0 then return TriggerClientEvent('AJFW:Notify', src, 'No passenger', 'error') end
--     local targetid = NetworkGetEntityOwner(passenger)
--     local target = AJFW.Functions.GetPlayer(targetid)
--     if not target then return TriggerClientEvent('AJFW:Notify', src, 'Couldnt get passenger info', 'error') end
--     if sellAmount then
...
--             TriggerClientEvent('AJFW:Notify', src, 'You sold your vehicle for $'..comma_value(sellAmount), 'success')
...
--             TriggerClientEvent('AJFW:Notify', target.PlayerData.source, 'You bought a vehicle for $'..comma_value(sellAmount), 'success')
--         else
--             TriggerClientEvent('AJFW:Notify', src, 'Not enough money', 'error')
--         end
--     else
--         local targetcid = target.PlayerData.citizenid
--         MySQL.Async.execute('UPDATE player_vehicles SET citizenid = ? WHERE plate = ?', {targetcid, plate})
--         TriggerClientEvent('AJFW:Notify', src, 'You gifted your vehicle', 'success')
--         TriggerClientEvent('vehiclekeys:client:SetOwner', target.PlayerData.source, plate)
--         TriggerClientEvent('AJFW:Notify', target.PlayerData.source, 'You were gifted a vehicle', 'success')
--     end
-- end, 'god')
