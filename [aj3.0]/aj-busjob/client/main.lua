            ...
            if dist < 20 then
                ...
                if dist < 5 then
                    ...
                    if IsControlJustPressed(0, 38) then
                        ...
                        AJFW.Functions.Notify('Person Was Dropped Off', 'success')
                        if NpcData.DeliveryBlip ~= nil then
                            RemoveBlip(NpcData.DeliveryBlip)
                        end
...
RegisterNetEvent("aj-busjob:client:TakeVehicle", function(data)
    ...
    if(BusData.Active) then
        AJFW.Functions.Notify('You can only have one active bus at a time', 'error')
        return
    else
        ...
        if vehNear < 5 then
           AJFW.Functions.Notify('There is a vehicle at the spot', 'error', 3000)
            ...
            AJFW.Functions.SpawnVehicle(data.model, function(veh)
            ...
            TriggerServerEvent('aj-busjob:server:Deposit')
            Wait(1000)
            TriggerEvent('aj-busjob:client:DoBusNpc')
        end
    end
...
RegisterNetEvent('aj-busjob:client:DoBusNpc', function()
    if whitelistedVehicle() then
        if not NpcData.Active then
            ...
            AJFW.Functions.Notify('Goto the bus stop', 'success')
                    ...
                    if dist < 20 then
                        ...
                        if dist < 5 then
                            ...
                            if IsControlJustPressed(0, 38) then
                                ...
                                TriggerServerEvent('aj-busjob:server:NpcPay', math.random(50, 75))
                            end
                        end
                    end
           ...
           AJFW.Functions.Notify('You Are Already Driving Bus')
        end
    else
        AJFW.Functions.Notify('You Are Not In A Bus')
    end
end)
                            ...
                            if IsControlJustReleased(0, 38) then
                                if (not NpcData.Active or NpcData.Active and NpcData.NpcTaken == false)then
                                    if IsPedInAnyVehicle(PlayerPedId(), false) then
                                        ...
                                        TriggerServerEvent('aj-busjob:server:Withdraw')
                                        DeleteVehicle(GetVehiclePedIsIn(PlayerPedId()))
                                        RemoveBlip(NpcData.NpcBlip)
                                    end
                                else
                                    AJFW.Functions.Notify("Drop off the passengers before you stop working", "error")
                                end
                            end
                        else
