...
local function CheckAuth(doorData)
    ...
    if doorData.items then
        AJFW.Functions.TriggerCallback('nui_doorlock:server:CheckItems', function(result)
...
local function DoorLoop()
    Started = true
    AJFW.Functions.TriggerCallback('nui_doorlock:server:getDoorList', function(doorList)
...
local function LockpickFinish(success)
    if success then
		AJFW.Functions.Notify('Success!', 'success', 2500)
		...
		if math.random(1, 100) <= 17 then
            if usingAdvanced then
                TriggerServerEvent("AJFW:Server:RemoveItem", "advancedlockpick", 1, false)
			    TriggerEvent('inventory:client:ItemBox', AJFW.Shared.Items["advancedlockpick"], "remove")
            else
			    TriggerServerEvent("AJFW:Server:RemoveItem", "lockpick", 1, false)
			    TriggerEvent('inventory:client:ItemBox', AJFW.Shared.Items["lockpick"], "remove")
            end
		end
        AJFW.Functions.Notify('Failed..', 'error', 2500)
    end
end
            ...
            if Config.DoorList[doorID].doors then
                for k, v in pairs(Config.DoorList[doorID].doors) do
                    ...
                    elseif not Config.DoorList[doorID].locked then
                        ...
                        if AJFW.Shared.Round(v.currentHeading, 0) == AJFW.Shared.Round(v.objHeading, 0) then
                ...
                elseif not Config.DoorList[doorID].locked then
                    ...
                    if AJFW.Shared.Round(Config.DoorList [doorID].currentHeading, 0) == AJFW.Shared.Round(Config.DoorList[doorID].objHeading, 0) then
