...
local function IsAuthorized(Player, doorID, usedLockpick, isScript)
    ...
    if Config.AdminAccess.enabled and AJFW.Functions.HasPermission(Player.PlayerData.source, Config.AdminAccess.permission) then
...
RegisterNetEvent('nui_doorlock:server:newDoorCreate', function(config, model, heading, coords, jobs, gangs, cids, item, doorLocked, maxDistance, slides, garage, doubleDoor, doorname)
    ...
    if Player then
        if not AJFW.Functions.HasPermission(source, Config.CommandPermission) then print(('^3[Warning] ^7%s (%s) attempted to create a new door but does not have permission'):format(Player.PlayerData.name, Player.PlayerData.license)) return end
...
AJFW.Functions.CreateCallback('nui_doorlock:server:getDoorList', function(source, cb)
    cb(Config.DoorList)
end)

AJFW.Functions.CreateCallback('nui_doorlock:server:CheckItems', function(source, cb, items, locked)
	local Player = AJFW.Functions.GetPlayer(source)
	if Player then
		local canOpen = false
		local count = false
...
AJFW.Commands.Add('newdoor', 'Create a new door using a gun', { { name = 'doorname', help = 'Name of the door' }, { name = 'doortype', help = 'door/double/sliding/garage/doublesliding' }, { name = 'locked', help = 'true/false' }, { name = 'jobs', help = 'Add up to 3 jobs to this, seperate with spaces and no commas' } }, false, function(source, args)
