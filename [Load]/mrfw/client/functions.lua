...
function MRFW.Functions.GetPlayerData(cb)
    if cb then
        cb(MRFW.PlayerData)
    else
        return MRFW.PlayerData
    end
end
...
function MRFW.Functions.HasItem(item)
    local p = promise.new()
...
function MRFW.Functions.DrawText(x, y, width, height, scale, r, g, b, a, text)
...
function MRFW.Functions.DrawText3D(x, y, z, text)
    -- Use local function instead
...
function MRFW.Functions.Draw2DText(x, y, text, scale)
    SetTextFont(4)
...
RegisterNUICallback('getNotifyConfig', function(_, cb)
    cb(MRFW.Config.Notify)
end)

function MRFW.Functions.Notify(text, textype, length)
...
function MRFW.Debug(resource, obj, depth)
    TriggerServerEvent('MRFW:DebugSomething', resource, obj, depth)
end

function MRFW.Functions.TriggerCallback(name, cb, ...)
    MRFW.ServerCallbacks[name] = cb
    TriggerServerEvent('MRFW:Server:TriggerCallback', name, ...)
end

function MRFW.Functions.Progressbar(name, label, duration, useWhileDead, canCancel, disableControls, animation, prop, propTwo, onFinish, onCancel)
...
MRFW.Functions.SpawnObject = function(model, coords, cb)
...
MRFW.Functions.SpawnLocalObject = function(model, coords, cb)
    local model = (type(model) == 'number' and model or GetHashKey(model))
...
MRFW.Functions.DeleteObject = function(object)
...
function MRFW.Functions.GetVehicles()
    return GetGamePool('CVehicle')
end
function MRFW.Functions.GetObjects()
...
function MRFW.Functions.GetPeds(ignoreList)
...
function MRFW.Functions.CreateBlip(id, data)
...
function MRFW.Functions.RemoveBlip(id)
    local blip = Blips[id]
    if blip then RemoveBlip(blip.blip) end
    Blips[id] = nil
end

function MRFW.Functions.HideBlip(id, toggle)
    local blip = Blips[id]
...
function MRFW.Functions.RequestAnimDict(animDict)
...
function MRFW.Functions.GetBlip(id)
    local blip = Blips[id]
    if not blip then return false end
    return blip
end

function MRFW.Functions.GetBoneDistance(entity, Type, Bone)
    local bone
...
function MRFW.Functions.GetClosestBone(entity, list)
...
function MRFW.Functions.GetClosestPed(coords, ignoreList)
    ...
    local ignoreList = ignoreList or {}
    local peds = MRFW.Functions.GetPeds(ignoreList)
...
function MRFW.Functions.GetClosestPlayer(coords)
    local ped = PlayerPedId()
    if coords then
        coords = type(coords) == 'table' and vec3(coords.x, coords.y, coords.z) or coords
    else
    ...
    local closestPlayers = MRFW.Functions.GetPlayersFromCoords(coords)
    local closestDistance = -1
...
function MRFW.Functions.GetPlayersFromCoords(coords, distance)
...
function MRFW.Functions.GetClosestVehicle(coords)
    local ped = PlayerPedId()
...
function MRFW.Functions.GetClosestObject(coords)
    local ped = PlayerPedId()
...
function MRFW.Functions.SpawnClear(coords, radius)
    local vehicles = GetGamePool('CVehicle')
    local closeVeh = {}
    for i=1, #vehicles, 1 do
        local vehicleCoords = GetEntityCoords(vehicles[i])
...
function MRFW.Functions.SpawnVehicle(model, cb, coords, isnetworked)
...
function MRFW.Functions.DeleteVehicle(vehicle)
    SetEntityAsMissionEntity(vehicle, true, true)
    DeleteVehicle(vehicle)
end

function MRFW.Functions.GetPlate(vehicle)
    if vehicle == 0 then return end
    return GetVehicleNumberPlateText(vehicle)
end

function MRFW.Functions.GetVehicleProperties(vehicle)
    if DoesEntityExist(vehicle) then
        ...
        return {
            ...
            plate = MRFW.Functions.GetPlate(vehicle),
            plateIndex = GetVehicleNumberPlateTextIndex(vehicle),
            -- bodyHealth = MRFW.Shared.Round(GetVehicleBodyHealth(vehicle), 0.1),
            -- engineHealth = MRFW.Shared.Round(GetVehicleEngineHealth(vehicle), 0.1),
            -- tankHealth = MRFW.Shared.Round(GetVehiclePetrolTankHealth(vehicle), 0.1),
            fuelLevel = MRFW.Shared.Round(GetVehicleFuelLevel(vehicle), 0.1),
            dirtLevel = MRFW.Shared.Round(GetVehicleDirtLevel(vehicle), 0.1),
            oilLevel = MRFW.Shared.Round(GetVehicleOilLevel(vehicle), 0.1),
            bodyHealth = ffff,
            engineHealth = eeee,
            tankHealth = gggg,
            clutchHealth = hhhh,
...
function MRFW.Functions.SetVehicleProperties(vehicle, props)
    if DoesEntityExist(vehicle) then
...
function MRFW.Functions.CheckFuckingCar(coords)
    local veh = 1000
...
function MRFW.Functions.LoadAnimSet(animSet)
