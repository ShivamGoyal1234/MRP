...
AddEventHandler("invest:buy", function(job, amount, rate)
    ...
    if(amount == nil or amount <= 0) then
        return TriggerClientEvent('AJFW:Notify', src, _U('invalid_amount'), 'error')
    elseif(Config.Stock.Limit ~= 0 and amount > Config.Stock.Limit) then
        return TriggerClientEvent('AJFW:Notify', src, string.gsub(_U('to_much'), "{limit}", format_int(Config.Stock.Limit)), 'error')
    else
        if(bank < amount) then
            return TriggerClientEvent('AJFW:Notify', src, _U('broke_amount'), 'error')
        end
        xPlayer.Functions.RemoveMoney('bank', amount)
        TriggerEvent("aj-log:server:CreateLog", "stocks", "Invested", "green", "**"..GetPlayerName(src) .. "** has invested $"..amount.." in stocks")
    end
    ...
    if(type(inf) == "table" and inf.job ~= nil) then
        ...
        TriggerClientEvent('AJFW:Notify', src, _U('added'), 'success')
    else
        if Config.Debug then
        ...
        if rate == nil then
            return TriggerClientEvent('AJFW:Notify', src, _U('unexpected_error'), 'error')
        end
        ...
        TriggerClientEvent('AJFW:Notify', src, _U('buy'), 'success')
    end
...
AddEventHandler("invest:sell", function(job)
    ...
    if(addMoney > 0) then
        ...
        TriggerEvent("aj-log:server:CreateLog", "stocks", "Sold", "blue", "**"..GetPlayerName(src) .. "** has sold his stocks for $"..addMoney.." ")
    else
        addMoney = math.abs(addMoney)*-1
        xPlayer.Functions.RemoveMoney('bank', addMoney)
        TriggerEvent("aj-log:server:CreateLog", "stocks", "Lost", "red", "**"..GetPlayerName(src) .. "** has lost his $"..addMoney.." in stocks ")
    end
    
    TriggerClientEvent('AJFW:Notify', src, _U('sold'), 'success')
    TriggerEvent(src, "invest:balance")
end)
