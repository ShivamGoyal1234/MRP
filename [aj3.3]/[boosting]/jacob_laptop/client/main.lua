        ...
        if not veh.state.hacked then
            ...
            if success then
                ...
                if trackerLeft == 0 then
                    ...
                    veh.state.hacked = true
                    return AJFW.Functions.Notify(Lang:t("success.disable_tracker"))
                else
                    AJFW.Functions.Notify(Lang:t('success.tracker_off', {time = randomSeconds, tracker_left = trackerLeft}))
                end
                CreateThread(function ()
                ...
                AJFW.Functions.Notify(Lang:t('error.disable_fail'), "error")
            end
        else
            AJFW.Functions.Notify(Lang:t('error.already_hacked'), "error")
        end
    else
        AJFW.Functions.Notify(Lang:t("error.no_tracker"), "error")
    end
end
...
local function BoostingAlert()
    if Config.Alert == 'aj-dispatch' then
        AlertBoosting(carID, 'aj-dispatch')
    ...
    if Config.BennysSell then
        for k, v in pairs(Config.BennysSell) do
            ...
            if not storeitem[k] then
                if AJFW.Shared.Items[v.item] ~= nil then
                    if Config.BennysSell[k].name then
                        name = Config.BennysSell[k].name
                    else
                       if AJFW.Shared.Items[v.item] ~= nil then
                           name = AJFW.Shared.Items[v.item].label
                       else
                        name = "Unknown"
                    ...
                    storeitem[#storeitem+1] = {
                        name = name,
                        item = v.item,
                        image = Config.Inventory ..AJFW.Shared.Items[v.item].image,
                        price = v.price,
                        stock = v.stock
                    }
                else
...
RegisterNUICallback('canStartContract', function (data, cb)
    ...
    if isContractStarted then
        ...
        if data.type == 'vin' then
            AJFW.Functions.TriggerCallback('jacob-carboost:server:vinmoney', function(result)
...
RegisterNUICallback('startcontract', function (data)
    ...
    if not isContractStarted then
        ...
        Tier = data.data.tier
        AJFW.Functions.TriggerCallback('jacob-carboost:server:getContractData', function (result)
            if result then
...
RegisterNUICallback('checkout', function (data)
    AJFW.Functions.TriggerCallback('jacob-carboost:server:canBuy', function (result)
        if result then
...
RegisterNUICallback('setupboostapp', function (data, cb)
    AJFW.Functions.TriggerCallback('jacob-carboost:server:getboostdata', function (result)
        if result then
...
RegisterNUICallback('sellcontract', function(data, cb)
    AJFW.Functions.TriggerCallback('jacob-carboost:server:sellContract', function (result)
        if result then
            cb(result)
        else
            cb({
...
RegisterNUICallback('transfercontract', function (data, cb)
    AJFW.Functions.TriggerCallback('jacob-carboost:server:transfercontract', function (result)
        if result and not result.error then
...
RegisterNUICallback('buycontract', function(data, cb)
    AJFW.Functions.TriggerCallback('jacob-carboost:server:buycontract', function(result)
        if result then
            cb(result)
        end
    end, data)
...
RegisterNetEvent('jacob-carboost:client:openMenu', function ()
    if Config.BennysItems[1] then
        ...
        for _, v in pairs(Config.BennysItems) do
            ...
            menu[#menu+1] = {
                header = AJFW.Shared.Items[item.name]["label"],
                id = item.name,
        ...
        AJFW.Functions.Notify(Lang:t("error.empty_post"), "error")
    end
end)

RegisterNetEvent('jacob-carboost:client:spawnCar', function(data)
    AJFW.Functions.TriggerCallback('jacob-carboost:server:spawnCar', function(result)
        if result then
            ...
            if carID ~= nil then
                if DoesEntityExist(NetworkGetEntityFromNetworkId(carID)) then
                    ...
                    if not modified then
                        AJFW.Functions.SetVehicleProperties(carSpawned, VehProp)
                        modified = true
                    end
    ...
    if vehicle ~= 0 then
        ...
        if #(PlayerPos - vehPos) <= 5.0 then
            ...
            if GetVehicleDoorLockStatus(vehicle) == 1 then
                    AJFW.Functions.Progressbar('check_vin', 'Checking VIN Number', 6000, false, true, { -- Name | Label | Time | useWhileDead | canCancel
                        ...
                        AJFW.Functions.TriggerCallback('jacob-carboost:server:checkvin', function(result)
                            if result and result.success then
    
                                AJFW.Functions.Notify(result.message, "primary")
                               
                            else
                                AJFW.Functions.Notify("Hmm you can't found the VIN", "error")
                            end
                        end, networkID)
                    ...
                    AJFW.Functions.Notify("The vehicle is locked", "error")
                end
            else
...
RegisterNetEvent('jacob-carboost:client:vinscratch', function(veh)
    ...
    AJFW.Functions.Progressbar('vin_scratching', 'Scratching VIN', 7000, false, true, { -- Name | Label | Time | useWhileDead | canCancel
        disableMovement = true,
        disableCarMovement = true,
        disableMouse = false,
        disableCombat = true,
        ...
        AJFW.Functions.TriggerCallback('jacob-carboost:server:checkvin', function(result)
            if result and result.owner == PlayerData.citizenid then
                if result.vinscratch == 1 then
                    return AJFW.Functions.Notify('You already scratch this vehicle VIN', 'error')
                end
            else
                AJFW.Functions.TriggerCallback('AJFW:HasItemV2', function(has, info)
                    if has then
                        if info.amount >= 15 then
                            -- AJFW.Functions.Notify('VIN Scratched, you can change your plate number', 'primary')
                            local vehProp = AJFW.Functions.GetVehicleProperties(veh)
                            TriggerServerEvent('jacob-carboost:server:vinscratch', ID, vehProp, carmodel)
                            TriggerEvent('jacob-carboost:client:deleteContract')
                            Scratching()
                        else
                            AJFW.Functions.Notify('Not Enough Material', 'error', 3000)
                        end
                    else
                        AJFW.Functions.Notify('Something is missing', 'error', 3000)
                    end
                end, 'vehicleticket')
            end
        end, ID)
    ...
    if data.type == 'vin' then
        ...
        scratchpoint:onPointInOut(function ()
            ...
            if inscratchPoint then
                AJFW.Functions.Notify(Lang:t("info.in_scratch"))
            else
                AJFW.Functions.Notify(Lang:t("info.not_in_scratch"))
            end
        end)
    else
        ...
        zone:onPointInOut(function ()
            ...
            if inZone then
                AJFW.Functions.Notify(Lang:t("info.car_inzone"))
            end
        end)
        CreateThread(function ()
            while true do
...
RegisterNetEvent('jacob-carboost:client:failedBoosting', function ()
    ...
    isContractStarted = false
    AJFW.Functions.Notify(Lang:t("error.no_car"), "error")
    TriggerEvent('jacob-carboost:client:refreshQueue')
    TriggerEvent('jacob-carboost:client:deleteContract')
end)
...
RegisterNetEvent('jacob-carboost:client:refreshContract', function ()
    AJFW.Functions.TriggerCallback('jacob-carboost:server:getContractData')
    SendNUIMessage({
...
RegisterNetEvent('jacob-carboost:client:useHackingDevice', function (item)
    ...
    if IsPedInVehicle(playerPed, vehicle) then
        if GetPedInVehicleSeat(GetVehiclePedIsIn(PlayerPedId(), false), 0) ~= 0 then
            if cooldown then
                return AJFW.Functions.Notify(Lang:t("error.cannot_use"), "error")
            end
            local infos = {}
            infos.uses = item.info.uses - 1
            TriggerServerEvent('AJFW:Server:RemoveItem', item.name, item.amount, item.slot)
            ...
            return AJFW.Functions.Notify(Lang:t('error.not_seat'), "error")
        end
    else
        AJFW.Functions.Notify(Lang:t("error.not_on_vehicle"), "error")
    end
end)
...
RegisterNetEvent('jacob-carboost:client:fakeplate', function()
    ...
    if veh ~= 0 and not IsPedInAnyVehicle(PlayerPedId()) then
        if distFront < 2.0 or distBack < 2.0 then 
            AJFW.Functions.TriggerCallback('jacob-carboost:server:checkvin', function (result)
                if result then
                    if result.owner == PlayerData.citizenid then
                        AJFW.Functions.Progressbar('change_plate', 'Changing the plate', 8000, false, true, { -- Name | Label | Time | useWhileDead | canCancel
                            disableMovement = true,
                            disableCarMovement = true,
                            disableMouse = false,
                            disableCombat = true,
            ...
            AJFW.Functions.Notify(Lang:t("error.not_plate"), "error")
        end
    end
end)
...
RegisterNetEvent('jacob-carboost:client:setPlate', function (vehID)
    ...
    if veh and veh ~= 0 then
        ...
        AJFW.Functions.Notify(Lang:t("success.plate_changed", {
            plate = newPlate
        }), "success")
        TriggerEvent('inventory:client:ItemBox', AJFW.Shared.Items["fake_plate"], "remove")
        TriggerServerEvent("AJFW:Server:RemoveItem", "fake_plate", 1)
...
-- exports
exports['aj-eye']:AddBoxZone("carboost:takeItem", vector3(1185.14, -3304.01, 7.1), 2, 2, {
...
exports['aj-eye']:AddTargetBone('windscreen', {
    options = {
        {
            icon = "fas fa-tally",
            label = "Scratch VIN",
