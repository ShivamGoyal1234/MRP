...
function boostProgress(currentVal, toVal) {
  ...
  if (toVal === 0) {
    ...
    let progBar = setInterval(() => {
      currentVal++;
      let finalvar = (currentVal / 10)
      boostpercent.style.width = finalvar + "%";
      if (finalvar == toVal / 5) {
...
function loadBoostData() {
  fetch("https://jacob_laptop/setupboostapp", {
    method: "POST",
    headers: {
...
function buyContract(event) {
  Confirm.open({
    ...
    cancelText: "No",
    onOk: () => {
      fetch("https://jacob_laptop/buycontract", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
...
function transferContract(event) {
  ...
  Confirm.Input({
    ...
    onOk: function (value) {
      ...
      fetch("https://jacob_laptop/transfercontract", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
...
function sellContract(event) {
  ...
  Confirm.Input({
    ...
    onOk: (value) => {
      if (value) {
        fetch(`https://jacob_laptop/sellcontract`, {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
          },
...
function toggleBoosting(event) {
  ...
  if (isStart) {
    ...
    function checkCanStart(type) {
      fetch("https://jacob_laptop/canStartContract", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
...
function startContract(data) {
  $.post("https://jacob_laptop/startcontract", JSON.stringify({ data: data }));
}

function stopContract(data) {
  $.post("https://jacob_laptop/stopcontract", JSON.stringify({ id: data }));
}
