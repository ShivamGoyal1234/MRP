...
RegisterNetEvent('jacob-carboost:server:takeItem', function (name, quantity)
   ...
   TriggerClientEvent('inventory:client:ItemBox', src, AJFW.Shared.Items[name], "add")
end)
...
RegisterNetEvent('jacob-carboost:server:finishBoosting', function (type, tier)
   ...
   if type ~= 'vin' then
      ...
      if chance < 80 then
         ...
         TriggerClientEvent('inventory:client:ItemBox', src, AJFW.Shared.Items["vehicleticket"], "add")
      end
      if Config.Payment == 'crypto' then
         TriggerClientEvent('AJFW:Notify', src, Lang:t('info.payment_crypto', {
            amount = amountMoney
         }), 'success')
      end
      ...
      TriggerClientEvent('AJFW:Notify', src, Lang:t('info.get_rep', {
         rep = randomRep
      }), "primary")  
   end 
end)
...
RegisterNetEvent('jacob-carboost:server:vinscratch', function(NetworkID, mods, model)
   ...
   if timeout == 0 then
      ...
      TriggerClientEvent("AJFW:Notify", src, "VIN Scratched, you can change your plate number", "error", 5000)
      TriggerEvent("aj-log:server:CreateLog", 'cardelivery', "Scratch", "blue", '**Source**: '..pData.PlayerData.name..'\n**car**: '..model..'\n**plate**: '..plate)
   else
      TriggerClientEvent("AJFW:Notify", src, "Please wait "..timeout.." hour(s) before doing this thing again", "error", 5000)
   end
end)

AJFW.Commands.Add('settier', 'Set Boosting Tier', {
   {
      name = 'tier',
   ...
   if not args[1] or not type(args[1]) == "string" or Config.Tier[tostring(args[1])] == nil then
      return TriggerClientEvent('AJFW:Notify', src, Lang:t('error.invalid_tier'), "error")
   end
   local PlayerID = tonumber(args[2])
   local player
   if PlayerID then
      player = AJFW.Functions.GetPlayer(PlayerID)
      if not player or player == nil then
         return TriggerClientEvent('AJFW:Notify', src, Lang:t('error.invalid_player'), "error")
      end
   else
      player = AJFW.Functions.GetPlayer(src)
   end
   player.Functions.SetMetaData('carboostclass', tostring(args[1]))
   TriggerClientEvent('AJFW:Notify', src, Lang:t('success.set_tier', {
      tier = args[1]
   }), "success")
end, 'god')

AJFW.Commands.Add('giveContract', 'Give contract, admin only', {
   {
      name = 'tier',
      help = 'Tier of contract, D, C, B, A, S, S+, X',
   },
   ...
   if Config.Tier[tostring(args[1])] ~= nil then
      ...
      MySQL.Async.insert('INSERT INTO boost_contract (owner, data, started, expire) VALUES (@owner, @data, NOW(),DATE_ADD(NOW(), INTERVAL @expire HOUR))', {
         ...
         TriggerClientEvent('AJFW:Notify', src, Lang:t('success.contract_give', {
...
RegisterNetEvent('jacob-carboost:server:takeAll', function (data)
   ...
   for _, v in pairs(data) do
      ...
      -- print(item.name)
      TriggerClientEvent('inventory:client:itemBox', src, AJFW.Shared.Items[item.name], 'add')
   end
   MySQL.Async.execute('UPDATE bennys_shop SET items = @items WHERE citizenid = @citizenid', {
      ['@citizenid'] = Player.PlayerData.citizenid,
      ['@items'] = json.encode({})
   })
   TriggerClientEvent('AJFW:Notify', src, Lang:t('success.take_all'), "success")
end)
...
RegisterNetEvent('jacob-carboost:server:setPlate', function (oldPlate, newPlate)
   ...
   TriggerEvent("aj-log:server:CreateLog", 'cardelivery', "Plate Change", "red", '**Old Plate**: '..oldPlate..'\n** New Plate**: '..newPlate)
end)
...
-- Callback
AJFW.Functions.CreateCallback('jacob-carboost:server:canBuy', function(source, cb, data)
   local src = source
   local pData = AJFW.Functions.GetPlayer(src)
...
AJFW.Functions.CreateCallback('jacob-carboost:server:sellContract', function (source, cb, data, toggle)
...
AJFW.Functions.CreateCallback('jacob-carboost:server:buycontract', function(source, cb, data)
   local src = source
   local Player = AJFW.Functions.GetPlayer(src)
   local toPlayer
   ...
   if result[1] then
      ...
      toPlayer = AJFW.Functions.GetPlayerByCitizenId(result[1].owner)
      ...
      if TierCheck(Player.PlayerData.metadata.carboostclass, contractData.tier) then
         ...
         TriggerClientEvent('AJFW:Notify', toPlayer.PlayerData.source, Lang:t('info.contract_buyed', {amount = moneyAmount}), "success")
...
AJFW.Functions.CreateCallback('jacob-carboost:server:transfercontract', function (source, cb, data, toggle)
   local src = source
   local id = tonumber(data.id) 
   local contractid = tonumber(data.contractid)
   local toPlayer = AJFW.Functions.GetPlayer(id)
   local Player = AJFW.Functions.GetPlayer(src)
   if src == id then
      return cb({
   ...
   if result[1] then
      ...
      if TierCheck(toPlayer.PlayerData.metadata.carboostclass, data.tier)then
         MySQL.Async.execute('UPDATE boost_contract SET owner = @owner WHERE owner = @citizenid', {
            ...
            TriggerClientEvent('AJFW:Notify', src, Lang:t('success.contract_transfer', {
               player = toPlayer.PlayerData.name
            }), "success")
            TriggerClientEvent('AJFW:Notify', id, Lang:t('info.receive_transfer', {
               player = Player.PlayerData.name,
            }), "primary")
...
AJFW.Functions.CreateCallback('jacob-carboost:server:vinmoney', function (source, cb, data)
   local src = source
   local Player = AJFW.Functions.GetPlayer(src)
   local money = Player.PlayerData.money[Config.Payment]
...
AJFW.Functions.CreateCallback('jacob-carboost:server:getboostdata', function (source, cb, citizenid)
...
AJFW.Functions.CreateCallback('jacob-carboost:server:getContractData', function (source, cb, data)
...
AJFW.Functions.CreateCallback('jacob-carboost:server:spawnCar', function (source, cb, data)
   local src = source
   local cardata = data
...
AJFW.Functions.CreateCallback('jacob-carboost:server:checkvin', function (source, cb, data)
   local src = source
...
function Queue()
   if queueNumber ~= 0 then
      ...
      for k, v in pairs(Config.QueueList) do
         local Player = AJFW.Functions.GetPlayerByCitizenId(k)
         if not v.getContract and v.status and Player and num <= Config.MaxQueueContract then
            TriggerClientEvent('AJFW:Notify', Player.PlayerData.source, Lang:t('info.new_contract'), "primary")
...
function RandomVIN()
   local random = tostring(AJFW.Shared.RandomInt(3) .. AJFW.Shared.RandomStr(3) .. AJFW.Shared.RandomInt(2)):upper()
   return random
end
...
function RandomPlate()
	local random = tostring(AJFW.Shared.RandomInt(1) .. AJFW.Shared.RandomStr(2) .. AJFW.Shared.RandomInt(3) .. AJFW.Shared.RandomStr(2)):upper()
   return random
end
...
AJFW.Functions.CreateUseableItem('tablet2' , function(source, item)
   local Player = AJFW.Functions.GetPlayer(source)
   if Player.Functions.GetItemByName('tablet2') ~= nil then
      if item.info.uses >= 1 then
         ...
         TriggerClientEvent("AJFW:Notify", source, "You System is corrupted", "error", 10000)
      end
   end
end)

-- AJFW.Functions.CreateUseableItem('hacking_device',  function (source, item)
--    TriggerClientEvent('jacob-carboost:client:useHackingDevice', source)
-- end)

AJFW.Functions.CreateUseableItem("disabler", function(source, item)
   local Player = AJFW.Functions.GetPlayer(source)
   if Player.Functions.GetItemByName('disabler') ~= nil then
       if item.info.uses >= 1 then
         TriggerClientEvent('jacob-carboost:client:useHackingDevice', source, item)
       else
           TriggerClientEvent("AJFW:Notify", source, "Chip Broken", "error", 10000)
       end
   end
end)

AJFW.Functions.CreateUseableItem('fake_plate', function(source, item)
   TriggerClientEvent('jacob-carboost:client:fakeplate', source)
end)

AJFW.Commands.Add('crypto', 'Check You Current Crypto Currency', {}, false, function(source, args)
   local src = source
   local Player = AJFW.Functions.GetPlayer(src)
