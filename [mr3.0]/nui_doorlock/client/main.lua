...
local function CheckAuth(doorData)
    ...
    if doorData.items then
        MRFW.Functions.TriggerCallback('nui_doorlock:server:CheckItems', function(result)
...
local function DoorLoop()
    Started = true
    MRFW.Functions.TriggerCallback('nui_doorlock:server:getDoorList', function(doorList)
...
local function LockpickFinish(success)
    if success then
		MRFW.Functions.Notify('Success!', 'success', 2500)
		...
		if math.random(1, 100) <= 17 then
            if usingAdvanced then
                TriggerServerEvent("MRFW:Server:RemoveItem", "advancedlockpick", 1, false)
			    TriggerEvent('inventory:client:ItemBox', MRFW.Shared.Items["advancedlockpick"], "remove")
            else
			    TriggerServerEvent("MRFW:Server:RemoveItem", "lockpick", 1, false)
			    TriggerEvent('inventory:client:ItemBox', MRFW.Shared.Items["lockpick"], "remove")
            end
		end
        MRFW.Functions.Notify('Failed..', 'error', 2500)
    end
end
            ...
            if Config.DoorList[doorID].doors then
                for k, v in pairs(Config.DoorList[doorID].doors) do
                    ...
                    elseif not Config.DoorList[doorID].locked then
                        ...
                        if MRFW.Shared.Round(v.currentHeading, 0) == MRFW.Shared.Round(v.objHeading, 0) then
                ...
                elseif not Config.DoorList[doorID].locked then
                    ...
                    if MRFW.Shared.Round(Config.DoorList [doorID].currentHeading, 0) == MRFW.Shared.Round(Config.DoorList[doorID].objHeading, 0) then
