            ...
            if dist < 20 then
                ...
                if dist < 5 then
                    ...
                    if IsControlJustPressed(0, 38) then
                        ...
                        ResetNpcTask()
                        route = route + 1
                        TriggerEvent('mr-busjob:client:DoBusNpc')
                        break                          
...
function BusGarage()
    ...
    for veh, v in pairs(Config.AllowedVehicles) do
        vehicleMenu[#vehicleMenu+1] = {
            header = v.label,
            params = {
                event = "mr-busjob:client:TakeVehicle",
                args = {
                    model = v.model
                }
            }
...
RegisterNetEvent("mr-busjob:client:TakeVehicle", function(data)
    local coords = Config.Location
    if(BusData.Active) then
        MRFW.Functions.Notify('You can only have one active bus at a time', 'error')
        return
    else
        ...
        if vehNear < 5 then
           MRFW.Functions.Notify('There is a vehicle at the spot', 'error', 3000)
            ...
            MRFW.Functions.SpawnVehicle(data.model, function(veh)
            ...
            TriggerServerEvent('mr-busjob:server:Deposit')
            Wait(1000)
            TriggerEvent('mr-busjob:client:DoBusNpc')
        end
    end
...
RegisterNetEvent('mr-busjob:client:DoBusNpc', function()
    if whitelistedVehicle() then
        if not NpcData.Active then
            ...
            MRFW.Functions.Notify('Goto the bus stop', 'success')
                    ...
                    if dist < 20 then
                        ...
                        if dist < 5 then
                            ...
                            if IsControlJustPressed(0, 38) then
                                ...
                                TriggerServerEvent('mr-busjob:server:NpcPay', math.random(50, 75))
                            end
                        end
                    end
           ...
           MRFW.Functions.Notify('You Are Already Driving Bus')
        end
    else
        MRFW.Functions.Notify('You Are Not In A Bus')
    end
end)
                            ...
                            if IsControlJustReleased(0, 38) then
                                if (not NpcData.Active or NpcData.Active and NpcData.NpcTaken == false)then
                                    if IsPedInAnyVehicle(PlayerPedId(), false) then
                                        ...
                                        TriggerServerEvent('mr-busjob:server:Withdraw')
                                        DeleteVehicle(GetVehiclePedIsIn(PlayerPedId()))
                                        RemoveBlip(NpcData.NpcBlip)
                                    end
                                else
                                    MRFW.Functions.Notify("Drop off the passengers before you stop working", "error")
                                end
                            end
                        else
