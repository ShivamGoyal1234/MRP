    ...
    while true do
        ...
        if scrapblip then
            ...
            if dist <= 3 then
                ...
                --     end
                --     TriggerServerEvent("mr-chopshop:server:callCops", "Chopshop", 0, streetLabel, pos)
                --     copsCalled = true
                -- end
                Citizen.Wait(5000)
				MRFW.Functions.Notify(Config.Locale["FoundVeh"], 'success')
				Citizen.Wait(10000)
				MRFW.Functions.Notify(Config.Locale["ScrapBlip"], 'primary')
				CreateBlip2()
...
RegisterNetEvent('mr-chopshop:jobaccept')
AddEventHandler('mr-chopshop:jobaccept', function()
    if cooldown == false then
        if LicensePlate ~= nil then
            MRFW.Functions.Notify(Config.Locale["JobActive"], 'error')
        else
        ...
        MRFW.Functions.Notify(Config.Locale["CoolDown"], 'error')
    end
end)
...
function SpawnVehicle(vehicle, x ,y, z, w)
    ...
    MRFW.Functions.SpawnVehicle(vehicle, function(veh)
    	...
    	MRFW.Functions.Notify(Config.Locale["Email"], 'success')
...
function ScrapVehicle()
    ...
    if GetVehicleNumberPlateText(veh) ~= LicensePlate then
        MRFW.Functions.Notify(Config.Locale["WrongVeh"], 'error')
	else 
		local netId = NetworkGetNetworkIdFromEntity(vehicle)
        MRFW.Functions.Notify(Config.Locale["Reminder"], 8000)
		StartChopping()
		DeleteBlip()
    end
end
...
-- RegisterNetEvent('mr-chopshop:client:robberyCall')
-- AddEventHandler('mr-chopshop:client:robberyCall', function(type, key, streetLabel, coords)
		...
		if start then
			...
			if GetVehiclePedIsIn(GetPlayerPed(-1)) == vehicle and removedpart == true then
				...
				DrawText3Ds(pos.x,pos.y,pos.z,Config.Locale["destroy"])
					if(IsControlJustPressed(1, 38))   then
						MRFW.Functions.Progressbar("crushing", Config.Locale["crushing"], (6500), false, true, {
						...
						TriggerServerEvent("mr-chopshop:server:crush")
						Citizen.Wait(1500)
...
function StartAnimation(k)
	if Config.CarTable[k].anim == "wheel1" or Config.CarTable[k].anim == "wheel2" or Config.CarTable[k].anim == "wheel3" or Config.CarTable[k].anim == "wheel4" then
		TriggerEvent('mr-chopshop:wheelanimation')
		Citizen.Wait(7400)
	...
	elseif Config.CarTable[k].anim == "trunk" then
		...
		Citizen.Wait(2500)  
		TriggerEvent('mr-chopshop:trunkanimation')
		Citizen.Wait(4000)
		Config.CarTable[k].chopped = true
		TriggerServerEvent('mr-chopshop:server:rewardplayer', Config.CarTable[k].anim)
		Citizen.Wait(9500)
	...
	elseif Config.CarTable[k].anim == "hood" then
		SetVehicleDoorOpen(vehicle, Config.CarTable[k].destroy, false, true)
		Citizen.Wait(2500)  
		TriggerEvent('mr-chopshop:hoodanimation')
		Citizen.Wait(9000)
		SetVehicleDoorBroken(vehicle,Config.CarTable[k].destroy,true)
		Config.CarTable[k].chopped = true
		TriggerServerEvent('mr-chopshop:server:rewardplayer', Config.CarTable[k].anim)
...
function VehicleToFar() 
	DeleteEntity(vehicle)
	Reset()
	MRFW.Functions.Notify(Config.Locale["FarAway"], 'error')
end
