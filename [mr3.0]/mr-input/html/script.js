...
const OpenMenu = (data) => {
    ...
    $("#mr-input-form").on("change", function (event) {
        formInputs[$(event.target).attr("name")] = $(event.target).val();
    });

    $("#mr-input-form").on("submit", async function (event) {
        if (event != null) {
            event.preventDefault();
        }
        let formData = $("#mr-input-form").serializeArray();
