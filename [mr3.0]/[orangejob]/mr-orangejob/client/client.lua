...
local targeteye = false

RegisterNetEvent('mr-orangejob:collect', function()
    -- local player, distance = MRFW.Functions.GetClosestPlayer()
    -- if distance == -1 or distance >= 4.0 then
    --     mining = true
    if not targeteye then
        targeteye = true
        FreezeEntityPosition(PlayerPedId(), true)
        TriggerEvent('close:inventory:bug')
        SetCurrentPedWeapon(PlayerPedId(), GetHashKey('WEAPON_UNARMED'))
        TriggerEvent('dpemote:custom:animation', {"mechanic5"})
        MRFW.Functions.Progressbar('drillling stone', 'Picking Orange...', 3000, false, true, { -- Name | Label | Time | useWhileDead | canCancel
...
RegisterNetEvent('mr-orangejob:orangejuice', function()
    if not cooldown then
        cooldown = true
        MRFW.Functions.TriggerCallback('orange:ingredient', function(result, cb)
            hasBagd7 = result
            s1d7 = true
        end)
        while(not s1d7) do
        ...
        if (hasBagd7) then
            ...
            MRFW.Functions.Notify('You are missing empty bottle or oranges..', 'error')
        end
    else
...
function Processorange()
    ...
    DeleteEntity(prop)
    TriggerServerEvent('orange:process')
    cooldown = false
