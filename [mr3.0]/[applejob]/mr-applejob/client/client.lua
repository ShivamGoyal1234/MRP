...
RegisterNetEvent('mr-applejob:collect', function()
    ...
    --     mining = true
        ...
        MRFW.Functions.Progressbar('drillling stone', 'Picking Apple...', 3000, false, true, { -- Name | Label | Time | useWhileDead | canCancel
...
--         Citizen.Wait(200)
--     end
--    while MRFW.Functions.GetPlayerData().job == nil do
-- 		Citizen.Wait(10)
--     end
...
--                         SetCurrentPedWeapon(PlayerPedId(), GetHashKey('WEAPON_UNARMED'))
--                         TriggerEvent('dpemote:custom:animation', {"mechanic5"})
--                         MRFW.Functions.Progressbar('drillling stone', 'Picking Apple...', 10000, false, true, { -- Name | Label | Time | useWhileDead | canCancel
--                             disableMovement = true,
--                             disableCarMovement = true,
--                             disableMouse = false,
--                             disableCombat = true,
...
RegisterNetEvent('mr-applejob:applejuice', function()
    if not cooldown then
        cooldown = true
        MRFW.Functions.TriggerCallback('apple:ingredient', function(result, cb)
            hasBagd7 = result
            s1d7 = true
        end)
        while(not s1d7) do
        ...
        if (hasBagd7) then
            ...
            MRFW.Functions.Notify('You are missing empty bottle or apples..', 'error')
        end
        Wait(3000)
        cooldown = false
    else
...
function Processapple()
    ...
    local inventory = MRFW.Functions.GetPlayerData().inventory
    local count = 0
    ----
    
    if(count == 0) then
    MRFW.Functions.Progressbar("search_register", "Process..", 5000, false, true, {disableMovement = true,
                        disableCarMovement = true,
                        disableMouse = false,
                        disableCombat = true,
