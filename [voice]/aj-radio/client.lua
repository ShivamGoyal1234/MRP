...
local function connecttoradio(channel)
    ...
    if SplitStr(tostring(channel), ".")[2] ~= nil and SplitStr(tostring(channel), ".")[2] ~= "" then
        AJFW.Functions.Notify(Config.messages['joined_to_radio'] ..channel.. ' MHz', 'success')
    else
        AJFW.Functions.Notify(Config.messages['joined_to_radio'] ..channel.. '.00 MHz', 'success')
    end
end
...
local function leaveradio()
    ...
    TriggerServerEvent("aj-log:server:CreateLog", "radio", "Leave Radio", "red", "**"..playerName.."** Just Leave This Radio Channel **(**`"..tonumber(RadioChannel).."`**)**")
    RadioChannel = 0
    onRadio = false
    exports["pma-voice"]:setRadioChannel(0)
    exports["pma-voice"]:setVoiceProperty("radioEnabled", false)
    AJFW.Functions.Notify(Config.messages['you_leave'] , 'error')
end
            ...
            if rchannel ~= RadioChannel then
                if rchannel < 12 then
                    if (PlayerData.job.name == 'police' or PlayerData.job.name == 'doctor' or PlayerData.job.name == 'government') and PlayerData.job.onduty then
                        TriggerServerEvent("aj-log:server:CreateLog", "radio", "Joined Radio", "green", "**"..playerName.."** Just Joined This Radio Channel **(**`"..tonumber(data.channel).."`**)**")
                        connecttoradio(rchannel)
                    else
                        AJFW.Functions.Notify(Config.messages['restricted_channel_error'], 'error')
                    end
                else
                    TriggerServerEvent("aj-log:server:CreateLog", "radio", "Joined Radio", "green", "**"..playerName.."** Just Joined This Radio Channel **(**`"..tonumber(data.channel).."`**)**")
                    connecttoradio(rchannel)
                ...
                AJFW.Functions.Notify(Config.messages['you_on_radio'] , 'error')
            end
        else
            AJFW.Functions.Notify(Config.messages['invalid_radio'] , 'error')
        end
    else
        AJFW.Functions.Notify(Config.messages['invalid_radio'] , 'error')
    end
...
RegisterNUICallback('leaveRadio', function(data, cb)
    if RadioChannel == 0 then
        AJFW.Functions.Notify(Config.messages['not_on_radio'], 'error')
    else
        leaveradio()
    end
end)
...
RegisterNUICallback("volumeUp", function()
	if RadioVolume <= 95 then
		...
		AJFW.Functions.Notify(Config.messages["volume_radio"] .. RadioVolume, "success")
		exports["pma-voice"]:setRadioVolume(RadioVolume)
	else
		AJFW.Functions.Notify(Config.messages["decrease_radio_volume"], "error")
	end
end)
...
RegisterNUICallback("volumeDown", function()
	if RadioVolume >= 10 then
		...
		AJFW.Functions.Notify(Config.messages["increase_radio_volume"], "error")
	end
end)
    ...
    if newChannel < 11 then
        if PlayerData.job.name == 'police' or PlayerData.job.name == 'doctor' or PlayerData.job.name == 'government' then
            if PlayerData.job.onduty then
                ...
                AJFW.Functions.Notify(Config.messages["increase_decrease_radio_channel"] .. newChannel, "success")
            else
                AJFW.Functions.Notify(Config.messages['restricted_channel_error_duty'], 'error')
            end
        else
            AJFW.Functions.Notify(Config.messages['restricted_channel_error'], 'error')
        end
...
CreateThread(function()
    while true do
        ...
        if LocalPlayer.state.isLoggedIn and onRadio then
            AJFW.Functions.TriggerCallback('aj-radio:server:GetItem', function(hasItem)
                if not hasItem then
                    if RadioChannel ~= 0 then
                        leaveradio()
                    end
...
RegisterCommand('+o', function()
    ...
    TriggerEvent("AJFW:Notify", "Voice Reconnected", "success", 3000)
end)
