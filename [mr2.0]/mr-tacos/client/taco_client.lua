...
AddEventHandler("MRFW:Client:OnPlayerLoaded", function()
    MRFW.Functions.TriggerCallback('mr-tacos:server:GetConfig', function(config)
        Config = config
    end)
end)
				 ...
				 if IsControlJustPressed(0, Keys['E']) then
				  if not Bezig then
					...
					elseif Config.JobData['locations'][k]['name'] == 'Meat' then
						BakeMeat()
					elseif Config.JobData['locations'][k]['name'] == 'Shell' then
						MRFW.Functions.TriggerCallback('mr-taco:server:get:ingredient', function(HasItems)  
                        if HasItems then
							FinishTaco()
						else
							MRFW.Functions.Notify("You don\'t have all the Ingredients yet.", "error")
						end
					end)
					elseif Config.JobData['locations'][k]['name'] == 'Register' then
						TakeMoney()
					...
					elseif Config.JobData['locations'][k]['name'] == 'GiveTaco' then
						...
						MRFW.Functions.Notify("You're still doing something man bro.", "error")
					end
				end
			end
		end
...
function FinishTaco()
	...
	MRFW.Functions.Progressbar("pickup_sla", "Making taco..", 3500, false, true, {
		...
		TriggerServerEvent('MRFW:Server:RemoveItem', "meat", 1)
		TriggerServerEvent('MRFW:Server:RemoveItem', "lettuce", 1)
		TriggerEvent("inventory:client:ItemBox", MRFW.Shared.Items["meat"], "remove")
		TriggerEvent("inventory:client:ItemBox", MRFW.Shared.Items["lettuce"], "remove")
		TriggerEvent("inventory:client:ItemBox", MRFW.Shared.Items["taco"], "add")
		TriggerServerEvent('MRFW:Server:AddItem', "taco", 1)
...
function BakeMeat()
	...
	MRFW.Functions.Progressbar("pickup_sla", "Baking meat..", 5000, false, true, {
		disableMovement = true,
		disableCarMovement = false,
		disableMouse = false,
		disableCombat = false,
		...
		TriggerServerEvent('MRFW:Server:AddItem', "meat", 1)
		Config.JobData['stock-meat']= Config.JobData['stock-meat'] - 1
		TriggerEvent("inventory:client:ItemBox", MRFW.Shared.Items["meat"], "add")
		Bezig = false
	end, function()
		MRFW.Functions.Notify("Canceled..", "error")
		Bezig = false
	...
	MRFW.Functions.Notify("There is not enough meat in stock.", "error")
 end  
end
...
function GetLettuce()
	...
	MRFW.Functions.Progressbar("pickup_sla", "Grabbing lettuce..", 4100, false, true, {
		disableMovement = true,
		disableCarMovement = false,
		disableMouse = false,
		disableCombat = false,
		...
		TriggerServerEvent('MRFW:Server:AddItem', "lettuce", 1)
		Config.JobData['stock-lettuce']= Config.JobData['stock-lettuce'] - 1
		TriggerEvent("inventory:client:ItemBox", MRFW.Shared.Items["lettuce"], "add")
		Bezig = false
	end, function()
		StopAnimTask(PlayerPedId(), "amb@prop_human_bum_bin@idle_b", "idle_d", 1.0)
		MRFW.Functions.Notify("Canceled..", "error")
	...
	MRFW.Functions.Notify("There is not enough lettuce in stock.", "error")
 end 
end

function GiveTacoToShop()
	MRFW.Functions.TriggerCallback('mr-taco:server:get:tacos', function(HasItem, type)
		if HasItem then
		  if not IsPedInAnyVehicle(PlayerPedId(), false) then
			if Config.JobData['tacos'] <= 9 then	
				MRFW.Functions.Notify("Taco delivered!", "success")
				Config.JobData['tacos'] = Config.JobData['tacos'] + 1
				TriggerServerEvent('MRFW:Server:RemoveItem', "taco", 1)
				TriggerEvent("inventory:client:ItemBox", MRFW.Shared.Items["taco"], "remove")
				else
					MRFW.Functions.Notify("There are still 10 taco\'s that have to be sold. We dont waste food here..", "error")
				end
		  elseif type == 'green' then
			if Config.JobData['green-tacos'] <= 9 then	
				TriggerServerEvent('MRFW:Server:RemoveItem', "taco", 1)
				TriggerEvent("inventory:client:ItemBox", MRFW.Shared.Items["green-taco"], "remove")
				else
					MRFW.Functions.Notify("There are still 10 taco\'s that have to be sold. We dont waste food here..", "error")
				end
		end
	    else
		MRFW.Functions.Notify("You dont even have a taco.", "error")
	 end
	end)
end

function AddStuff()
	MRFW.Functions.TriggerCallback('MRFW:HasItem', function(HasItem)
		if HasItem then
			if Config.JobBusy == true then
				TriggerServerEvent('MRFW:Server:RemoveItem', "taco-box", 1)
				TriggerEvent("inventory:client:ItemBox", MRFW.Shared.Items["taco-box"], "remove")
				...
				MRFW.Functions.Notify("Taco Shop has been restocked!", "success")
				Config.JobBusy = false
			else
				MRFW.Functions.Notify("You\'re coming straight from the taco store.", "error")
			end
		else
			MRFW.Functions.Notify("You don\'t even have a box of ingredients.", "error")
		end
	end, 'taco-box')
end
...
function TakeMoney()
	if Config.JobData['register'] >= 10000 then
		...
		MRFW.Functions.Progressbar("search_register", "Empty cash register..", lockpickTime, false, true, {
            disableMovement = true,
		...
		MRFW.Functions.Notify("There is not enough money in the register yet.", "error")
	end
end
