...
Citizen.CreateThread(function()
    ...
    MRFW.Functions.TriggerCallback('jacob:get:meth', function(loc, loc2, loc3)
...
Citizen.CreateThread(function()
    Wait(500)
    MRFW.Functions.TriggerCallback('mr-methlab:server:GetData', function(data)
                ...
                if(GetDistanceBetweenCoords(pos, Config.Locations["laptop"].coords.x, Config.Locations["laptop"].coords.y, Config.Locations["laptop"].coords.z, true) < 1)then
                    ...
                    if IsControlJustPressed(0, Keys["E"]) then
                        if not complete then
                            if not hacki then
                                MRFW.Functions.TriggerCallback('MRFW:HasItemV2', function(result,iteminfo)
                                --StartHackingm()
                                    if result then
                                        if iteminfo.info.uses >= 1 then
                                            TaskStartScenarioInPlace(PlayerPedId(), "PROP_HUMAN_BUM_BIN", 0, true)
                                            MRFW.Functions.Progressbar("starting", "Hacking..", 10000, false, true, {}, {}, {}, {}, function() -- Done
                                                local infos = {}
                                                infos.uses = iteminfo.info.uses - 1
                                                TriggerServerEvent('MRFW:Server:RemoveItem', iteminfo.name, iteminfo.amount, iteminfo.slot)
                                                TriggerServerEvent("MRFW:Server:AddItem", iteminfo.name, iteminfo.amount, iteminfo.slot, infos)
                                                hacki = true
                                                TriggerEvent("mr-fingerprint:Start")
                                            end)
                                        else
                                            MRFW.Functions.Notify('itemBroken', 'error', 3000)
                                        end
                                    else 
                                        MRFW.Functions.Notify("You\'re Missing Something", "error")
                                    end
                                end , "trojan_usb")
                            end
                        else
                            MRFW.Functions.Notify("You\'re already doing this work", "error")
                        end
                    end
                            ...
                            if(GetDistanceBetweenCoords(pos, Config.Ingredients["lab"].coords.x, Config.Ingredients["lab"].coords.y, Config.Ingredients["lab"].coords.z, true) < 20)then
                                ...
                                if(GetDistanceBetweenCoords(pos, Config.Ingredients["lab"].coords.x, Config.Ingredients["lab"].coords.y, Config.Ingredients["lab"].coords.z, true) < 1)then
                                    if CarryPackage == nil then
                                        ...
                                        if IsControlJustPressed(0, Keys["E"]) then
                                            TaskStartScenarioInPlace(PlayerPedId(), "PROP_HUMAN_BUM_BIN", 0, true)
                                            MRFW.Functions.Progressbar("pickup_reycle_package", "Grabbing ingredients..", 5000, false, true, {
...
RegisterNetEvent('mr-methlab:client:UseLabKey')
AddEventHandler('mr-methlab:client:UseLabKey', function(labkey)
    if ClosestMethlab == Config.CurrentLab then
        ...
        if dist < 1 then
            if labkey == ClosestMethlab then
                -- MRFW.Functions.TriggerCallback('methlab:check:SecretDrive', function(HasDrive)
                    -- if HasDrive then
                        EnterMethlab()
                    -- else
                        -- MRFW.Functions.Notify('Something is missing you Can\'t enter here', 'error')
                    -- end
                -- end)
            else
                MRFW.Functions.Notify('This isn\'t the right key..', 'error')
            end
        end
    end
end)
...
function TakeIngredients()
    ...
    TriggerServerEvent("mr-methlab:server:receiveoxy")
end
...
function processmeth()
	MRFW.Functions.TriggerCallback('mr-methlab:server:ingredient', function(HasItem, type)
        if HasItem then
			...
			MRFW.Functions.Progressbar("process-", "Processing..", 18000, false, true, {
				disableMovement = true,
				disableCarMovement = true,
				disableMouse = false,
				disableCombat = true,
			}, {}, {}, {}, function()
				TriggerServerEvent("mr-methlab:server:processed")
				ClearPedTasks(PlayerPedId())
		...
		MRFW.Functions.Notify("You have ingredients", "error")
		end
	end)
end

function packmeth()
	MRFW.Functions.TriggerCallback('mr-methlab:server:packing', function(HasItem, type)
        if HasItem then
            local playerPed = PlayerPedId()
            TaskStartScenarioInPlace(playerPed, "PROP_HUMAN_PARKING_METER", 0, true)
			MRFW.Functions.Progressbar("process-", "Packing..", 15000, false, true, {
				disableMovement = true,
				disableCarMovement = true,
				disableMouse = false,
				disableCombat = true,
			}, {}, {}, {}, function()
				TriggerServerEvent("mr-methlab:server:packed",3)
				ClearPedTasks(PlayerPedId())
		...
		MRFW.Functions.Notify("You Missing Something", "error")
		end
	end)
end
...
function CreateAccetonVehicle()
	...
	MRFW.Functions.SpawnVehicle(carName, function(veh)
        -- print(plateText)
        ...
        exports['mr-fuel']:SetFuel(veh, math.random(50,70))
    ...
    while complete do
        ...
        if am > 0 then
            ...
            if done then
                ...
                if first then
                    MRFW.Functions.Notify("wait for few seconds for location", "error")
                    first = false
                else
                    MRFW.Functions.Notify("wait for few seconds for next location", "error")
                end
                -- AddCancelBlip()
        ...
        if dit2 <= 20 then
            ...
            if dit2 <= 10 then
                ...
                if dit2 <= 2 then
                    ...
                    local secondplate = tostring(MRFW.Functions.GetPlate(vehi))
                    ...
                    if IsControlJustPressed(0, 38) then
                        if firstplate == secondplate then
                            ...
                            MRFW.Functions.Notify("bewaqoof samjha he kiya same start car return karo warna chatko", "error")
                        end
                    end
                ...
                if dist <= 2 then
                    ...
                    if IsControlJustReleased(0, 38) then
                        if not IsPedInAnyVehicle(PlayerPedId(), false) then
                            -- MRFW.Functions.Progressbar("pickss", "Picking..", 10000, false, true, {}, {}, {}, {}, function() -- Done
                            --     p = false
                            --     RemoveBlipObj()
                            --     TriggerServerEvent('mr-methlab:server:re:ac')
                            --     am = am - 1
                            --     done = true
                            -- end)

                            MRFW.Functions.Progressbar("pickss", "Picking..", 10000, false, true, {
                                disableMovement = true,
                                disableCarMovement = true,
                                disableMouse = false,
                                disableCombat = true,
                            ...
                            MRFW.Functions.Notify("You Need To Get Out Of The Vehicle", "error")
                        end
                    end
                elseif dist <= 5 then
