...
function PullOutVehicle()
    ...
    if ownsVan == true then
        MRFW.Functions.Notify("You already have a van! Go and collect it or end your job.", "error")
    elseif ownsVan == false then
        coords = Config.Zones.Spawn.Pos
        MRFW.Functions.SpawnVehicle('blazer3', function(veh)
            ...
            plaquevehicule = MRFW.Functions.GetPlate(veh)
        end, coords, true)
        InService = true
        DrawTarget()
        AddCancelBlip()
        ownsVan = true
        TriggerServerEvent("mr-pizzaruns:TakeDeposit")
    end
end
            ...
            if dist <= 2.5 then
                ...
                if dist <= 3.0 then
                    if IsControlJustReleased(0, 38) then
                        local vehNear = MRFW.Functions.CheckFuckingCar(Config.Zones.Spawn.Pos)
                        if vehNear < 5 then
...
function DrawTarget()
    ...
    if DeliveriesCount == 4 then
        MRFW.Functions.Notify("All Pizzas have been delivered","success")
        RemoveCancelBlip()
      ...
      if pizza == 1 then
        MRFW.Functions.Notify("You have one pizza","success")
      else
        if pizza == 4 then
          pizza = 'Four'
        elseif pizza == 3 then
        ...
        MRFW.Functions.Notify("You have "..pizza.." pizzas","success")
      end
        if LastGoal == RandomPoint then
		    ...
		    MRFW.Functions.Notify("Take the Next Pizza to the customer","success")
        end
    end
end
    ...
    if not IsPedInAnyVehicle(player, false) then
        ...
        if ( DoesEntityExist( player ) and not IsEntityDead( player )) then
            ...
            if HasPizza then
                ...
                HasPizza = true
                MRFW.Functions.Notify("Take the Pizza to the customer","success")
            end
        end
    end
end
...
function DeliverPizza()
    if not PizzaDelivered then
        ...
        HasPizza = false    
        NextDelivery()
        TriggerServerEvent('mr-pizzaruns:items')
        Citizen.Wait(2500)
...
function NextDelivery()
    TriggerServerEvent('mr-pizzaruns:Payment')
    Citizen.Wait(300)
    DrawTarget()
end
-----------------
            ...
            if Delivered then
                if DistanceFromEndZone <= 2.5 then
                    ...
                    if DistanceFromEndZone <= 7 then
                        if IsControlJustReleased(0, 38) then
                            MRFW.Functions.Notify("The work is finished! Rest a while, the next orders are already waiting!", "success")
                            EndOfWork()
                        end
                    end
                end
                ...
                if DistanceFromCancelZone <= 2.5 then
                    ...
                    if DistanceFromCancelZone <= 7 then
                        if IsControlJustReleased(0, 38) then
                            MRFW.Functions.Notify("Orders Canceled!", "error")
							EndOfWork()
                        end
                    end
                end
...
function EndOfWork()
    ...
    if IsPedInAnyVehicle(ped, false) then
        ...
        if IsVehicleModel(Van, GetHashKey('blazer3')) then
            MRFW.Functions.DeleteVehicle(Van)
            if Delivered == true then
                TriggerServerEvent("mr-pizzaruns:ReturnDeposit", 'end')
            end
            Hired = false
            InService = false
            BlipSell = nil
            ...
            MRFW.Functions.Notify("You must return to pizza blazer3!", "error")
            MRFW.Functions.Notify("If you lost the blazer3 cancel the job on foot", "error")
        end
    else
        Hired = false
        InService = false
