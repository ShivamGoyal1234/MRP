...
RegisterNetEvent('MRFW:Client:OnPlayerLoaded', function()
	MRFW.Functions.TriggerCallback('mr-tattoos:GetPlayerTattoos', function(tattooList)
		if tattooList then
			Tattoosss = tattooList
		end
	end)
end)
...
RegisterNetEvent('mr-tattoos:update', function(data)
	Tattoosss = data
end)
...
local outfitFemale = {outfitData = {
	...
	["vest"]        = { item = 0, texture = 0},  -- Body Vest
	["torso2"]      = { item = 15, texture = 0},  -- Jas / Vesten
	["shoes"]       = { item = 53, texture = 0},  -- Schoenen
	["decals"]      = { item = 0, texture = 0},  -- Decals
	["accessory"]   = { item = 0, texture = 0},  -- Nek / Das
	["bag"]         = { item = 0, texture = 0},  -- Tas
	["hat"]         = { item = -1, texture = -1},  -- Pet
...
function GetNaked()
	...
	if GetEntityModel(PlayerPedId()) == `mp_m_freemode_01` then
		...
		TriggerEvent('mr-clothing:client:loadOutfit', outfitMan)
	elseif GetEntityModel(PlayerPedId()) == `mp_f_freemode_01` then
		...
		TriggerEvent('mr-clothing:client:loadOutfit', outfitFemale)
	else
		MRFW.Functions.TriggerCallback('mr-tattoos:GetPlayerTattoos', function(tattooList)
			if tattooList then
				ClearPedDecorations(PlayerPedId())
...
function BuyTattoo(collection, name, label, price)
	MRFW.Functions.TriggerCallback('mr-tattoos:PurchaseTattoo', function(success)
		if success then
...
function RemoveTattoo(name, label)
	...
	TriggerServerEvent("mr-tattoos:RemoveTattoo", currentTattoos)
	MRFW.Functions.Notify("You have tattoo "..GetLabelText(label).. " deleted.", "error", 2500)
end
...
RegisterNetEvent('mr-tattoo:client:tattooppen')
AddEventHandler('mr-tattoo:client:tattooppen', function()
    OpenTattooShop()
end)
