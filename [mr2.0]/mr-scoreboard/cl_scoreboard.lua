...
AddEventHandler("MRFW:Client:OnPlayerLoaded", function()
    ...
    MRFW.Functions.TriggerCallback('jacob:custom:getperms', function(UserGroup)
        group = UserGroup
    end)
end)
...
RegisterNetEvent('MRFW:Client:OnPermissionUpdate', function(UserGroup)
    group = UserGroup
end)

RegisterNetEvent('MRFW:Client:OnJobUpdate')
AddEventHandler('MRFW:Client:OnJobUpdate', function(JobInfo)
    MRFW.Functions.TriggerCallback('shitidk', function(g)
        group = g
    end)
end)
...
Citizen.CreateThread(function()
    while true do
        Citizen.Wait(10000)
        MRFW.Functions.TriggerCallback('Jacob:custom:count', function(ccc)
            tplayers = ccc
        end)
    end
end)
