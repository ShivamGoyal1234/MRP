...
MRFW.Functions.CreateCallback('jacob:custom:getperms', function(source, cb)
    local group = MRFW.Functions.GetPermission(source)
    cb(group)
end)

MRFW.Functions.CreateCallback('Jacob:custom:count', function(source, cb)
...
function ST.Scoreboard.AddAllPlayers(self)
    ...
    for i=1, #xPlayers, 1 do
        ...
        local group = MRFW.Functions.GetPermission(xPlayers[i])
        local stid = string.sub(steamIdentifier, 9)
