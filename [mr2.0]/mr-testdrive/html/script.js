...
function menuVehicle(event) {
    ...
    $.post('https://mr-testdrive/menuSelected', JSON.stringify({menuId: categoryVehicleSelected.toLowerCase()}));
}

function testDrive(){
    $.post('https://mr-testdrive/TestDrive', JSON.stringify({vehicleModel: vehicleSelected.modelcar}));    
...
function buyVehicle(option) {
    ...
    switch(option){
        case 'cancel':
            break;
        case 'confirm':
            $.post('https://mr-testdrive/Buy', JSON.stringify(vehicleSelected));
            break;
    }   
}
...
$(document).on('keydown', function() {
    switch(event.keyCode) {
        ...
        case 17: // TAB
            break;
        case 68: // LEFT A
            $.post('https://mr-testdrive/rotate', JSON.stringify({key: "left"}))
            break;            
        case 65: // RIGHT D
            $.post('https://mr-testdrive/rotate', JSON.stringify({key: "right"}))
            break;
    }
});
...
$(document).on('keydown', function(ev) {
    ...
    if (_key) {
        $.post('https://mr-testdrive/rotate', JSON.stringify({key: _key}))
    }
    ...
    switch(event.which) {        
        ...
        case 1:
            ...
            if(!itemDisabled) {
                ...
                $.post("https://mr-testdrive/SpawnVehicle", JSON.stringify({ modelcar: dataCar[9].outerText, price: dataCar[7].outerText }));
...
function setVehicleColorRGB(R,G,B){
    ...
    if (selectedColor == 'primary'){
        $.post("https://mr-testdrive/RGBVehicle", JSON.stringify({primary: true, R: R, G: G, B: B}));
    } else {
        $.post("https://mr-testdrive/RGBVehicle", JSON.stringify({primary: false, R: R, G: G, B: B}));
    }
}
