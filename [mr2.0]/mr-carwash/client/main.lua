                        ...
                        if Driver == PlayerPed then
                            if not washingVehicle then
                                ...
                                if IsControlJustPressed(0, Keys["E"]) then
                                    TriggerServerEvent('mr-carwash:server:washCar')
                                    washVirus()
                                end
                            else
...
RegisterNetEvent('mr-carwash:client:washCar')
AddEventHandler('mr-carwash:client:washCar', function()
    local PlayerPed = PlayerPedId()
    local PedVehicle = GetVehiclePedIsIn(PlayerPed)
    local Driver = GetPedInVehicleSeat(PedVehicle, -1)
    ...
    MRFW.Functions.Progressbar("search_cabin", "Vehicle is being washed..", math.random(4000, 8000), false, true, {
        ...
        MRFW.Functions.Notify("Washing canceled..", "error")
        washingVehicle = false
    end)
end)
