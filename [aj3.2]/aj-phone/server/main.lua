    ...
    if Player ~= nil then
        ...
        if garageresult[1] ~= nil then
            for k, v in pairs(garageresult) do
                ...
                if (AJFW.Shared.Vehicles[vehicleModel] ~= nil) and (Garages[v.garage] ~= nil) then
                    v.garage = Garages[v.garage].label
                    v.vehicle = AJFW.Shared.Vehicles[vehicleModel].name
                    v.brand = AJFW.Shared.Vehicles[vehicleModel].brand
                end
...
AJFW.Functions.CreateCallback('aj-phone:server:PayInvoice', function(source, cb, society, amount, invoiceId, sendercitizenid)
    ...
    TriggerEvent('aj-phone:server:sendNewMailToOffline', sendercitizenid, invoiceMailData)
    TriggerEvent("aj-bossmenu:server:addAccountMoney", society, amount)
...
AJFW.Functions.CreateCallback('aj-phone:server:DeclineInvoice', function(source, cb, sender, amount, invoiceId)
    local Invoices = {}
    local Ply = AJFW.Functions.GetPlayer(source)
    MySQL.query('DELETE FROM phone_invoices WHERE id = ?', {invoiceId})
    local invoices = MySQL.query.await('SELECT * FROM phone_invoices WHERE citizenid = ?', {Ply.PlayerData.citizenid})
...
AJFW.Functions.CreateCallback('aj-phone:server:GetContactPictures', function(source, cb, Chats)
    for k, v in pairs(Chats) do
        local Player = AJFW.Functions.GetPlayerByPhone(v.number)
...
AJFW.Functions.CreateCallback('aj-phone:server:GetContactPicture', function(source, cb, Chat)
...
AJFW.Functions.CreateCallback('aj-phone:server:GetPicture', function(source, cb, number)
    local Picture = nil
    local query = '%' .. number .. '%'
    local result = MySQL.query.await('SELECT * FROM players WHERE charinfo LIKE ?', {query})
    if result[1] ~= nil then
...
AJFW.Functions.CreateCallback('aj-phone:server:FetchResult', function(source, cb, search)
...
AJFW.Functions.CreateCallback('aj-phone:server:GetVehicleSearchResults', function(source, cb, search)
    local search = escape_sqli(search)
    local searchData = {}
    local query = '%' .. search .. '%'
...
AJFW.Functions.CreateCallback('aj-phone:server:GetCurrentLawyers', function(source, cb)
    local Lawyers = {}
...
AJFW.Functions.CreateCallback("aj-phone:server:GetWebhook",function(source,cb)
		cb(WebHook)
end)
...
RegisterNetEvent('aj-phone:server:sendNewMailToOffline', function(citizenid, mailData)
...
RegisterNetEvent('aj-phone:server:MentionedPlayer-dark', function(firstName, lastName, AnonMessage)
    for k, v in pairs(AJFW.Functions.GetPlayers()) do
        local Player = AJFW.Functions.GetPlayer(v)
        if Player ~= nil then
            if (Player.PlayerData.charinfo.firstname == firstName and Player.PlayerData.charinfo.lastname == lastName) then
                ...
                TriggerClientEvent('aj-phone:client:GetMentioned-dark', Player.PlayerData.source, AnonMessage, AppAlerts[Player.PlayerData.citizenid]["twitter"])
            else
                local query1 = '%' .. firstName .. '%'
                local query2 = '%' .. lastName .. '%'
...
RegisterNetEvent('aj-phone:server:UpdateHashtags-dark', function(Handle, messageData)
    ...
    TriggerClientEvent('aj-phone:client:UpdateHashtags-dark', -1, Handle, messageData)
end)
...
RegisterNetEvent('aj-phone:server:DeleteAnon', function(anonId)
    local Player = AJFW.Functions.GetPlayer(source)
    local delete = false
    local TID = anonId
    ...
    if delete then
        ...
        TriggerClientEvent('aj-phone:client:UpdateAnons', -1, DNData)
    end
end)
...
RegisterNetEvent('aj-phone:server:UpdateAnons', function(NewAnons, AnonData)
    local src = source
	...
	TriggerClientEvent('aj-phone:client:UpdateAnons', -1, src, NewAnons, AnonData, false)
    else
	local InsertAnon = MySQL.Async.insert('INSERT INTO phone_anons (citizenid, firstName, lastName, message, date, url, picture, anonid, anon) VALUES (?, ?, ?, ?, NOW(), ?, ?, ?, ?)', {
    AnonData.citizenid,
	AnonData.firstName,
...
RegisterNetEvent('aj-phone:server:TransferMoney', function(iban, amount)
    ...
    if result[1] ~= nil then
        ...
        TriggerClientEvent('AJFW:Notify', src, "That account number doesn't exist!", "error")
    end
end)

RegisterNetEvent('aj-phone:server:EditContact', function(newName, newNumber, newIban, oldName, oldNumber, oldIban)
    local src = source
    local Player = AJFW.Functions.GetPlayer(src)
    MySQL.query(
...
RegisterNetEvent('aj-phone:server:UpdateMessages', function(ChatMessages, ChatNumber, New)
    local src = source
...
AJFW.Commands.Add('ac', 'accept Call', {}, false, function(source, args)
    TriggerClientEvent('accept:call',source)
end)

AJFW.Commands.Add('cc', 'cancel Call', {}, false, function(source, args)
...
RegisterNetEvent('aj-pings:server:SendPing2', function(id)
    local src = source

    TriggerClientEvent('aj-pings:client:DoPing', src, tonumber(id))
end)

RegisterNetEvent('aj-pings:server:acceptping', function()
    local src = source
    local Player = AJFW.Functions.GetPlayer(src)
    if Pings[src] ~= nil then
        TriggerClientEvent('aj-pings:client:AcceptPing', src, Pings[src], AJFW.Functions.GetPlayer(Pings[src].sender))
        TriggerClientEvent('AJFW:Notify', Pings[src].sender, Player.PlayerData.source.." ID accepted your ping request!")
        Pings[src] = nil
    else
        TriggerClientEvent('AJFW:Notify', src, "You have no ping...", "error")
    end
end)

RegisterNetEvent('aj-pings:server:denyping', function()
    local src = source
    local Player = AJFW.Functions.GetPlayer(src)
    if Pings[src] ~= nil then
        TriggerClientEvent('AJFW:Notify', Pings[src].sender, "Your ping request has been rejected...", "error")
        TriggerClientEvent('AJFW:Notify', src, "You turned down the ping...", "success")
        Pings[src] = nil
    else
        TriggerClientEvent('AJFW:Notify', src, "You have no ping...", "error")
    end
...
RegisterNetEvent('aj-pings:server:SendPing', function(id)
    local src = source
    local Player = AJFW.Functions.GetPlayer(src)
    local Target = AJFW.Functions.GetPlayer(id)
    ...
    if Target ~= nil then
        ...
        if OtherItem ~= nil then
            TriggerClientEvent('AJFW:Notify', src, "You have requested the location of ID "..Target.PlayerData.source)
            Pings[id] = {
                coords = coords,
                sender = src,
            }
            TriggerClientEvent('AJFW:Notify', id, "You have received a ping request from ID "..Player.PlayerData.source..". Use the app to allow or reject!")
            TriggerClientEvent('aj-phone:ping:client:UiUppers', id, true)
        else
            TriggerClientEvent('AJFW:Notify', src, "Could not send ping...", "error")
        end
    else
        TriggerClientEvent('AJFW:Notify', src, "This person is not in the city...", "error")
    end
end)

RegisterNetEvent('aj-pings:server:SendLocation', function(PingData, SenderData)
    TriggerClientEvent('aj-pings:client:SendLocation', PingData.sender, PingData, SenderData)
end)

AJFW.Commands.Add("p#", "Provide Phone Number", {{name = "id", help='id of player'}}, false, function(source, args)
    local src = source
    local Player = AJFW.Functions.GetPlayer(src)
    ...
    if target and target ~='' then
        ...
        if tPLayer then
            if tPLayer.PlayerData.source ~= src then
                ...
                if dist < 2 then
                    TriggerClientEvent("aj-phone:client-annphonenumber", tPLayer.PlayerData.source, number, true)
                end
            end
        else
            TriggerClientEvent("AJFW:Notify", src, "Player is not online", "error", 5000)
        end
        TriggerClientEvent("aj-phone:client-annphonenumber", src, number, true)
    else
        local Players = AJFW.Functions.GetPlayers()
        for k,v in pairs(Players) do
            if v ~= src then
                local tPLayer = AJFW.Functions.GetPlayer(tonumber(v))
                if tPLayer then
                    local ped = GetPlayerPed(src)
                    local coords = GetEntityCoords(ped)
                    ...
                    local dist = #(coords - coords2)
                    if dist < 2 then
                        TriggerClientEvent("aj-phone:client-annphonenumber", tPLayer.PlayerData.source, number, false)
                    end
                end
            end
        end
        TriggerClientEvent("aj-phone:client-annphonenumber", src, number, false)
    end
end)
...
RegisterNetEvent('aj-phone:server:DeleteAndClearTable', function()
    ...
    BetNumber = 0
    TriggerClientEvent('aj-phone:client:addbetForAll', -1, CasinoTable)
    TriggerClientEvent('AJFW:Notify', src, "Done", "success")
end)

AJFW.Functions.CreateCallback('aj-phone:server:CheckHasBetTable', function(source, cb)
    cb(CasinoTable)
end)
...
AJFW.Functions.CreateCallback('aj-phone:server:CheckHasBetStatus', function(source, cb)
    cb(casino_status)
end)
...
RegisterNetEvent('aj-phone:server:SetJobJobCenter', function(data)
    local src = source
    local Player = AJFW.Functions.GetPlayer(src)
    if Player.Functions.SetJob(data.job) then
        TriggerClientEvent('AJFW:Notify', src, 'Changed your job to: '..data.label)
    else
        TriggerClientEvent('AJFW:Notify', src, 'Invalid Job...', 'error')
    end
end)

local EmploymentGroup = {}
RegisterNetEvent('aj-phone:server:employment_CreateJobGroup', function(data)
    local src = source
    local Player = AJFW.Functions.GetPlayer(src)
    local CSN = Player.PlayerData.citizenid
    if Player then
        if not EmploymentGroup[CSN] then
            ...
            TriggerClientEvent('aj-phone:client:EveryoneGrupAddsForAll', -1, EmploymentGroup)
            TriggerClientEvent('AJFW:Notify', src, "Group created!", "success")
        else
            TriggerClientEvent('AJFW:Notify', src, "You have already created a group...", "error")
        end
    end
end)

RegisterNetEvent('aj-phone:server:employment_DeleteGroup', function(data)
    EmploymentGroup[data.delete] = nil
    TriggerClientEvent('aj-phone:client:EveryoneGrupAddsForAll', -1, EmploymentGroup)
end)

AJFW.Functions.CreateCallback('aj-phone:server:GetGroupsApp', function(source, cb)
    cb(EmploymentGroup)
end)

RegisterNetEvent('aj-phone:server:employment_JoinTheGroup', function(data)
    local src = source
    for k, v in pairs(EmploymentGroup) do
        for ke, ve in pairs(v['UserName']) do
            if ve == data.PCSN then
                TriggerClientEvent('AJFW:Notify', src, "You have already joined a group!", "error")
                return
            end
        end
...
AJFW.Functions.CreateCallback('aj-phone:server:employment_CheckPlayerNames', function(source, cb, csn)
    local Names = {}
    for k, v in pairs(EmploymentGroup[csn]['UserName']) do
        local OtherPlayer = AJFW.Functions.GetPlayerByCitizenId(v)
        if OtherPlayer then
...
AJFW.Functions.CreateCallback('aj-phone:server:GetGroupCSNs', function(source, cb, csn)
...
RegisterNetEvent('aj-phone:server:employment_leave_grouped', function(data)
    local src = source
...
RegisterNetEvent('aj-phone:server:SendBillForPlayer_debt', function(data)
    local src = source
    local biller = AJFW.Functions.GetPlayer(src)
    local billed = AJFW.Functions.GetPlayer(tonumber(data.ID))
    local amount = tonumber(data.Amount)
    if billed then
            if amount and amount > 0 then
                ...
                TriggerClientEvent('AJFW:Notify', src, 'Debt successfully sent!', 'success')
                TriggerClientEvent('AJFW:Notify', billed.PlayerData.source, 'New Invoice Received')
                Wait(1)
                TriggerClientEvent('aj-phone:RefreshPhoneForDebt', billed.PlayerData.source)
            else
                TriggerClientEvent('AJFW:Notify', src, 'Must be a valid amount above 0', 'error')
            end
    else
        TriggerClientEvent('AJFW:Notify', src, 'Player not Online', 'error')
    end
end)

AJFW.Functions.CreateCallback('aj-phone:server:GetHasBills_debt', function(source, cb)
    local src = source
    local Ply = AJFW.Functions.GetPlayer(src)
...
RegisterNetEvent('aj-phone:server:debit_AcceptBillForPay', function(data)
    local src = source
    local Ply = AJFW.Functions.GetPlayer(src)
    local OtherPly = AJFW.Functions.GetPlayerByCitizenId(data.CSN)
    ...
    if OtherPly then
        if Ply.PlayerData.money.bank then
            ...
            Wait(1)
            TriggerClientEvent('aj-phone:RefreshPhoneForDebt', OtherPly.PlayerData.source)
        else
            TriggerClientEvent('AJFW:Notify', src, 'You don\'t have enough money...', 'error')
        end
    else
        TriggerClientEvent('AJFW:Notify', src, 'Player not Online', 'error')
    end
...
RegisterNetEvent('aj-phone:server:documents_Save_Note_As', function(data)
    local src = source
    local Ply = AJFW.Functions.GetPlayer(src)
    if data.Type == "New" then
        ...
        TriggerClientEvent('AJFW:Notify', src, 'Note Saved')
    elseif data.Type == "Update" then
        ...
        if Note[1] ~= nil then
            ...
            MySQL.insert('INSERT INTO phone_note (citizenid, title,  text, lastupdate) VALUES (?, ?, ?, ?)',{Ply.PlayerData.citizenid, data.Title, data.Text, data.Time})
            TriggerClientEvent('AJFW:Notify', src, 'Note Updated', 'success')
        end
    elseif data.Type == "Delete" then
        local ID = tonumber(data.ID)
        MySQL.query('DELETE FROM phone_note WHERE id = ?', {ID})
        TriggerClientEvent('AJFW:Notify', src, 'Note Deleted', 'error')
    end
    Wait(100)
    TriggerClientEvent('aj-phone:RefReshNotes_Free_Documents', src)
end)

AJFW.Functions.CreateCallback('aj-phone:server:GetNote_for_Documents_app', function(source, cb)
    local src = source
    local Ply = AJFW.Functions.GetPlayer(src)
