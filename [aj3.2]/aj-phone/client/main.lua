...
RegisterNetEvent('9d:PassedPassword', function()
    VPNopen = true
    AJFW.Functions.Notify('VPN: Login', 'success', 5000)
end)

RegisterNetEvent('9d:FailedPassword', function(wrong)
    if wrong then
        attempts = 1
        AJFW.Functions.Notify('VPN: Wrong Password', 'error', 5000)
    else
        attempts = 1
    end
end)
...
RegisterNUICallback('toggelVPN', function()
    if not VPNopen then
        ...
        VPNopen = false
        attempts = 1
        AJFW.Functions.Notify('VPN: Logout', 'error', 5000)
    end
end)
...
RegisterNUICallback('vpnerror', function()
    AJFW.Functions.Notify('VPN is off', 'error', 3000)
end)
...
local function PublicPhone()
    local PublicPhoneobject = {
        ...
        exports["aj-eye"]:AddTargetModel(PublicPhoneobject, {
            options = {
                {
                    type = "client",
...
RegisterNUICallback('jacob_duty', function()
    ...
    AJFW.Functions.Notify('No Access', 'error', 3000)
end)
...
RegisterNUICallback('DeclineInvoice', function(data, cb)
    ...
    AJFW.Functions.TriggerCallback('aj-phone:server:DeclineInvoice', function(CanPay, Invoices)
...
RegisterNUICallback('PostNewAnon', function(data, cb)
    -- local hasVPN = AJFW.Functions.HasItem(Config.VPN)
    if VPNopen then
        local AnonMessage = {
            firstName = PhoneData.PlayerData.charinfo.firstname,
            lastName = PhoneData.PlayerData.charinfo.lastname,
        ...
        for i = 2, #Hashtag, 1 do
            ...
            if Handle ~= nil or Handle ~= "" then
                ...
                TriggerServerEvent('aj-phone:server:UpdateHashtags-dark', Handle, AnonMessage)
            end
        end
        ...
        for i = 2, #MentionTag, 1 do
            ...
            if Handle ~= nil or Handle ~= "" then
                ...
                if (Firstname ~= nil and Firstname ~= "") and (Lastname ~= nil and Lastname ~= "") then
                    if Firstname ~= PhoneData.PlayerData.charinfo.firstname and Lastname ~= PhoneData.PlayerData.charinfo.lastname then
                        TriggerServerEvent('aj-phone:server:MentionedPlayer-dark', Firstname, Lastname, AnonMessage)
                    end
                end
            end
        end
        ...
        TriggerServerEvent('aj-phone:server:UpdateAnons', PhoneData.Anons, AnonMessage)
    else
        cb(PhoneData.Anons)
        AJFW.Functions.Notify('Turn on vpn First', 'error', 5000)
    end
end)
...
RegisterNUICallback('DeleteAnon',function(data)
    TriggerServerEvent('aj-phone:server:DeleteAnon', data.id)
end)
...
-- RegisterNUICallback('checkforvpn', function(data, cb)
--     -- cb(AJFW.Functions.HasItem(Config.VPN))
--     cb(true)
-- end)

-- RegisterNUICallback('vpnerror', function(data, cb)
--     AJFW.Functions.Notify('You Don\'t have vpn', 'error', 3000)
-- end)
...
RegisterNUICallback('SetAlertWaypoint', function(data)
    ...
    AJFW.Functions.Notify('GPS set: '..data.alert.title)
...
RegisterNUICallback('ClearMentions-dark', function()
    ...
    TriggerServerEvent('aj-phone:server:SetPhoneAlerts', "darkweb", 0)
    SendNUIMessage({ action = "RefreshAppAlerts", AppData = Config.PhoneApplications })
end)
...
RegisterNUICallback('CallContact', function(data, cb)
    AJFW.Functions.TriggerCallback('aj-phone:server:GetCallState', function(CanCall, IsOnline, contactData)
        local status = {
            CanCall = CanCall,
            IsOnline = IsOnline,
        ...
        elseif IsControlJustPressed(1, 176) then
            AJFW.Functions.TriggerCallback("aj-phone:server:GetWebhook",function(hook)
                ...
                exports['screenshot-basic']:requestScreenshotUpload(tostring(hook), "files[]", function(data)
                    ...
                    AJFW.Functions.Notify('Photo ssaved!', 'success')
                    OpenPhone()
                end)
            end)
...
RegisterNetEvent('aj-phone:client:UpdateAnons', function(src, Anons, NewAnonData, delete)
...
RegisterNetEvent('aj-phone:client:sendNewMail',function(data)TriggerServerEvent('aj-phone:server:sendNewMail',data)end)
...
RegisterNetEvent('aj-phone:client:AddTransaction', function(SenderData, TransactionData, Message, Title)
    local Data = {
...
RegisterNetEvent('aj-phone:client:UpdateHashtags-dark', function(Handle, msgData)
...
RegisterNetEvent('aj-phone:client:GetMentioned-dark', function(AnonMessage, AppAlerts)
...
RegisterNUICallback('AcceptPingPlayer', function()
    TriggerServerEvent('aj-pings:server:acceptping')
    TriggerEvent("aj-phone:ping:client:UiUppers", false)
end)

RegisterNUICallback('rejectPingPlayer', function()
    TriggerServerEvent('aj-pings:server:denyping')
    TriggerEvent("aj-phone:ping:client:UiUppers", false)
end)

RegisterNUICallback('SendPingPlayer', function(data)
    TriggerServerEvent('aj-pings:server:SendPing2', data.id)
    
end)

local CurrentPings = {}

RegisterNetEvent('aj-pings:client:DoPing', function(id)
    ...
    --     z = pos.z,
    -- }
    --     TriggerServerEvent('aj-pings:server:SendPing', id, coords)
    TriggerServerEvent('aj-pings:server:SendPing', id)
end)

RegisterNetEvent('aj-pings:client:AcceptPing', function(PingData, SenderData)
    local ped = PlayerPedId()
    local pos = GetEntityCoords(ped)

        TriggerServerEvent('aj-pings:server:SendLocation', PingData, SenderData)
end)

RegisterNetEvent('aj-pings:client:SendLocation', function(PingData, SenderData)
    AJFW.Functions.Notify('Their location has been blipped on your map', 'success')
    ...
    SetTimeout(5 * (60 * 1000), function()
        AJFW.Functions.Notify('Ping '..PingData.sender..' has expired...', 'error')
...
RegisterNetEvent('aj-phone:ping:client:UiUppers', function(toggle)
    if toggle then
        SendNUIMessage({
            action = "acceptrejectBlock",
        })
        TriggerEvent("aj-hud:ping:client:ShowIcon", true)
    else
        SendNUIMessage({
            action = "acceptrejectNone",
        })
        TriggerEvent("aj-hud:ping:client:ShowIcon", false)
    end
end)


RegisterNetEvent('aj-phone:client-annphonenumber', function(number, copy)
    TriggerEvent('chat:addMessage', {
        template = '<div class="chat-message" style="background-color: rgba(234, 135, 23, 0.50);">Number : <b>{0}</b></div>',
        args = {" "..number}
    })
    if copy then
        exports['aj-adminmenu']:CopyToClipboard2(tostring(number))
    end
end)
...
RegisterNUICallback('CasinoPhoneJobCenter', function(data)
    if data.action == 1 then
        TriggerServerEvent('aj-phone:server:SetJobJobCenter', data)
...
RegisterNUICallback('employment_CreateJobGroup', function(data)
    TriggerServerEvent('aj-phone:server:employment_CreateJobGroup', data)
end)

RegisterNetEvent('aj-phone:client:EveryoneGrupAddsForAll', function(data)
    SendNUIMessage({
        action = "GroupAddDIV",
        datas = data,
    })
...
RegisterNUICallback('employment_DeleteGroup', function(data)
    TriggerServerEvent('aj-phone:server:employment_DeleteGroup', data)
end)

RegisterNUICallback('GetGroupsApp', function(data, cb)
    AJFW.Functions.TriggerCallback('aj-phone:server:GetGroupsApp', function(HasGroups)
        cb(HasGroups)
    end)
end)

RegisterNUICallback('employment_JoinTheGroup', function(data)
    TriggerServerEvent('aj-phone:server:employment_JoinTheGroup', data)
end)

RegisterNUICallback('employment_leave_grouped', function(data)
    TriggerServerEvent('aj-phone:server:employment_leave_grouped', data)
end)

RegisterNUICallback('employment_CheckPlayerNames', function(data, cb)
    AJFW.Functions.TriggerCallback('aj-phone:server:employment_CheckPlayerNames', function(HasName)
        cb(HasName)
    end, data.id)
end)
...
RegisterNUICallback('SendBillForPlayer_debt', function(data)
    for k,v in pairs(Config.dept) do
        if PhoneData.PlayerData.job.name == v then
            TriggerServerEvent('aj-phone:server:SendBillForPlayer_debt', data)
            return
        end
    end
    AJFW.Functions.Notify('You Don\'t have enough perms', 'error', 3000)
end)

RegisterNUICallback('GetHasBills_debt', function(data, cb)
    AJFW.Functions.TriggerCallback('aj-phone:server:GetHasBills_debt', function(Has)
        cb(Has)
    end)
end)

RegisterNUICallback('debit_AcceptBillForPay', function(data)
    TriggerServerEvent('aj-phone:server:debit_AcceptBillForPay', data)
end)

RegisterNetEvent('aj-phone:RefreshPhoneForDebt', function()
    SendNUIMessage({
        action = "DebtRefresh",
    })
end)
...
RegisterNUICallback('GetNote_for_Documents_app', function(data, cb)
    AJFW.Functions.TriggerCallback('aj-phone:server:GetNote_for_Documents_app', function(Has)
        cb(Has)
    end)
end)

RegisterNetEvent('aj-phone:RefReshNotes_Free_Documents', function()
    SendNUIMessage({
...
RegisterNUICallback('openHelp', function()  
    TriggerEvent('aj-help')
end)

local function GetGroupCSNs(Data)
    local gData = {}
    AJFW.Functions.TriggerCallback('aj-phone:server:GetGroupCSNs', function(HasGroup)
        gData = HasGroup
    end, Data)
    Wait(100)
    return gData
