...
RegisterNetEvent('mdt:server:setRadio', function(cid, newRadio)
	...
	if Player.PlayerData.citizenid ~= cid then
		TriggerClientEvent("AJFW:Notify", src, 'You can only change your radio!', 'error')
		return
	else
		...
		if radio ~= nil then
			...
			TriggerClientEvent("AJFW:Notify", src, 'You do not have a radio!', 'error')
		end
	end
