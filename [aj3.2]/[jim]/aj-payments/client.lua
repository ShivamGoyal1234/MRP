...
RegisterNetEvent('AJFW:Client:OnGangUpdate')
AddEventHandler('AJFW:Client:OnGangUpdate', function(GangInfo)
    PlayerGang = GangInfo
end)

--Keeps track of duty on script restarts
AddEventHandler('onResourceStart', function(resource) 
if GetCurrentResourceName() == resource then AJFW.Functions.GetPlayerData(function(PlayerData) PlayerJob = PlayerData.job PlayerGang = PlayerData.gang onDuty = PlayerJob.onduty end) end end)

Citizen.CreateThread(function()
	local jobroles = {}
	for k, v in pairs(Config.Jobs) do jobroles[tostring(k)] = 0 end
	exports['aj-eye']:AddBoxZone("JimBank", vector3(251.75, 222.17, 106.2), 0.6, 2.0, { name="JimBank", heading = 340.0, debugPoly=false, minZ = 105.75, maxZ = 107.29, }, 
...
RegisterNetEvent('jim-payments:client:Charge', function(data)
	if not onDuty then TriggerEvent("AJFW:Notify", "Not Clocked in!", "error") return end  -- Require to be on duty when making a payment
	local onlineList = {}
	local nearbyList = {}
	AJFW.Functions.TriggerCallback('jim-payments:MakePlayerList', function(cb) onlineList = cb if onlineList[1] == nil then Wait(200) end
		...
		local dialog = exports['aj-input']:ShowInput({ header = img..PlayerJob.label.." Cash Register", submitText = "Send",
		inputs = {
...
RegisterNetEvent('jim-payments:Tickets:Menu', function()
	local amount = nil
	while amount == nil do AJFW.Functions.TriggerCallback('jim-payments:Ticket:Count', function(cb) amount = cb end) Citizen.Wait(1) end
	for k, v in pairs(Config.Jobs) do if k ~= PlayerJob.name then 
		else exports['aj-menu']:openMenu({
...
RegisterNetEvent('jim-payments:Tickets:Sell:no', function() exports['aj-menu']:closeMenu() end)
