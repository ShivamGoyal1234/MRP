...
-- This requires aj-eye and has the location of every ATM, ATM prop and bank window
-- If Config.useATM = false, then none of this load

--Specific to ATM config options
Config.useATM = false
Config.useBanks = false
Config.BankBlips = false
...
RegisterNetEvent('AJFW:Client:OnPlayerLoaded', function() createBlips() end)

Citizen.CreateThread(function()
	if Config.useATM or Config.useBanks then
		local bossroles = {}
		for k, v in pairs(AJFW.Shared.Jobs) do 
			for l, b in pairs(AJFW.Shared.Jobs[tostring(k)].grades) do
				if AJFW.Shared.Jobs[tostring(k)].grades[l].isboss == true then
		...
		local gangroles = {}
		for k, v in pairs(AJFW.Shared.Gangs) do 
			for l, b in pairs(AJFW.Shared.Gangs[tostring(k)].grades) do
	...
	if Config.useATM then
		exports['aj-eye']:AddTargetModel(Config.ATMModels, { options = { { event = "jim-payments:Client:ATM:use", icon = "fas fa-money-check-alt", label = "Use ATM", id = "atm" },}, distance = 1.5, })
		for k,v in pairs(Config.ATMLocations) do
			exports['aj-eye']:AddCircleZone("jimatm"..k, vector3(tonumber(v.x), tonumber(v.y), tonumber(v.z)+0.2), 0.5, { name="jimatm"..k, debugPoly=false, useZ=true, }, 
	...
	if Config.useBanks then
		for k,v in pairs(Config.BankLocations) do
			exports['aj-eye']:AddCircleZone("jimbank"..k, vector3(tonumber(v.x), tonumber(v.y), tonumber(v.z)), 2.0, { name="jimbank"..k, debugPoly=false, useZ=true, }, 
...
RegisterNetEvent('jim-payments:Client:ATM:use', function(data)
	...
	while name == nil do 
	AJFW.Functions.TriggerCallback('jim-payments:ATM:Find', function(cb1, cb2, cb3, cb4, cb5, cb6, cb7) name = cb1 cash = cb2 bank = cb3 account = cb4 cid = cb5 savbal = cb6 aid = cb7 end) 
		Citizen.Wait(100) 
	end
	while society == nil do 
	AJFW.Functions.TriggerCallback('aj-bossmenu:server:GetAccount', function(cb) society = cb end, PlayerJob.name)
		Citizen.Wait(100) 
	end	
	while gsociety == nil do 
	AJFW.Functions.TriggerCallback('aj-gangmenu:server:GetAccount', function(cb) gsociety = cb end, PlayerGang.name)
		Citizen.Wait(100) 
	end
	...
	if data.id == "atm" then
		...
		for k, v in pairs(Config.ATMModels) do
			...
			if atm then
				...
				AJFW.Functions.Progressbar("accessing_atm", "Accessing ATM", atmbartime, false, true, { disableMovement = false, disableCarMovement = false, disableMouse = false, disableCombat = false, }, {}, {}, {}, function() -- Done
				end, function()
	...
	elseif data.id == "bank" then
		...
		setinputs = { { type = 'radio', name = 'billtype', text = setview, options = setoptions },
				      { type = 'number', isRequired = true, name = 'amount', text = '💵 Amount to transfer' }, }
		PlayATMAnimation('enter')
		AJFW.Functions.Progressbar("accessing_atm", "Accessing Bank", atmbartime, false, true, { disableMovement = false, disableCarMovement = false, disableMouse = false, disableCombat = false, }, {}, {}, {}, function() -- Done
		end, function()
			TriggerEvent("AJFW:Notify", "Cancelled!", "error")
			return
		end)	
	...
	elseif data.id == "transfer" then
		...
		setinputs = { { type = 'radio', name = 'billtype', text = setview, options = setoptions },
					  ...
					  { type = 'number', isRequired = true, name = 'amount', text = '💸 Amount to transfer' }, }
		PlayATMAnimation('enter')
		AJFW.Functions.Progressbar("accessing_atm", "Accessing Transfers", atmbartime, false, true, { disableMovement = false, disableCarMovement = false, disableMouse = false, disableCombat = false, }, {}, {}, {}, function() -- Done
		end, function()
			TriggerEvent("AJFW:Notify", "Cancelled!", "error")
			return
		end)
	...
	elseif data.id == "savings" then
		...
		setinputs = { { type = 'radio', name = 'billtype', text = setview, options = setoptions },
					  { type = 'number', isRequired = true, name = 'amount', text = '💵 Amount to transfer' }, }					
		PlayATMAnimation('enter')
		AJFW.Functions.Progressbar("accessing_atm", "Accessing Savings", atmbartime, false, true, { disableMovement = false, disableCarMovement = false, disableMouse = false, disableCombat = false, }, {}, {}, {}, function() -- Done
		end, function()
			TriggerEvent("AJFW:Notify", "Cancelled!", "error")
			return
		end)
	...
	elseif data.id == "society" then
		...
		setinputs = { { type = 'radio', name = 'billtype', text = setview, options = setoptions },
				      { type = 'number', isRequired = true, name = 'amount', text = '💵 Amount to transfer' }, }
		PlayATMAnimation('enter')
		AJFW.Functions.Progressbar("accessing_atm", "Accessing Society Account", atmbartime, false, true, { disableMovement = false, disableCarMovement = false, disableMouse = false, disableCombat = false, }, {}, {}, {}, function() -- Done
		end, function()
			TriggerEvent("AJFW:Notify", "Cancelled!", "error")
			return
		end)
	...
	Wait(atmbartime+500)
	local dialog = exports['aj-input']:ShowInput({ header = setheader, txt = "test", submitText = "Transfer", inputs = setinputs })
	if dialog then
...
RegisterNetEvent('jim-payments:client:ATM:give', function()
	local onlineList = {}
	local nearbyList = {}
	AJFW.Functions.TriggerCallback('jim-payments:MakePlayerList', function(cb) onlineList = cb if onlineList[1] == nil then Wait(200) end
		for k, v in pairs(AJFW.Functions.GetPlayersFromCoords(GetEntityCoords(PlayerPedId()), Config.PaymentRadius)) do
		...
		if nearbyList[#nearbyList] == nil then TriggerEvent("AJFW:Notify", "No one near by to charge", "error") return end
		local dialog = exports['aj-input']:ShowInput({ header = "Give someone cash", submitText = "Give",
		inputs = {
