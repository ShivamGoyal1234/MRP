...
AddEventHandler("jim-recycle:getrecyclablematerial", function()
    ...
    TriggerClientEvent('inventory:client:ItemBox', src, AJFW.Shared.Items["recyclablematerial"], 'add', amount)
    Citizen.Wait(500)
...
AddEventHandler("jim-recycle:TradeItems", function(data)
	...
	if data == 1 then
		if Player.Functions.GetItemByName('recyclablematerial') ~= nil and Player.Functions.GetItemByName('recyclablematerial').amount >= 1 then
			...
			TriggerClientEvent('inventory:client:ItemBox', src, AJFW.Shared.Items["recyclablematerial"], 'remove', 1)
			Citizen.Wait(1000)
			...
			TriggerClientEvent('inventory:client:ItemBox', src, AJFW.Shared.Items[randItem], 'add', amount)
			else
	...
	elseif data == 2 then
		if Player.Functions.GetItemByName('recyclablematerial') ~= nil and Player.Functions.GetItemByName('recyclablematerial').amount >= 10 then
			...
			TriggerClientEvent('inventory:client:ItemBox', src, AJFW.Shared.Items["recyclablematerial"], 'remove', 10)
			Citizen.Wait(1000)
			randItem = ItemTable[math.random(1, #ItemTable)]
	...
	elseif data == 3 then
		if Player.Functions.GetItemByName('recyclablematerial') ~= nil and Player.Functions.GetItemByName('recyclablematerial').amount >= 100 then
			...
			TriggerClientEvent('inventory:client:ItemBox', src, AJFW.Shared.Items["recyclablematerial"], 'remove', 100)
			Citizen.Wait(1000)
    		...
    		if chance < 50 then
				...
				TriggerClientEvent('inventory:client:ItemBox', src, AJFW.Shared.Items["wire"], "add", 6)
			end
		else
...
AddEventHandler("jim-recycle:Selling:All", function()
	...
	for k, v in pairs(Config.Prices) do
        if Player.Functions.GetItemByName(v.name) ~= nil then
            ...
            TriggerClientEvent('inventory:client:ItemBox', src, AJFW.Shared.Items[v.name], 'remove', copper)
        payment = payment + pay
        end
    end
    Citizen.Wait(500)
	TriggerClientEvent("AJFW:Notify", src, "Total: $"..payment, 'success')
end)
...
AddEventHandler("jim-recycle:Selling:Mat", function(data)
    ...
    if Player.Functions.GetItemByName(data) ~= nil then
        ...
        TriggerClientEvent("AJFW:Notify", src, "Payment received", "Total: $"..pay, "error")
    else
...
AddEventHandler('jim-recycle:Dumpsters:Reward', function(listKey)
    ...
    if chance < 50 then
		...
		TriggerClientEvent("AJFW:Notify", src, "No item found", "error")
	end
end)
