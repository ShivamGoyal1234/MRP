--New AJFW way of getting the Object comment out if your using old QB
...
RegisterNetEvent("cc-fuel:client:siphonfuel",function() 
    ...
    if distanceToVehicle > 2.5 then
        AJFW.Functions.Notify("You are too far away from the vehicle","error")
        return
    end

    --Check petrol can is able to take fuel
    if petrolCanDurability == nil then
        AJFW.Functions.Notify("You need a petrol can in your hands","error")
        return
    elseif petrolCanDurability == 100 then
        AJFW.Functions.Notify("You petrol can is full","error")
        return
    end
    ...
    if currentFuel > 0 then
        ...
        AJFW.Functions.Notify("You siphoned fuel","success")
    else
        AJFW.Functions.Notify("The tank is empty","error")
    end
...
RegisterNetEvent("cc-fuel:client:refillpetrolcan", function()
    ...
    if petrolCanDurability ~= nil then
        if CurrentWeaponData.info.ammo == 100 then
            AJFW.Functions.Notify("Your can is full","error")
        else
            local refillCost = 4000 - CurrentWeaponData.info.ammo
            -- print(refillCost)
            if refillCost > 0 then
                local currentCash = AJFW.Functions.GetPlayerData().money['cash']  
                local lMera = refillCost / 40
			    if currentCash >= lMera then
					TriggerServerEvent('cc-fuel:server:pay', lMera, GetPlayerServerId(PlayerId()))
                    -- local infos = {}
                    -- infos.uses = 100
                    -- TriggerServerEvent("AJFW:Server:AddItem", CurrentWeaponData.name, CurrentWeaponData.amount, CurrentWeaponData.slot, infos)
                    -- TriggerServerEvent('Perform:Decay:item2', CurrentWeaponData.name, CurrentWeaponData.slot, 100)
                    -- Wait(1000)
					SetPetrolCanDurability2(4000)
				    AJFW.Functions.Notify("You refilled your petrol can","success")
                else
                    AJFW.Functions.Notify("Not enough cash to refill the can","error")
                end
            end
        end
    else
        AJFW.Functions.Notify("You don't have a petrol can to refill","error")
    end
end)
...
RegisterNetEvent("cc-fuel:client:buypetrolcan", function()
    ...
    if currentCash >= Config.JerryCanCost then
        ...
        infos.ammo = 4000
        -- infos.uses = 100
		TriggerServerEvent('AJFW:Server:AddItem', "weapon_petrolcan", 1, nil,infos)
		...
		AJFW.Functions.Notify("You bought a jerry can","success")
	else
		AJFW.Functions.Notify("You don't have enough money to buy a jerry can","error")
	end
end)
...
RegisterNetEvent("cc-fuel:client:pumprefuel", function(pump) 
    ...
    if CanFuelVehicle(Vehicle) then
        ...
        AJFW.Functions.Notify("You paid $" .. currentCost .. " for fuel","success")
    else
        AJFW.Functions.Notify("The tank is full","error")
    end
...
RegisterNetEvent("cc-fuel:client:petrolcanrefuel", function() 
    ...
    elseif CurrentWeaponData.info.ammo <= 0 then
        AJFW.Functions.Notify("You petrol can is empty","error")
        return
    end
    ...
    if CanFuelVehicle(Vehicle) then
        ...
        ClearPedTasks(PlayerPed)
        AJFW.Functions.Notify("You refueled your car","success")
    else
        AJFW.Functions.Notify("The tank is full","error")
    end
