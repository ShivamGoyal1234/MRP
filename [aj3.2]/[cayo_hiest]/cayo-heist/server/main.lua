...
RegisterNetEvent('nxte-cayo:server:opendoor', function(doorid)
    local src = source
    TriggerClientEvent('nui_doorlock:client:setState', -1, src, doorid, false, false, false, false)
end)

-- open a door
RegisterNetEvent('nxte-cayo:server:closedoor', function(doorid)
    local src = source
    TriggerClientEvent('nui_doorlock:client:setState', -1, src, doorid, true, false, false, false)
end)
...
AJFW.Functions.CreateCallback('nxte-cayo:server:SpawnNPC', function(source, cb, loc)
    local netIds = {}
    local netId
    local npc
