...
local function OnHack1Done(success)
    if success then 
        AJFW.Functions.Notify('Check your GPS for the next location!')
        AJFW.Functions.Notify('You successfully hacked the alarm system!','success')
        TriggerServerEvent('nxte-cayo:server:sethack1', true)
        TriggerServerEvent('AJFW:Server:RemoveItem', Config.HackI1tem, 1, slot, info)
        TriggerEvent('inventory:client:ItemBox', AJFW.Shared.Items[Config.Hack1Item], 'remove')
        RemoveBlip(hackBlip)
        HackBlip(vector3(5084.7, -5731.45, 15.77))
    else
        AJFW.Functions.Notify('You failed to hack the alarm system!', 'error')
        TriggerServerEvent('AJFW:Server:RemoveItem', Config.Hack1Item, 1, slot, info)
        TriggerEvent('inventory:client:ItemBox', AJFW.Shared.Items[Config.Hack1Item], 'remove')
    end
end
...
local function OnHack3Done(success)
    if success then 
        ...
        AJFW.Functions.Notify('Go to the basement to grab the items!')
        AJFW.Functions.Notify('You successfully hacked the camera system!', 'success')
        TriggerServerEvent('AJFW:Server:RemoveItem', Config.Hack3Item, 1, slot, info)
        TriggerEvent('inventory:client:ItemBox', AJFW.Shared.Items[Config.Hack3Item], 'remove')
        RemoveBlip(hackBlip)
        if Config.SpawnPedOnHack3 then
            TriggerEvent('nxte-cayo:client:SpawnNPC', 2)
        end
    else
        AJFW.Functions.Notify('You failed to hack the camera system!', 'error')
        TriggerServerEvent('AJFW:Server:RemoveItem', Config.Hack3Item, 1, slot, info)
        TriggerEvent('inventory:client:ItemBox', AJFW.Shared.Items[Config.Hack3Item], 'remove')
    end
end
...
local PlantBomb1 = function()
    ...
    CreateThread(function()
        AJFW.Functions.Notify('The bomb will go off in '..Config.BombTimer..' seconds', 'success')
        ...
        AJFW.Functions.Notify('The door opened!', 'success')
    end)
end
...
local PlantBomb4 = function()
    ...
    CreateThread(function()
        ...
        AJFW.Functions.Notify('The door opened!', 'success')
        TriggerEvent('inventory:client:ItemBox', AJFW.Shared.Items[Config.BombItem], 'remove')
    end)
end
...
RegisterNetEvent('nxte-cayo:client:startheist', function()
    ...
    AJFW.Functions.Progressbar("knock", "Knocking on door...", 4000, false, false, {
        ...
        if not isActive then 
            if CopCount >= Config.MinCop then
                if cash >= Config.JobPrice then
                    ...
                    AJFW.Functions.Notify('You purchased the GPS location for $'..Config.JobPrice..'!', 'success')
                    TriggerServerEvent('nxte-cayo:server:removemoney', Config.JobPrice)
                    HackBlip(vector3(4491.46, -4583.42, 5.57))
                else 
                    AJFW.Functions.Notify('I only talk to people who bring the money i ask for', 'error')
                end
            else 
                AJFW.Functions.Notify('There is not enough police', 'error')
            end   
        else 
            AJFW.Functions.Notify('No one seems to be home', 'error')
        end
    end, function() -- Cancel
        TriggerEvent('animations:client:EmoteCommandStart', {"c"})
        AJFW.Functions.Notify('Cancelled knocking on the door', 'error')
    end)
end)
...
RegisterNetEvent('nxte-cayo:client:hack1', function()
    ...
    AJFW.Functions.Progressbar("knock", "Preparing Equipement...", 4000, false, false, {
        disableMovement = true,
        disableCarMovement = true,
        disableMouse = false,
        disableCombat = true,
        ...
        TriggerEvent('animations:client:EmoteCommandStart', {"c"})
        if isActive then 
            if AJFW.Functions.HasItem(Config.Hack1Item) then
                if not isHacked1 then
                    TriggerEvent('nxte-anim:hack1')
                else
                    AJFW.Functions.Notify('The alarm system has already been hacked !', 'error')
                end
            else
                AJFW.Functions.Notify('You do not have the right equipement to do this !', 'error')
            end
        else 
            AJFW.Functions.Notify('You can not do this right now', 'error')
        end
    end, function() -- Cancel
        TriggerEvent('animations:client:EmoteCommandStart', {"c"})
        AJFW.Functions.Notify('Cancelled preparing equipement', 'error')
    end)
end)
    ...
    if isActive then
        if isHacked1 then 
            if not isHacked2 then 
                if AJFW.Functions.HasItem(Config.Hack2Item) then
                    TriggerEvent('nxte-anim:hack2')
                else 
                    AJFW.Functions.Notify('You do not have the right tools to do this!', 'error')
                end
            else
                AJFW.Functions.Notify('The door lock has already been hacked !', 'error')
            end
        else
            AJFW.Functions.Notify('The alarm system is still active !', 'error')
        end
    else 
        AJFW.Functions.Notify('You can not do this right now', 'error')
    end
...
RegisterNetEvent('nxte-anim:hack2', function()
    ...
    function() -- success
        ...
        AJFW.Functions.Notify('You hacked the door lock!', 'success')
        TriggerServerEvent('AJFW:Server:RemoveItem', Config.Hack2Item, 1, slot, info)
        TriggerEvent('inventory:client:ItemBox', AJFW.Shared.Items[Config.Hack2Item], 'remove')
    end,
    function() -- failure
        AJFW.Functions.Notify('You failed to hack the door lock!', 'error')
        NetworkStartSynchronisedScene(netScene3)
        NetworkStopSynchronisedScene(netScene3)
        DeleteObject(bag)
        DeleteObject(laptop)
    ...
    if isActive then
        if isHacked2 then 
            if not isHacked3 then
                if AJFW.Functions.HasItem(Config.Hack3Item) then
                    TriggerEvent('nxte-anim:hack3')
                else 
                    AJFW.Functions.Notify('You do not have the right tools to do this!', 'error')
                end
            else
                AJFW.Functions.Notify('The camera system has already been hacked !', 'error')
            end
        else
            AJFW.Functions.Notify('The lock system is still active !', 'error')
        end
    else 
        AJFW.Functions.Notify('You can not do this right now', 'error')
    end
...
RegisterNetEvent('nxte-cayo:client:door1', function()
    if isActive then 
        if isHacked3 then 
            if not door1 then
                if AJFW.Functions.HasItem(Config.BombItem) then
                    ...
                    function() -- success
                        ...
                        PlantBomb1()
                        TriggerServerEvent('AJFW:Server:RemoveItem', Config.BombItem, 1, slot, info)
                        TriggerEvent('inventory:client:ItemBox', AJFW.Shared.Items[Config.BombItem], 'remove')
                    end,
                    function() -- failure
                        AJFW.Functions.Notify('You failed to open the door', 'error')
                        TriggerServerEvent('AJFW:Server:RemoveItem', Config.BombItem, 1, slot, info)
                        TriggerEvent('inventory:client:ItemBox', AJFW.Shared.Items[Config.BombItem], 'remove')
                    end)                                                                                                                                                        
                else 
                ...
                AJFW.Functions.Notify('The door lock has already been opened!', 'error')
            end
        else
            AJFW.Functions.Notify('The camera system is still active !', 'error')
        end
    else 
        AJFW.Functions.Notify('You can not do this right now', 'error')
    end
...
RegisterNetEvent('nxte-cayo:client:loot1', function()
    ...
    AJFW.Functions.Progressbar("loot", "Searching barrel...", 5000, false, false, {
        disableMovement = true,
        disableCarMovement = true,
        disableMouse = false,
        disableCombat = true,
            ...
            if isHacked3 then
                if not Loot1 then 
                    ...
                    if random >= 2 then -- normal loot 
                        ...
                        TriggerServerEvent('AJFW:Server:AddItem', Config.Loot1Item, chance, slot, info)
                        TriggerEvent('inventory:client:ItemBox', AJFW.Shared.Items[Config.Loot1Item], 'add', chance)
                        ...
                        TriggerServerEvent('AJFW:Server:AddItem', Config.Loot1RareItem, chance, slot, info)
                        TriggerEvent('inventory:client:ItemBox', AJFW.Shared.Items[Config.LootRare1Item], 'add', chance)
                        TriggerServerEvent('nxte-cayo:server:setloot1', true)
                    end
                else
                    AJFW.Functions.Notify('The barrel is already emptied' , 'error')
                end
            else
                AJFW.Functions.Notify('The camera system is still active' , 'error')
            end
        else 
            AJFW.Functions.Notify('You can\'t do this right now' , 'error')
        end
    end, function() -- Cancel
        TriggerEvent('animations:client:EmoteCommandStart', {"c"})
        AJFW.Functions.Notify('Cancelled searching the barrel', 'error')
    end)
end)

RegisterNetEvent('nxte-cayo:client:loot2', function()
    TriggerEvent('animations:client:EmoteCommandStart', {"medic"})
    AJFW.Functions.Progressbar("loot", "Searching safe...", 5000, false, false, {
        disableMovement = true,
        disableCarMovement = true,
        disableMouse = false,
        disableCombat = true,
            ...
            if isHacked3 then
                if not Loot2 then 
                    ...
                    if random >= 2 then -- normal loot 
                        ...
                        TriggerServerEvent('AJFW:Server:AddItem', Config.Loot2Item, chance, slot, info)
                        TriggerEvent('inventory:client:ItemBox', AJFW.Shared.Items[Config.Loot2Item], 'add', chance)
                        ...
                        TriggerServerEvent('AJFW:Server:AddItem', Config.Loot2RareItem, chance, slot, info)
                        TriggerEvent('inventory:client:ItemBox', AJFW.Shared.Items[Config.LootRare2Item], 'add', chance)
                        TriggerServerEvent('nxte-cayo:server:setloot2', true)
                    end
                else
                    AJFW.Functions.Notify('The safe is already emptied' , 'error')
                end
            else
                AJFW.Functions.Notify('The camera system is still active' , 'error')
            end
        ...
        TriggerEvent('animations:client:EmoteCommandStart', {"c"})
        AJFW.Functions.Notify('Cancelled searching the safe', 'error')
    end)
end)

RegisterNetEvent('nxte-cayo:client:loot3', function()
    TriggerEvent('animations:client:EmoteCommandStart', {"medic"})
    AJFW.Functions.Progressbar("loot", "Searching crate...", 5000, false, false, {
        disableMovement = true,
        disableCarMovement = true,
        disableMouse = false,
        disableCombat = true,
            ...
            if isHacked3 then
                if not Loot3 then 
                    ...
                    if random >= 2 then -- normal loot 
                        ...
                        TriggerServerEvent('AJFW:Server:AddItem', Config.Loot3Item, chance, slot, info)
                        TriggerEvent('inventory:client:ItemBox', AJFW.Shared.Items[Config.Loot3Item], 'add', chance)
                        ...
                        TriggerServerEvent('AJFW:Server:AddItem', Config.Loot3RareItem, chance, slot, info)
                        TriggerEvent('inventory:client:ItemBox', AJFW.Shared.Items[Config.Loot3RareItem], 'add', chance)
                        TriggerServerEvent('nxte-cayo:server:setloot3', true)
                    end
                else
                    AJFW.Functions.Notify('The crate is already emptied' , 'error')
                end
            else
                AJFW.Functions.Notify('The camera system is still active' , 'error')
            end
        ...
        TriggerEvent('animations:client:EmoteCommandStart', {"c"})
        AJFW.Functions.Notify('Cancelled searching the crate', 'error')
    end)
end)
-------------------------------------------- SERVER SYNC --------------------------------------
...
RegisterNetEvent('nxte-cayo:client:SpawnNPC', function(position)
    AJFW.Functions.TriggerCallback('nxte-cayo:server:SpawnNPC', function(netIds, position)
        Wait(1000)
        local ped = PlayerPedId()
        for i=1, #netIds, 1 do
        ...
        if isActive then
            ...
            if Playerid == Buyer then
                if Player.metadata["inlaststand"] or Player.metadata["isdead"] then
                    AJFW.Functions.Notify('Mission Failed', 'error')
                    TriggerEvent('nxte-cayo:client:ResetHeist')
                    Citizen.Wait(2000)
                    Buyer = nil
                end
