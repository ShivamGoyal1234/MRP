...
RegisterNetEvent('AJFW:Client:OnPlayerLoaded', function()
	AJFW.Functions.TriggerCallback('aj-tattoos:GetPlayerTattoos', function(tattooList)
		if tattooList then
...
RegisterNetEvent('aj-tattoos:update', function(data)
	Tattoosss = data
end)
...
function GetNaked()
	...
	if GetEntityModel(PlayerPedId()) == `mp_m_freemode_01` then
		...
		TriggerEvent('aj-clothing:client:loadOutfit', outfitMan)
	elseif GetEntityModel(PlayerPedId()) == `mp_f_freemode_01` then
		...
		TriggerEvent('aj-clothing:client:loadOutfit', outfitFemale)
	else
		AJFW.Functions.TriggerCallback('aj-tattoos:GetPlayerTattoos', function(tattooList)
			if tattooList then
				ClearPedDecorations(PlayerPedId())
...
function ResetSkin()
	TriggerServerEvent("aj-clothes:loadPlayerSkins")
	ClearPedDecorations(PlayerPedId())
	for k, v in pairs(currentTattoos) do
		if v.Count ~= nil then
			for i = 1, v.Count do
...
function BuyTattoo(collection, name, label, price)
	AJFW.Functions.TriggerCallback('aj-tattoos:PurchaseTattoo', function(success)
		if success then
...
function RemoveTattoo(name, label)
	...
	TriggerServerEvent("aj-tattoos:RemoveTattoo", currentTattoos)
	AJFW.Functions.Notify("You have tattoo "..GetLabelText(label).. " deleted.", "error", 2500)
end
...
RegisterNetEvent('aj-tattoo:client:tattooppen')
AddEventHandler('aj-tattoo:client:tattooppen', function()
    OpenTattooShop()
end)
