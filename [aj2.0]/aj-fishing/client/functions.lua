TryToFish = function(item)
    AJFW.Functions.TriggerCallback('aj-fishing:GetItemData', function(count)
        if IsPedSwimming(cachedData["ped"]) then return AJFW.Functions.Notify("You can't be swimming and fishing at the same time.", "error") end 
        if IsPedInAnyVehicle(cachedData["ped"]) then return AJFW.Functions.Notify("You need to exit your vehicle to start fishing.", "error") end 
        if count ~= nil then
            if count == 0 then
                AJFW.Functions.Notify("You need both a fishing rod and bait to fish.", "primary")
            else
                ...
                if waterValidated then
                    ...
                    AJFW.Functions.Notify("You need to aim towards the water to fish", "primary")
                end
            end
        end
...
CastBait = function(rodHandle, castLocation, item)
    ...
    if not HasFishingBait() then
        AJFW.Functions.Notify('You don\'t have any bait!', 'error')
    ...
    while not IsControlJustPressed(0, 47) do
        ...
        if GetGameTimer() - startedCasting > 5000 then
            AJFW.Functions.Notify("You need to cast the bait.", "primary")
    ...
    AJFW.Functions.Notify("Waiting for a fish to bite...", "success", "10000")
    TriggerServerEvent('AJFW:Server:RemoveItem', "fishingbait", 1)
	TriggerEvent("inventory:client:ItemBox", AJFW.Shared.Items["fishingbait"], "remove")
    ...
    if caughtFish then
        ...
        AJFW.Functions.Notify("The fish got loose.", "error")
    end
...
IsInWater = function()
    ...
    AJFW.Functions.Notify("Checking fishing location...", "success", "3000")
...
SellFish = function()
    AJFW.Functions.TriggerCallback('aj-fishing:GetItemData', function(count)
        ...
        TriggerServerEvent("aj-fishing:sellFish", function(sold, fishesSold) end)
    end)
end
...
function HasFishingBait()
    local rtval = false
    AJFW.Functions.TriggerCallback('AJFW:HasItem', function(amount)
        if amount then
            rtval = true
        end
    end, "fishingbait")
...
function giveFish(item)
    ...
    if kismatFound then
        for i = 1, kismatCount, 1 do
            if kismat == 100 then
                AJFW.Functions.Progressbar("makingphoto", "Catching fish..", 500, false, true, {
                }, {}, {}, {}, function()
                end)
                TriggerServerEvent("aj-fishing:receiveFish", castLocation, function(received) end)
            ...
            elseif kismat >= 80 and kismat <= 90 then
                ...
                AJFW.Functions.Notify("Opps! Your rod got broken!", "error")
                interupted = true
