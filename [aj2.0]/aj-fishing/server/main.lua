...
AJFW.Functions.CreateCallback('aj-fishing:GetItemData', function(source, cb, itemName)
	local retval = false
...
AJFW.Functions.CreateUseableItem("fishingrod", function(source, item)
    local Player = AJFW.Functions.GetPlayer(source)
	if item.info.uses >= 1 then
    	TriggerClientEvent('aj-fishing:tryToFish', source, item)
	else
		TriggerClientEvent("AJFW:Notify", source, "Broken ROD", "error", 3000)
	end
end)

-- -RegisterServerEvent('aj-fishing:receiveFish')
-- AddEventHandler('aj-fishing:receiveFish', function(cabin, house)
--     local src = source
...
RegisterServerEvent("aj-fishing:sellFish")
AddEventHandler("aj-fishing:sellFish", function()
    local src = source
	local Player = AJFW.Functions.GetPlayer(src)
	local price = 0
    if Player.PlayerData.items ~= nil and next(Player.PlayerData.items) ~= nil then 
		...
		TriggerClientEvent('AJFW:Notify', src, "You have sold your fish")
	end
end)