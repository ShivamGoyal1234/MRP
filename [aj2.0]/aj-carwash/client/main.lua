...
RegisterNetEvent('aj-carwash:client:washCar')
AddEventHandler('aj-carwash:client:washCar', function()
    local PlayerPed = PlayerPedId()
    local PedVehicle = GetVehiclePedIsIn(PlayerPed)
    local Driver = GetPedInVehicleSeat(PedVehicle, -1)
    ...
    AJFW.Functions.Progressbar("search_cabin", "Vehicle is being washed..", math.random(4000, 8000), false, true, {
        ...
        AJFW.Functions.Notify("Washing canceled..", "error")
        washingVehicle = false
    end)
end)
