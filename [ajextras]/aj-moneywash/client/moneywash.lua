...
Citizen.CreateThread(function()
    ...
    AJFW.Functions.TriggerCallback('jacob:get:wash', function(t, p, s)
		tpLocations       = t
        password          = p
        target            = s
	end)
...
Citizen.CreateThread(function()
	...
	while true do 
        ...
        for location, val in pairs(tpLocations) do
            ...
            --                 TriggerServerEvent("InteractSound_SV:PlayOnSource", "houses_door_close", 0.1)
            --             else
            --                 AJFW.Functions.Notify("Wrong passcode, get it from drug dealers!", "error")
            --             end
            --         end
            --     end
            -- end
            ...
            if GetDistanceBetweenCoords(pos.x, pos.y, pos.z, washLocations.pOne.x, washLocations.pOne.y, washLocations.pOne.z, true) < 0.5 then
                if GetDistanceBetweenCoords(pos.x, pos.y, pos.z, washLocations.pOne.x, washLocations.pOne.y, washLocations.pOne.z, true) < 0.5 then      
                    if not washing and not collect then      
                        ...
                        if IsControlJustReleased(1, Keys['E']) then
                            AJFW.Functions.TriggerCallback('aj-moneywash:server:checkWash', function(HasItem, tamount)
                                if HasItem then
                                    -- TriggerServerEvent('aj-moneywash:server:getItem', amt)
                                    WashAnimation()
                                    AJFW.Functions.Progressbar("bills_wash", "Putting Black Money into the washer...", math.random(2000, 5000), false, true, {
...
--                     DrawText3D(washLocations.pOne.x, washLocations.pOne.y, washLocations.pOne.z, "~g~E~w~ - Start Washer")
--                         if IsControlJustReleased(1, Keys['E']) then
--                             AJFW.Functions.TriggerCallback('aj-moneywash:server:checkWash', function(HasItem, type)
--                                 if HasItem then
--                                     TriggerServerEvent('aj-moneywash:server:getItem', amt)
--                                     WashAnimation()
--                                     AJFW.Functions.Progressbar("bills_wash", "Putting Black Money into the washer...", math.random(2000, 5000), false, true, {
...
RegisterNetEvent('aj-moneywash:client:washTimer')
AddEventHandler('aj-moneywash:client:washTimer', function()
        Citizen.Wait(0)
        local pos = GetEntityCoords(PlayerPedId())
        local ped = PlayerPedId()
        local inRange = false
            ...
            if GetDistanceBetweenCoords(pos.x, pos.y, pos.z, collectCoordX,collectCoordY, collectCoordZ, true) < 0.5 then
                ...
                if GetDistanceBetweenCoords(pos.x, pos.y, pos.z, collectCoordX,collectCoordY, collectCoordZ, true) < 0.5 then      
                    if IsControlJustReleased(1, Keys['E']) and inRange then
                        WashAnimation()
                        AJFW.Functions.Progressbar("bills_collect", "Collecting clean bill\'s from washer...", math.random(5000, 10000), false, true, {
                            disableMovement = false,
                            disableCarMovement = false,
                            disableMouse = false,
                            disableCombat = true,
...
RegisterNetEvent('aj-moneywash:client:startTimer')
AddEventHandler('aj-moneywash:client:startTimer', function(amt)
    ...
    TriggerEvent('aj-moneywash:client:washTimer')
...
function IsWearingHandshoes()
    ...
    if model == GetHashKey("mp_m_freemode_01") then
        if AJFW.Shared.MaleNoGloves[armIndex] ~= nil and AJFW.Shared.MaleNoGloves[armIndex] then
            retval = false
        end
    else
        if AJFW.Shared.FemaleNoGloves[armIndex] ~= nil and AJFW.Shared.FemaleNoGloves[armIndex] then
            retval = false
        end
    end
    return retval
