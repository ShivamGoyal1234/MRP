    ...
    window.addEventListener('message', function (event) {
        ...
        if (data.action == "ui") {
            if (data.toggle) {
                ...
                setTimeout(function(){
                    $.post('https://mr-multicharacter/setupCharacters');
                    setTimeout(function(){
                        ...
                        $.post('https://mr-multicharacter/removeBlur');
                    }, 2000);
                }, 2000);
            } else {
...
$('.disconnect-btn').click(function(e){
    ...
    $.post('https://mr-multicharacter/closeUI');
    $.post('https://mr-multicharacter/disconnectButton');
});
...
$(document).on('click', '.character', function(e) {
    ...
    if (selectedChar === null) {
        ...
        if ((selectedChar).data('cid') == "") {
            ...
            $.post('https://mr-multicharacter/cDataPed', JSON.stringify({
                cData: cDataPed
            }));
        } else {
            $(selectedChar).addClass("char-selected");
...
function checkVulgor(firstname, lastname, nationality) {
    let retval = true
    profList.forEach(e =>{
        if (e === firstname) {
            retval = false;
...
$(document).on('click', '#create', function (e) {
    ...
    let backstory= escapeHtml($('#backstory').val())
    ...
    if (!firstname || !lastname || !nationality || !backstory || !birthdate || hasWhiteSpace(firstname) || hasWhiteSpace(lastname)|| hasWhiteSpace(nationality) ){
        console.log("FIELDS REQUIRED")
        return false;
    }

    if(checkVulgor(firstname, lastname, nationality)){
        //go ahead
    }else{
    ...
    $.post('https://mr-multicharacter/createNewCharacter', JSON.stringify({
        firstname: firstname,
        lastname: lastname,
...
$(document).on('click', '#accept-delete', function(e){
    $.post('https://mr-multicharacter/removeCharacter', JSON.stringify({
...
$(document).on('click', '#play', function(e) {
    ...
    if (selectedChar !== null) {
        if (charData !== "") {
            $.post('https://mr-multicharacter/selectCharacter', JSON.stringify({
