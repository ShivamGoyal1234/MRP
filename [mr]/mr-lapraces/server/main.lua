...
MRFW.Functions.CreateCallback('mr-lapraces:server:GetRacingLeaderboards', function(source, cb)
    cb(Races)
end)
...
RegisterServerEvent('mr-lapraces:server:FinishPlayer')
AddEventHandler('mr-lapraces:server:FinishPlayer', function(RaceData, TotalTime, TotalLaps, BestLap)
    local src = source
    ...
    if Races[RaceData.RaceId].Records ~= nil and next(Races[RaceData.RaceId].Records) ~= nil then
        if BLap < Races[RaceData.RaceId].Records.Time then
            ...
            TriggerClientEvent('mr-phone:client:RaceNotify', src, 'You have won the WR from ' .. RaceData.RaceName ..
    ...
    TriggerClientEvent('mr-lapraces:client:PlayerFinishs', -1, RaceData.RaceId, PlayersFinished, Player)
    ...
    TriggerClientEvent('mr-phone:client:UpdateLapraces', -1)
end)
...
function IsWhitelisted(CitizenId)
    ...
    local Perms = MRFW.Functions.GetPermission(Player.PlayerData.source)
...
RegisterServerEvent('mr-lapraces:server:CreateLapRace')
AddEventHandler('mr-lapraces:server:CreateLapRace', function(RaceName)
    local src = source
    local Player = MRFW.Functions.GetPlayer(src)

    if IsWhitelisted(Player.PlayerData.citizenid) then
        if IsNameAvailable(RaceName) then
            TriggerClientEvent('mr-lapraces:client:StartRaceEditor', source, RaceName)
        else
            TriggerClientEvent('MRFW:Notify', source, 'There is already a race with this name.', 'error')
        end
    else
        TriggerClientEvent('MRFW:Notify', source, 'You have not been authorized to race\'s to create.', 'error')
    end
end)

MRFW.Functions.CreateCallback('mr-lapraces:server:GetRaces', function(source, cb)
    -- print(tablePrintOut(AvailableRaces))
    cb(AvailableRaces)
end)
...
MRFW.Functions.CreateCallback('mr-lapraces:server:GetListedRaces', function(source, cb)
    cb(Races)
end)

MRFW.Functions.CreateCallback('mr-lapraces:server:GetRacingData', function(source, cb, RaceId)
    cb(Races[RaceId])
end)

MRFW.Functions.CreateCallback('mr-lapraces:server:HasCreatedRace', function(source, cb)
    cb(HasOpenedRace(MRFW.Functions.GetPlayer(source).PlayerData.citizenid))
end)

MRFW.Functions.CreateCallback('mr-lapraces:server:IsAuthorizedToCreateRaces', function(source, cb, TrackName)
    cb(IsWhitelisted(MRFW.Functions.GetPlayer(source).PlayerData.citizenid), IsNameAvailable(TrackName))
end)
...
MRFW.Functions.CreateCallback('mr-lapraces:server:GetTrackData', function(source, cb, RaceId)
...
RegisterServerEvent('mr-lapraces:server:JoinRace')
AddEventHandler('mr-lapraces:server:JoinRace', function(RaceData)
    local src = source
    local Player = MRFW.Functions.GetPlayer(src)
    ...
    if CurrentRace ~= nil then
        ...
        if (AmountOfRacers - 1) == 0 then
            ...
            TriggerClientEvent('MRFW:Notify', src, 'You were the only one in the race, the race had ended', 'error')
            TriggerClientEvent('mr-lapraces:client:LeaveRace', src, Races[CurrentRace])
        else
    ...
    TriggerClientEvent('mr-lapraces:client:JoinRace', src, Races[RaceId], RaceData.Laps)
    TriggerClientEvent('mr-phone:client:UpdateLapraces', -1)
    local creatorsource = MRFW.Functions.GetPlayerByCitizenId(AvailableRaces[AvailableKey].SetupCitizenId).PlayerData
                              .source
    if creatorsource ~= Player.PlayerData.source then
        TriggerClientEvent('mr-phone:client:RaceNotify', creatorsource,
            string.sub(Player.PlayerData.charinfo.firstname, 1, 1) .. '. ' .. Player.PlayerData.charinfo.lastname ..
                ' the race has been joined!')
    end
end)

RegisterServerEvent('mr-lapraces:server:LeaveRace')
AddEventHandler('mr-lapraces:server:LeaveRace', function(RaceData)
    local src = source
    local Player = MRFW.Functions.GetPlayer(src)
    local RaceName
    ...
    if (AmountOfRacers - 1) == 0 then
        ...
        table.remove(AvailableRaces, AvailableKey)
        TriggerClientEvent('MRFW:Notify', src, 'You were the only one in the race.The race had ended.', 'error')
        TriggerClientEvent('mr-lapraces:client:LeaveRace', src, Races[RaceId])
...
RegisterServerEvent('mr-lapraces:server:SetupRace')
AddEventHandler('mr-lapraces:server:SetupRace', function(RaceId, Laps)
            ...
            if not Races[RaceId].Started then
                ...
                SetTimeout(5 * 60 * 1000, function()
                    if Races[RaceId].Waiting then
                        ...
                        for cid, _ in pairs(Races[RaceId].Racers) do
                            local RacerData = MRFW.Functions.GetPlayerByCitizenId(cid)
                            if RacerData ~= nil then
                                TriggerClientEvent('mr-lapraces:client:LeaveRace', RacerData.PlayerData.source,
                                    Races[RaceId])
                            end
                        end
                        table.remove(AvailableRaces, AvailableKey)
                ...
                TriggerClientEvent('MRFW:Notify', source, 'The race is already running', 'error')
            end
        else
            TriggerClientEvent('MRFW:Notify', source, 'The race is already running', 'error')
        end
    else
        TriggerClientEvent('MRFW:Notify', source, 'This race does not exist :(', 'error')
    end
end)

RegisterServerEvent('mr-lapraces:server:UpdateRaceState')
AddEventHandler('mr-lapraces:server:UpdateRaceState', function(RaceId, Started, Waiting)
    Races[RaceId].Waiting = Waiting
    Races[RaceId].Started = Started
end)

RegisterServerEvent('mr-lapraces:server:UpdateRacerData')
AddEventHandler('mr-lapraces:server:UpdateRacerData', function(RaceId, Checkpoint, Lap, Finished)
    local src = source
    local Player = MRFW.Functions.GetPlayer(src)
    ...
    TriggerClientEvent('mr-lapraces:client:UpdateRaceRacerData', -1, RaceId, Races[RaceId])
end)

RegisterServerEvent('mr-lapraces:server:StartRace')
AddEventHandler('mr-lapraces:server:StartRace', function(RaceId)
    local src = source
    local MyPlayer = MRFW.Functions.GetPlayer(src)
    local AvailableKey = GetOpenedRaceKey(RaceId)
    ...
    if RaceId ~= nil then
        if AvailableRaces[AvailableKey].SetupCitizenId == MyPlayer.PlayerData.citizenid then
            ...
            for CitizenId, _ in pairs(Races[RaceId].Racers) do
                local Player = MRFW.Functions.GetPlayerByCitizenId(CitizenId)
                if Player ~= nil then
                    TriggerClientEvent('mr-lapraces:client:RaceCountdown', Player.PlayerData.source)
                end
            end
            TriggerClientEvent('mr-phone:client:UpdateLapraces', -1)
        else
            TriggerClientEvent('MRFW:Notify', src, 'You are not the creator of the race..', 'error')
        end
    else
        TriggerClientEvent('MRFW:Notify', src, 'You are not in a race..', 'error')
    end
end)

RegisterServerEvent('mr-lapraces:server:SaveRace')
AddEventHandler('mr-lapraces:server:SaveRace', function(RaceData)
    local src = source
    local Player = MRFW.Functions.GetPlayer(src)
...
MRFW.Commands.Add("togglesetup", "Turn on / off racing setup", {}, false, function(source, args)
    local Player = MRFW.Functions.GetPlayer(source)

    if IsWhitelisted(Player.PlayerData.citizenid) then
        Config.RaceSetupAllowed = not Config.RaceSetupAllowed
        if not Config.RaceSetupAllowed then
            TriggerClientEvent('MRFW:Notify', source, 'No more races can be created!', 'error')
        else
            TriggerClientEvent('MRFW:Notify', source, 'Races can be created again!', 'success')
        end
    else
        TriggerClientEvent('MRFW:Notify', source, 'You have not been authorized to do this.', 'error')
    end
end)

MRFW.Commands.Add("cancelrace", "Cancel going race..", {}, false, function(source, args)
    local Player = MRFW.Functions.GetPlayer(source)
        ...
        if RaceName ~= nil then
            ...
            if Races[RaceId].Started then
                ...
                for cid, _ in pairs(Races[RaceId].Racers) do
                    local RacerData = MRFW.Functions.GetPlayerByCitizenId(cid)
                    if RacerData ~= nil then
                        TriggerClientEvent('mr-lapraces:client:LeaveRace', RacerData.PlayerData.source, Races[RaceId])
                    end
                end
                Citizen.Wait(2500)
                ...
                TriggerClientEvent('MRFW:Notify', source, 'This race has not started yet.', 'error')
            end
        end
    else
        TriggerClientEvent('MRFW:Notify', source, 'You have not been authorized to do this.', 'error')
...
MRFW.Functions.CreateCallback('mr-lapraces:server:CanRaceSetup', function(source, cb)
