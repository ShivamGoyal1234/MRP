...
--[[RegisterNetEvent('MRFW:Client:Refreshskin')
AddEventHandler('MRFW:Client:Refreshskin', function()
    TriggerServerEvent("mr-clothes:loadPlayerSkin")
end)]]
RegisterNetEvent('MRFW:Client:Refreshskin')
AddEventHandler('MRFW:Client:Refreshskin', function()
    --TriggerServerEvent('mr-clothing:saveSkin')
    Citizen.Wait(100)
    local ped = PlayerPedId()
    if not IsPedInAnyVehicle(ped, false) then
        TriggerServerEvent("mr-clothes:loadPlayerSkins")
    else
        MRFW.Functions.Notify('Chala ja kal aana kal', 'error', 3000)
    end
end)
...
RegisterNetEvent('mr-clothing:clothing', function()
    purchase = true
...
RegisterNetEvent('mr-clothing:barber', function()
    purchase = true
    customCamLocation = nil
    openMenu({
                        ...
                        if dist < 1 then
                            if PlayerData.job.name == Config.ClothingRooms[k].requiredJob then
                                ...
                                if IsControlJustPressed(0, Keys["E"]) then
                                    ...
                                    gender = "male"
                                    if MRFW.Functions.GetPlayerData().charinfo.gender == 1 then gender = "female" end
                                    MRFW.Functions.TriggerCallback('mr-clothing:server:getOutfits', function(result)
                                        openMenu({
...
RegisterNetEvent('mr-clothing:client:openOutfitMenu')
AddEventHandler('mr-clothing:client:openOutfitMenu', function()
    MRFW.Functions.TriggerCallback('mr-clothing:server:getOutfits', function(result)
        openMenu({
...
RegisterNetEvent('mr-clothing:client:openBarberMenu')
AddEventHandler('mr-clothing:client:openBarberMenu', function()
    customCamLocation = nil
    openMenu({
...
RegisterNetEvent('mr-clothing:client:openClothingMenu')
AddEventHandler('mr-clothing:client:openClothingMenu', function()
    customCamLocation = nil
    openMenu({
        {menu = "character", label = "Clothes", selected = true},
        {menu = "accessoires", label = "Accessories", selected = false}
...
RegisterNUICallback('selectOutfit', function(data)
    ...
    TriggerEvent('mr-clothing:client:loadOutfit', data)
end)
...
RegisterNetEvent('mr-clothing:client:openMenu')
AddEventHandler('mr-clothing:client:openMenu', function()
    customCamLocation = nil
    openMenu({
...
function GetMaxValues()
    maxModelValues = {
        ...
        ["lipstick2"]    = {type = "hair", item = 0, texture = 0},
...
RegisterNUICallback('saveOutfit', function(data, cb)
    ...
    TriggerServerEvent('mr-clothes:saveOutfit', data.outfitName, model, skinData)
end)

RegisterNetEvent('mr-clothing:client:reloadOutfits')
AddEventHandler('mr-clothing:client:reloadOutfits', function(myOutfits)
    SendNUIMessage({
...
RegisterNUICallback('resetOutfit', function()
    TriggerServerEvent("mr-multicharacter:server:removeBucket")
...
function resetClothing(data)
    ...
    SetPedHeadOverlayColor(ped, 8, 2, data["lipstick"].texture, data["lipstick2"].texture)
...
RegisterNUICallback('removeOutfit', function(data, cb)
    TriggerServerEvent('mr-clothing:server:removeOutfit', data.outfitName, data.outfitId)
...
function ChangeVariation(data)
    ...
    elseif clothingCategory == "lipstick" then
        local retval, overlayValue, colourType, firstColour, secondColour, overlayOpacity = GetPedHeadOverlayData(PlayerPedId(),8)
        if type == "item" then
            SetPedHeadOverlay(ped, 8, item, 1.0)
            skinData["lipstick"].item = item
        elseif type == "texture" then
            SetPedHeadOverlayColor(ped, 8, 2, item, secondColour)
            skinData["lipstick"].texture = item
        end
    elseif clothingCategory == "lipstick2" then
        local retval, overlayValue, colourType, firstColour, secondColour, overlayOpacity = GetPedHeadOverlayData(PlayerPedId(),8)
        if type == "item" then
        elseif type == "texture" then
            SetPedHeadOverlayColor(ped, 8,2, firstColour, item)
            skinData["lipstick2"].texture = item
        end
    ...
    Citizen.CreateThread(function()
        ...
        for k, v in pairs(skinData) do
            if skin == "mp_m_freemode_01" or skin == "mp_f_freemode_01" then
                ...
                if k ~= "face" and k ~= "hair2"and k ~= "face2" then
                    ChangeVariation({
                        clothingType = k,
                        articleNumber = v.defaultItem,
                        type = "item",
...
function SaveSkin()
	...
	TriggerServerEvent("mr-clothing:saveSkin", model, clothing, cut)
    TriggerServerEvent("mr-multicharacter:server:removeBucket")
end

RegisterNetEvent('mr-clothes:client:CreateFirstCharacter')
AddEventHandler('mr-clothes:client:CreateFirstCharacter', function()
...
RegisterNetEvent("mr-clothes:loadSkin")
AddEventHandler("mr-clothes:loadSkin", function(new, model, data)
    ...
    Citizen.CreateThread(function()
        ...
        TriggerEvent('mr-clothing:client:loadPlayerClothing', data, PlayerPedId())
    end)
end)

--[[RegisterNetEvent("mr-clothes:loadSkins")
AddEventHandler("mr-clothes:loadSkins", function(new, model, data)
    model = model ~= nil and tonumber(model) or false
    ...
    Citizen.CreateThread(function()
        ...
        Citizen.Wait(10)
        TriggerServerEvent("mr-clothing:saveSkin" , PlayerPedId())
        Citizen.Wait(100)
...
RegisterNetEvent('mr-clothing:client:loadPlayerClothing')
AddEventHandler('mr-clothing:client:loadPlayerClothing', function(data, ped)
    ...
    if data["lipstick2"].texture == nil then
        SetPedHeadOverlayColor(ped, 8, 2, data["lipstick"].texture, data["lipstick"].texture)
    else
        SetPedHeadOverlayColor(ped, 8, 2, data["lipstick"].texture, data["lipstick2"].texture)
    end
    skinData = data
...
RegisterNetEvent('mr-clothing:client:loadOutfit')
AddEventHandler('mr-clothing:client:loadOutfit', function(oData)
    local ped = PlayerPedId()
    ...
    if data["accessory"] ~= nil then
        if MRFW.Functions.GetPlayerData().metadata["tracker"] then
...
RegisterNetEvent("mr-clothing:client:adjustfacewear")
AddEventHandler("mr-clothing:client:adjustfacewear",function(type)
    if MRFW.Functions.GetPlayerData().metadata["ishandcuffed"] then return end
