...
AJClothing.ResetItemTexture = function(obj, category) {
    ...
    $.post('https://mr-clothing/updateSkin', JSON.stringify({
...
$(document).on('click', '.clothing-menu-option-item-right', function(e){
    ...
    if (canChange) {
        if (hasTracker && clothingCategory == "accessory") {
            $.post('https://mr-clothing/TrackerError');
            return
        } else {
            if (clothingCategory == "model") {
                $(inputElem).val(newValue);
                $.post('https://mr-clothing/setCurrentPed', JSON.stringify({ped: newValue}), function(model){
...
$(document).on('change', '.item-number', function(){
    ...
    if (hasTracker && clothingCategory == "accessory") {
        ...
        if (clothingCategory == "accessory" && inputVal == 13) {
            ...
            $.post('https://mr-clothing/updateSkinOnInput', JSON.stringify({
                clothingType: clothingCategory,
...
$(document).on('click', '.clothing-menu-header-camera-btn', function(e){
    ...
    if (selectedCam == null) {
        ...
        $.post('https://mr-clothing/setupCam', JSON.stringify({
            value: camValue
        }));
        selectedCam = this;
    } else {
...
$(document).on('keydown', function() {
    switch(event.keyCode) {
        case 68: // D
            $.post('https://mr-clothing/rotateRight');
            break;
        case 65: // A
            $.post('https://mr-clothing/rotateLeft');
            break;
        case 38: // UP
            ChangeUp();
            break;
    ...
    window.addEventListener('message', function(event) {
        switch(event.data.action) {
            ...
            case "HideFaceAndModel":
                AJClothing.HideFaceAndModel(event.data.toggle);
            break;
        }
    })
});
...
$(document).on('click', "#save-menu", function(e){
    e.preventDefault();
    AJClothing.Close();
    $.post('https://mr-clothing/saveClothing');
});

$(document).on('click', "#cancel-menu", function(e){
    e.preventDefault();
    AJClothing.Close();
    $.post('https://mr-clothing/resetOutfit');
});
...
$(document).on('click', '.clothing-menu-outfit-option-button', function(e){
    ...
    $.post('https://mr-clothing/selectOutfit', JSON.stringify({
...
$(document).on('click', '.clothing-menu-myOutfit-option-button-remove', function(e){
    ...
    $.post('https://mr-clothing/removeOutfit', JSON.stringify({
        outfitData: outfitData.skin,
        outfitName: outfitData.outfitname,
        outfitId: outfitData.outfitId,
    }));
...
AJClothing.HideFaceAndModel = function(toggle) {
    if(toggle){
        $("div[data-type='face']").hide();
        $("div[data-type='face2']").hide();
        $("div[data-type='facemix']").hide();
        ...
        $("div[data-type='face2']").show();
        $("div[data-type='facemix']").show();
...
AJClothing.HideFaceAndModel2 = function(toggle) {
    if(toggle){
        $("div[data-type='eye_color']").hide();
        $("div[data-type='moles']").hide();
        $("div[data-type='nose_0']").hide();
...
$(document).on('click', '#save-outfit-save', function(e){
    ...
    $.post('https://mr-clothing/saveOutfit', JSON.stringify({
...
$(document).on('click', '.change-camera-button', function(e){
    ...
    $.post('https://mr-clothing/rotateCam', JSON.stringify({
        type: rotationType
    }))
});
