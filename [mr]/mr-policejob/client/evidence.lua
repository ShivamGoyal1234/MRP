...
function IsSilentWeapon(weapon)
	...
	if not retval then
		MRFW.Functions.TriggerCallback('police:IsSilencedWeapon', function(result)
			retval = result
			return result
		end, weapon)
		Citizen.Wait(100)
...
RegisterNetEvent('evidence:client:SetStatus', function(statusId, time)
    if time > 0 and StatusList[statusId] then
        if (CurrentStatusList == nil or CurrentStatusList[statusId] == nil) or
            ...
            MRFW.Functions.Notify(''..CurrentStatusList[statusId].text..'', 'error')
        end
...
RegisterNetEvent('evidence:client:ClearBlooddropsInArea', function()
    ...
    local blooddropList = {}
    MRFW.Functions.Progressbar('clear_blooddrops', 'Clearing blood..', 5000, false, true, {
        ...
        if Blooddrops and next(Blooddrops) then
            ...
            MRFW.Functions.Notify('Blood cleared :)')
        end
    end, function() -- Cancel
        MRFW.Functions.Notify('Blood not cleared..', 'error')
    end)
end)
...
RegisterNetEvent('evidence:client:ClearCasingsInArea', function()
    local pos = GetEntityCoords(PlayerPedId())
    local casingList = {}
    MRFW.Functions.Progressbar('clear_casings', 'Removing bullet sleeves..', 5000, false, true, {
        disableMovement = false,
        disableCarMovement = false,
        disableMouse = false,
        disableCombat = true
        ...
        if Casings and next(Casings) then
            ...
            MRFW.Functions.Notify('Bullet sleeves removed :)')
        end
    end, function() -- Cancel
        MRFW.Functions.Notify('Bullet sleeves not removed', 'error')
    end)
end)
            ...
            if #(pos -vector3(Casings[CurrentCasing].coords.x, Casings[CurrentCasing].coords.y, Casings[CurrentCasing].coords.z)) < 1.5 then
                ...
                if IsControlJustReleased(0, 47) then
                    ...
                    local info = {
                        ...
                        ammolabel = Config.AmmoLabels[MRFW.Shared.Weapons[Casings[CurrentCasing].type]['ammotype']],
        ...
        if isArmed then
            if PlayerJob.name ~= 'police' then
                if IsPedShooting(PlayerPedId()) and not IsSilentWeapon(currentWeapon) and IsPedNearby() then
                    ...
                    if MRFW.Shared.Weapons[currentWeapon]['ammotype'] ~= 'AMMO_PISTOL' then
                        automatic = true
                    end
                ...
                if IsPedShooting(PlayerPedId()) and not IsSilentWeapon(currentWeapon) and IsPedNearby() then
                    ...
                    if IsPedInAnyVehicle(PlayerPedId(), true) then
                        ...
                        TriggerServerEvent('police:server:GunshotAlert2', streetLabel, automatic, true, coords, vehicleInfo)
                    else
                        TriggerServerEvent('police:server:GunshotAlert2', streetLabel, automatic, false, coords)
                    end
                    Wait(5000)
                end
