...
-- Script starts here
local helicam = false
local police_heli = `as350`
local ems_heli = `polmav`
local news_heli = `newsheli`
local news_heli2 = `newsheli2`
...
local function IsPlayerInPolmav()
	...
	if     IsVehicleModel(vehicle, police_heli) then
		return true
	elseif IsVehicleModel(vehicle, ems_heli) then
		return true
	elseif IsVehicleModel(vehicle, news_heli) then
		return true
	elseif IsVehicleModel(vehicle, news_heli2) then
		return true
	end
	return false
end

local function IsPlayerInPolmav2()
	local lPed = PlayerPedId()
	local vehicle = GetVehiclePedIsIn(lPed)
...
local function RenderVehicleInfo(vehicle)
	...
	local licenseplate = MRFW.Functions.GetPlate(vehicle)
...
RegisterCommand('+helicoptor', function()
...
RegisterKeyMapping("+helicoptor", "Heli Camera", "keyboard", "E")
...
Citizen.CreateThread(function()
	while true do
		...
		if LocalPlayer.state['isLoggedIn'] then
			sleep = 2000
			if PlayerJob.name == 'police' or PlayerJob.name == 'doctor' or PlayerJob.name == 'reporter'then
				sleep = 2000
				if IsPlayerInPolmav() then
					sleep = 5
					local lPed = PlayerPedId()
