...
RegisterNetEvent('police:client:spawnCone', function()
    MRFW.Functions.Progressbar("spawn_object", "Place object..", 2500, false, true, {
...
RegisterNetEvent('police:client:tv')
AddEventHandler('police:client:tv', function()
    local coords = GetEntityCoords(PlayerPedId())
    MRFW.Functions.Progressbar("spawn_object", "Place object..", 2500, false, true, {
        disableMovement = true,
        disableCarMovement = true,
        ...
        StopAnimTask(PlayerPedId(), "anim@narcotics@trash", "drop_front", 1.0)
        TriggerServerEvent("police:server:spawnObject", "tv")
    end, function() -- Cancel
        StopAnimTask(PlayerPedId(), "anim@narcotics@trash", "drop_front", 1.0)
        MRFW.Functions.Notify("Canceled..", "error")
    end)
...
RegisterNetEvent('police:client:deleteObject', function()
    ...
    if dist < 5.0 then
        MRFW.Functions.Progressbar("remove_object", "Removing object..", 2500, false, true, {
            disableMovement = true,
            disableCarMovement = true,
            disableMouse = false,
            disableCombat = true,
...
RegisterNetEvent('police:client:SpawnSpikeStrip', function()
    if #SpawnedSpikes + 1 <= MaxSpikes then
        ...
        MRFW.Functions.Notify('There are no Spikestrips left..', 'error')
    end
end)
