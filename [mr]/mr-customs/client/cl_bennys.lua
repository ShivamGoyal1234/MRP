...
local function saveVehicle()
    ...
    local myCar = MRFW.Functions.GetVehicleProperties(veh)
    TriggerServerEvent('updateVehicle',myCar)  
end

RegisterNetEvent('mr-custom:client:updatemodsindatabase', function()
    saveVehicle()
end)
...
function AttemptPurchase(type, upgradeLevel)
    ...
    TriggerServerEvent("mr-customs:attemptPurchase", type, upgradeLevel)
...
RegisterNetEvent('event:control:bennys', function(useID, tg)
    if IsPedInAnyVehicle(PlayerPedId(), false) then
        bennyHeading = bennyGarages[useID].coords.w
        if not isPlyInBennys then -- Bennys
            enterLocation(bennyLocation, tg)
        end
    end
end)

function enterLocation(locationsPos, tg)
    local plyPed = PlayerPedId()
    local plyVeh = GetVehiclePedIsIn(plyPed, false)
    ...
    InitiateMenus(isMotorcycle, GetVehicleBodyHealth(plyVeh), tg)

    SetTimeout(100, function()
        MRFW.Functions.TriggerCallback('mr-customs:getMechanic', function(result)
            if result then
                DisplayMenu(true, "mainMenu")
            else
                if GetVehicleBodyHealth(plyVeh) < 1000.0 or GetVehicleEngineHealth(plyVeh) < 1000.0 then
...
RegisterNetEvent('event:openfor:admins', function(data)
    if GetPedInVehicleSeat(GetVehiclePedIsIn(PlayerPedId()), -1) == PlayerPedId() then
        if not adminmenu then
            adminmenu = true
            openMenuCustom()
        else
            MRFW.Functions.Notify("Already in menu", "error")
        end
    else
        MRFW.Functions.Notify("You are not in vehicle", "error")
    end
end)
...
function openMenuCustom()
    ...
    InitiateAdminMenus(isMotorcycle, GetVehicleBodyHealth(plyVeh))
    SetTimeout(100, function()
        if GetVehicleBodyHealth(plyVeh) < 1000.0 or GetVehicleEngineHealth(plyVeh) < 1000.0 then
            DisplayMenu(true, "repairMenu")
        else
                ...
                if nearDefault then
                    ...
                    if nearDefault then
                        if not isPlyInBennys then
                            Draw3DText(v.coords.x, v.coords.y, v.coords.z + 0.5, "[Press ~p~Y~w~ - Enter Los Santos Customs]", 255, 255, 255, 255, 4, 0.45, true, true, true, true, 0, 0, 0, 0, 55)
                            if IsControlJustReleased(1, 246) then
                                if GetPedInVehicleSeat(GetVehiclePedIsIn(PlayerPedId()), -1) == PlayerPedId() then
                                    local isallowed
                                    local check_props = MRFW.Functions.GetVehicleProperties(GetVehiclePedIsIn(plyPed, false))
                                    local check_hash = check_props.model
                                    local check_vehname = GetDisplayNameFromVehicleModel(check_hash):lower()
                                    if v.AllowedVehicles then
                                        if v.AllowedVehicles[check_vehname] then
                                            isallowed = true
                                        else
                                            isallowed = false
                                        end
                                    elseif v.BlockedVehicle then
                                        if v.BlockedVehicle[check_vehname] then
                                            isallowed = false
                                        else
                                            isallowed = true
                                        end
                                    ...
                                    if isallowed then
                                        local playerssssssssssss = MRFW.Functions.GetPlayerData().job
                                        if (v.useJob and isAuthorized((playerssssssssssss.name), k)) or not v.useJob then
                                            if playerssssssssssss.name == 'police' and v.usejob then
                                                if playerssssssssssss.grade.level >= 8 then
                                                    TriggerEvent('event:control:bennys', k, v.useJob)
                                                else
                                                    MRFW.Functions.Notify("Contact High Command", "error")
                                                end
                                            else
                                                TriggerEvent('event:control:bennys', k, v.useJob)
                                            end
                                        else
                                            MRFW.Functions.Notify("You are not authorized", "error")
                                        end
                                    else
                                        MRFW.Functions.Notify("can't customize this vehicle here", "error")
                                    end
                                end
                            end
                        else
...
--#[Event Handlers]#--
RegisterNetEvent("mr-customs:purchaseSuccessful", function()
    isPurchaseSuccessful = true
    attemptingPurchase = false
    MRFW.Functions.Notify("Purchase Successful")
end)

RegisterNetEvent("mr-customs:purchaseFailed", function()
    isPurchaseSuccessful = false
    attemptingPurchase = false
    MRFW.Functions.Notify("Not enough money", "error")
end)
