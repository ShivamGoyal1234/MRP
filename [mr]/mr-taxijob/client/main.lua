...
RegisterNetEvent('mr-taxi:client:DoTaxiNpc')
AddEventHandler('mr-taxi:client:DoTaxiNpc', function()
    if whitelistedVehicle() then
       -- if NpcData.CountDown == 180 then
            if not NpcData.Active then
                ...
                MRFW.Functions.Notify('The NPC Is Indicated On Your GPS', 'success')
                        ...
                        if dist < 20 then
                            ...
                            if dist < 5 then
                                ...
                                if IsControlJustPressed(0, 38) then
                                    ...
                                    MRFW.Functions.Notify('Bring The NPC To The Specified Location')
                                    if NpcData.NpcBlip ~= nil then
                                        RemoveBlip(NpcData.NpcBlip)
                                    end
                                    GetDeliveryLocation()
                ...
                MRFW.Functions.Notify('You Are Already Doing An NPC Mission')
            end
      --  else
         --   MRFW.Functions.Notify('No NPCs Are Available')
       -- end
    else
        MRFW.Functions.Notify('You Are Not In A Taxi')
    end
end)
            ...
            if dist < 20 then
                ...
                if dist < 5 then
                    ...
                    if IsControlJustPressed(0, 38) then
                        ...
                        TriggerServerEvent('mr-taxi:server:NpcPay', meterData.currentFare)
                        meterActive = false
                        SendNUIMessage({
                            action = "resetMeter"
                        })
                        MRFW.Functions.Notify('Person Was Dropped Off', 'success')
                        if NpcData.DeliveryBlip ~= nil then
                            RemoveBlip(NpcData.DeliveryBlip)
                        end
...
RegisterNetEvent('mr-taxi:client:toggleMeter')
AddEventHandler('mr-taxi:client:toggleMeter', function()
    local ped = PlayerPedId()
    ...
    if IsPedInAnyVehicle(ped, false) then
        if whitelistedVehicle() then 
            ...
            MRFW.Functions.Notify('This Vehicle Has No Taxi Meter', 'error')
        end
    else
        MRFW.Functions.Notify('You\'re Not In A Vehicle ', 'error')
    end
end)

RegisterNetEvent('mr-taxi:client:enableMeter')
AddEventHandler('mr-taxi:client:enableMeter', function()
    local ped = PlayerPedId()
    ...
    if meterIsOpen then
        ...
        MRFW.Functions.Notify('The Taxi Meter Is Not Active', 'error')
    end
end)
...
RegisterNetEvent('mr-taxi:client:toggleMuis')
AddEventHandler('mr-taxi:client:toggleMuis', function()
    Citizen.Wait(400)
    if meterIsOpen then
        ...
        MRFW.Functions.Notify('No Taxi Meter In Sight', 'error')
    end
end)
...
function TakeVehicle(k)
    ...
    MRFW.Functions.SpawnVehicle(Config.AllowedVehicles[k].model, function(veh)
        ...
        dutyPlate = MRFW.Functions.GetPlate(veh)
    end, coords, true)
end
