MRFW = exports['mrfw']:GetCoreObject() -- Used Globally
inJail = false
jailTime = 0
...
RegisterNetEvent('MRFW:Client:OnPlayerLoaded', function()
	...
	MRFW.Functions.TriggerCallback('prison:server:IsAlarmActive', function(active)
		if active then
...
RegisterNetEvent('prison:client:Enter', function(time)
	MRFW.Functions.Notify( Lang:t("error.injail", {Time = time}), "error")
	...
	Wait(2000)
	DoScreenFadeIn(1000)
	MRFW.Functions.Notify( Lang:t("error.do_some_work", {currentjob = Config.Jobs[currentJob] }), "error")
end)

RegisterNetEvent('prison:client:Leave', function()
	if jailTime > 0 then
		MRFW.Functions.Notify( Lang:t("info.timeleft", {JAILTIME = jailTime}))
	else
		jailTime = 0
		...
		RemoveBlip(ShopBlip)
		ShopBlip = nil
		MRFW.Functions.Notify(Lang:t("success.free_"))
		DoScreenFadeOut(500)
		while not IsScreenFadedOut() do
			Wait(10)
		end
	...
	while true do
		...
		if jailTime > 0 and inJail then
			...
			if jailTime > 0 and inJail then
				...
				if jailTime <= 0 then
					jailTime = 0
					MRFW.Functions.Notify(Lang:t("success.timesup"), "success", 10000)
				end
				TriggerServerEvent("prison:server:SetJailStatus", jailTime)
			end
		else
