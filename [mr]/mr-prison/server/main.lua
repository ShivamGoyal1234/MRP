...
RegisterNetEvent('prison:server:SetJailStatus', function(jailTime)
    ...
    --         TriggerClientEvent('MRFW:Notify', src, Lang:t("info.lost_job"))
    --     end
    -- end
end)
    ...
    -- for k, v in pairs(MRFW.Functions.GetPlayers()) do
        ...
        -- if Player ~= nil then
            -- if (Player.PlayerData.job.name == "police" and Player.PlayerData.job.onduty) then
                TriggerClientEvent("prison:client:PrisonBreakAlert", source)
            -- end
        -- end
	-- end
end)
...
MRFW.Functions.CreateCallback('prison:server:IsAlarmActive', function(source, cb)
    cb(AlarmActivated)
end)
