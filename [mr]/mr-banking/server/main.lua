...
RegisterNetEvent('mr-banking:createNewCard', function()
    local src = source
    ...
    TriggerEvent('mr-log:server:CreateLog', 'banking', 'Banking', 'lightgreen', "**"..GetPlayerName(xPlayer.PlayerData.source) .. " (citizenid: "..xPlayer.PlayerData.citizenid.." | id: "..xPlayer.PlayerData.source..")**" .. " created new card")
end)

--[[ -- Only used by the following "mr-banking:initiateTransfer"
...
RegisterNetEvent('mr-banking:initiateTransfer', function(_)
    --[[
    local _src = source
    local _startChar = MRFW.Functions.GetPlayer(_src)
    ...
    if (checkAccount) then
        ...
        if receiptName ~= false or receiptName ~= nil then
            local userOnline = exports.mr-base:checkOnline(cid)

            if userOnline ~= false then
                -- User is online so we can do a straght transfer
                local _targetUser = exports.mr-base:Source(userOnline)
                if acType == "Current" then
                    ...
                    TriggerClientEvent('mr-banking:openBankScreen', _src)
                    TriggerClientEvent('mr-banking:successAlert', _src, 'You have sent a bank transfer to '..receiptName..' for the amount of $'..data.amount)
                else
        ...
        TriggerClientEvent('mr-banking:transferError', _src, 'The account details entered could not be located.')
    end
]]
end)
...
MRFW.Functions.CreateCallback('mr-banking:getBankingInformation', function(source, cb)
    local src = source
    local xPlayer = MRFW.Functions.GetPlayer(src)
...
RegisterNetEvent('mr-banking:createBankCard', function(pin)
    local src = source
    local xPlayer = MRFW.Functions.GetPlayer(src)
    local cid = xPlayer.PlayerData.citizenid
    ...
    TriggerClientEvent('mr-banking:openBankScreen', src)
    TriggerClientEvent('MRFW:Notify', src, Lang:t('success.debit_card'), 'success')

    TriggerEvent('mr-log:server:CreateLog', 'banking', 'Banking', 'lightgreen', "**"..GetPlayerName(xPlayer.PlayerData.source) .. " (citizenid: "..xPlayer.PlayerData.citizenid.." | id: "..xPlayer.PlayerData.source..")** successfully ordered a debit card")
end)

RegisterNetEvent('mr-banking:doQuickDeposit', function(amount)
    local src = source
    local xPlayer = MRFW.Functions.GetPlayer(src)
    while xPlayer == nil do Wait(0) end
    ...
    if tonumber(amount) <= currentCash then
        ...
        if bank then
            TriggerClientEvent('mr-banking:openBankScreen', src)
            TriggerClientEvent('mr-banking:successAlert', src, Lang:t('success.cash_deposit', {value = amount}))
            TriggerEvent('mr-log:server:CreateLog', 'banking', 'Banking', 'lightgreen', "**"..GetPlayerName(xPlayer.PlayerData.source) .. " (citizenid: "..xPlayer.PlayerData.citizenid.." | id: "..xPlayer.PlayerData.source..")** made a cash deposit of $"..amount.." successfully.")
        end
    end
end)

RegisterNetEvent('mr-banking:toggleCard', function(_)
    local src = source
    local xPlayer = MRFW.Functions.GetPlayer(src)
...
RegisterNetEvent('mr-banking:doQuickWithdraw', function(amount, _)
    local src = source
    local xPlayer = MRFW.Functions.GetPlayer(src)
    while xPlayer == nil do Wait(0) end
    ...
    if tonumber(amount) <= currentCash then
        ...
        if cash then
            TriggerClientEvent('mr-banking:openBankScreen', src)
            TriggerClientEvent('mr-banking:successAlert', src, Lang:t('success.cash_withdrawal', {value = amount}))
            TriggerEvent('mr-log:server:CreateLog', 'banking', 'Banking', 'red', "**"..GetPlayerName(xPlayer.PlayerData.source) .. " (citizenid: "..xPlayer.PlayerData.citizenid.." | id: "..xPlayer.PlayerData.source..")** made a cash withdrawal of $"..amount.." successfully.")
        end
    end
end)

RegisterNetEvent('mr-banking:updatePin', function(pin)
    if pin ~= nil then
        local src = source
        local xPlayer = MRFW.Functions.GetPlayer(src)
        while xPlayer == nil do Wait(0) end
        ...
        TriggerClientEvent('mr-banking:openBankScreen', src)
        TriggerClientEvent('mr-banking:successAlert', src, Lang:t('success.updated_pin'))
    end
end)

RegisterNetEvent('mr-banking:savingsDeposit', function(amount)
    local src = source
    local xPlayer = MRFW.Functions.GetPlayer(src)
    while xPlayer == nil do Wait(0) end
    ...
    if tonumber(amount) <= currentBank then
        ...
        TriggerClientEvent('mr-banking:openBankScreen', src)
        TriggerClientEvent('mr-banking:successAlert', src, Lang:t('success.savings_deposit', {value = tostring(amount)}))
        TriggerEvent('mr-log:server:CreateLog', 'banking', 'Banking', 'lightgreen', "**"..GetPlayerName(xPlayer.PlayerData.source) .. " (citizenid: "..xPlayer.PlayerData.citizenid.." | id: "..xPlayer.PlayerData.source..")** made a savings deposit of $"..tostring(amount).." successfully..")
    end
end)

RegisterNetEvent('mr-banking:savingsWithdraw', function(amount)
    local src = source
    local xPlayer = MRFW.Functions.GetPlayer(src)
    while xPlayer == nil do Wait(0) end
    local currentSavings = savingsAccounts[xPlayer.PlayerData.citizenid].GetBalance()

    if tonumber(amount) <= tonumber(currentSavings) then
        ...
        while savings == nil do Wait(0) end
        TriggerClientEvent('mr-banking:openBankScreen', src)
        TriggerClientEvent('mr-banking:successAlert', src, Lang:t('success.savings_withdrawal', {value = tostring(amount)}))
        TriggerEvent('mr-log:server:CreateLog', 'banking', 'Banking', 'red', "**"..GetPlayerName(xPlayer.PlayerData.source) .. " (citizenid: "..xPlayer.PlayerData.citizenid.." | id: "..xPlayer.PlayerData.source..")** made a savings withdrawal of $"..tostring(amount).." successfully.")
    end
end)

RegisterNetEvent('mr-banking:createSavingsAccount', function()
    local src = source
    local xPlayer = MRFW.Functions.GetPlayer(src)
    local success = createSavingsAccount(xPlayer.PlayerData.citizenid)
    repeat Wait(0) until success ~= nil
    TriggerClientEvent('mr-banking:openBankScreen', src)
    TriggerClientEvent('mr-banking:successAlert', src, Lang:t('success.opened_savings'))
    TriggerEvent('mr-log:server:CreateLog', 'banking', 'Banking', "lightgreen", "**"..GetPlayerName(xPlayer.PlayerData.source) .. " (citizenid: "..xPlayer.PlayerData.citizenid.." | id: "..xPlayer.PlayerData.source..")** opened a savings account")
end)


MRFW.Commands.Add('givecash', Lang:t('command.givecash'), {{name = 'id', help = 'Player ID'}, {name = 'amount', help = 'Amount'}}, true, function(source, args)
    local src = source
	...
	if id and amount then
		local xPlayer = MRFW.Functions.GetPlayer(src)
		local xReciv = MRFW.Functions.GetPlayer(id)
                    ...
                    if amount > 0 then
                        if xPlayer.Functions.RemoveMoney('cash', amount) then
                            if xReciv.Functions.AddMoney('cash', amount) then
                                TriggerClientEvent('MRFW:Notify', src, Lang:t('success.give_cash',{id = tostring(id), cash = tostring(amount)}), "success")
                                TriggerClientEvent('MRFW:Notify', id, Lang:t('success.received_cash',{id = tostring(src), cash = tostring(amount)}), "success")
                                TriggerClientEvent("payanimation", src)
                            else
                                -- Return player cash
                                xPlayer.Functions.AddMoney('cash', amount)
                                TriggerClientEvent('MRFW:Notify', src, Lang:t('error.not_give'), "error")
                            end
                        else
                            TriggerClientEvent('MRFW:Notify', src, Lang:t('error.not_enough'), "error")
                        end
                    else
                        TriggerClientEvent('MRFW:Notify', src, Lang:t('error.invalid_amount'), "error")
                    end
				else
					TriggerClientEvent('MRFW:Notify', src, Lang:t('error.too_far_away'), "error")
				end
			else
				TriggerClientEvent('MRFW:Notify', src, Lang:t('error.dead'), "error")
			end
		else
			TriggerClientEvent('MRFW:Notify', src, Lang:t('error.wrong_id'), "error")
		end
	else
		TriggerClientEvent('MRFW:Notify', src, Lang:t('error.givecash'), "error")
	end
end)
