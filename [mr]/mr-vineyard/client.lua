							...
							DrawText3Ds(-1928.81, 2059.53, 140.84, "[E] Start Picking Grapes") 
								...
								if IsControlJustReleased(0,38) then
									-- if PlayerJob.name == "vineyard" then
										startVineyard = true
									-- else
									-- 	MRFW.Functions.Notify("I dont think I work here...", "error")
									-- end
								end
						end
					end
	...
	while true do
		if startVineyard then
			if tasking then
				...
				TriggerEvent("mr-vineyard:client:startVineyard")
...
RegisterNetEvent('mr-vineyard:client:startVineyard')
AddEventHandler('mr-vineyard:client:startVineyard', function()
...
function pickgrapes()
	...
	if success then
		TriggerServerEvent("mr-vineyard:server:getGrapes")
		tasking = false
		DeleteBlip()
	end
end
...
function pickProcess()
    MRFW.Functions.Progressbar("pick_grape", "Picking Grapes ..", math.random(6000,8000), false, true, {
							...
							if not loadIngredients then
								if #(pos - vector3(Config.Vineyard["wine"].coords.x, Config.Vineyard["wine"].coords.y, Config.Vineyard["wine"].coords.z)) < 1 then
									...
									if IsControlJustPressed(0, 38) then
										-- if PlayerJob.name == "vineyard" then
											TriggerServerEvent("mr-vineyard:server:loadIngredients")
										-- else
										-- 	MRFW.Functions.Notify("I dont think I work here...", "error")
										-- end
									end
								...
								if not finishedWine then
									...
									if #(pos - vector3(Config.Vineyard["wine"].coords.x, Config.Vineyard["wine"].coords.y, Config.Vineyard["wine"].coords.z)) < 1 then
										...
										if IsControlJustPressed(0, 38) then
											-- if PlayerJob.name == "vineyard" then
												TriggerServerEvent("mr-vineyard:server:receiveWine")
												finishedWine = false
...
RegisterNetEvent('mr-vineyard:grapeJuice', function()
    grapemaking = true
	TriggerServerEvent("mr-vineyard:server:grapeJuice")
...
RegisterNetEvent('mr-vineyard:client:grapeJuice')
AddEventHandler('mr-vineyard:client:grapeJuice', function()
	PrepareAnim()
	grapeJuiceProcess()
end)

RegisterNetEvent('mr-vineyard:client:loadIngredients')
AddEventHandler('mr-vineyard:client:loadIngredients', function()
...
function grapeJuiceProcess()
    MRFW.Functions.Progressbar("grape_juice", "Processing Grapes ..", math.random(15000,20000), false, true, {
        disableMovement = true,
        disableCarMovement = true,
        disableMouse = false,
        disableCombat = true,
    }, {}, {}, {}, function() -- Done
        TriggerServerEvent("mr-vineyard:server:receiveGrapeJuice")
        ClearPedTasks(PlayerPedId())
