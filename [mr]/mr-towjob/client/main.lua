...
AddEventHandler('jobs:client:ToggleNpc', function()
    if MRFW.Functions.GetPlayerData().job.name == "tow" then
        if CurrentTow ~= nil then 
            MRFW.Functions.Notify("First Finish Your Work", "error")
            return
        end
        NpcOn = not NpcOn
        if NpcOn then
...
RegisterNetEvent('mr-tow:client:TowVehicle')
AddEventHandler('mr-tow:client:TowVehicle', function()
    local vehicle = GetVehiclePedIsIn(PlayerPedId(), true)
    if isTowVehicle(vehicle) then
        if CurrentTow == nil then 
            --[[ Replaced "MRFW.Functions.GetClosestVehicle()" with custom implementation "getVehicleInDirection"
                 MRFW native could not return polcice and other vehicles types (NPC) ]] 
            ...
            if NpcOn and CurrentLocation ~= nil then
                if GetEntityModel(targetVehicle) ~= GetHashKey(CurrentLocation.model) then
                    MRFW.Functions.Notify("This Is Not The Right Vehicle", "error")
                    return
                end
            end
            if not IsPedInAnyVehicle(PlayerPedId()) then
                if vehicle ~= targetVehicle then
                    ...
                    if #(towPos - targetPos) < 11.0 then
                        MRFW.Functions.Progressbar("towing_vehicle", "Hoisting the Vehicle...", 5000, false, true, {
                            ...
                            if NpcOn then
                                RemoveBlip(CurrentBlip)
                                MRFW.Functions.Notify("Take The Vehicle To Hayes Depot", "success", 5000)
                                ...
                                if chance < 5 then
                                    TriggerServerEvent('mr-tow:server:nano')
                                end
                            end
                            MRFW.Functions.Notify("Vehicle Towed")
                        end, function() -- Cancel
                            StopAnimTask(PlayerPedId(), "mini@repair", "fixing_a_ped", 1.0)
            ...
            MRFW.Functions.Progressbar("untowing_vehicle", "Remove The Vehicle", 5000, false, true, {
                disableMovement = true,
                disableCarMovement = true,
                disableMouse = false,
                disableCombat = true,
                ...
                CurrentTow = nil
                MRFW.Functions.Notify("Vehicle Taken Off")
            end, function() -- Cancel
                StopAnimTask(PlayerPedId(), "mini@repair", "fixing_a_ped", 1.0)
                MRFW.Functions.Notify("Failed", "error")
            end)
        ...
        MRFW.Functions.Notify("You Must Have Been In A Towing Vehicle First", "error")
    end
end)
...
--                                 TriggerServerEvent('mr-tow:server:DoBail', false)
--                             else
--                                 MenuGarage()
...
--                             if JobsDone > 0 then
--                                 RemoveBlip(CurrentBlip)
--                                 TriggerServerEvent("mr-tow:server:11101110", JobsDone)
--                                 JobsDone = 0
--                                 NpcOn = false
--                             else
--                                 MRFW.Functions.Notify("You Havent Done Any Work Yet", "error")
--                             end
--                         end
...
--                         VehicleSpawned = true
--                         MRFW.Functions.SpawnVehicle(CurrentLocation.model, function(veh)
--                             exports['mr-fuel']:SetFuel(veh, 0.0)
...
function deliverVehicle(vehicle)
    ...
    JobsDone = JobsDone + 1
    VehicleSpawned = false
    MRFW.Functions.Notify("You Have Delivered A Vehicle", "success")
    MRFW.Functions.Notify("A New Vehicle Can Be Picked Up")
...
function TakeOutVehicle(vehicleInfo)
    TriggerServerEvent('mr-tow:server:DoBail', true, vehicleInfo)
    selectedVeh = vehicleInfo
end

RegisterNetEvent('mr-tow:client:SpawnVehicle')
AddEventHandler('mr-tow:client:SpawnVehicle', function()
