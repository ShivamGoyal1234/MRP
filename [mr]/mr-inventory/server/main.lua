					...
					if fromItemData.name == 'bag' then
						...
						if toItemData ~= nil then
							...
							if toItemData.name ~= fromItemData.name then
								...
								Player.Functions.AddItem(toItemData.name, toAmount, fromSlot, toItemData.info)
								TriggerEvent("aj-log:server:CreateLog", "stash", "Swapped Item", "orange", "**".. GetPlayerName(src) .. "** (citizenid: *"..Player.PlayerData.citizenid.."* | id: *"..src.."*) swapped item; name: **"..itemInfo["name"].."**, amount: **" .. toAmount .. "** with name: **" .. fromItemData.name .. "**, amount: **" .. fromAmount .. "** - stash: *" .. stashId .. "*",true)
							end
						else
							local itemInfo = MRFW.Shared.Items[fromItemData.name:lower()]
	...
	elseif MRFW.Shared.SplitStr(fromInventory, "-")[1] == "itemshop" then
		...
		elseif MRFW.Shared.SplitStr(shopType, "_")[1] == "Itemshop" then
			if Player.Functions.RemoveMoney("cash", price, "itemshop-bought-item") then
				...
				Player.Functions.AddItem(itemData.name, fromAmount, toSlot, itemData.info)
				TriggerClientEvent('aj-shops:client:UpdateShop', src, MRFW.Shared.SplitStr(shopType, "_")[2], itemData, fromAmount)
				TriggerClientEvent('MRFW:Notify', src, itemInfo["label"] .. " bought!", "success")
				TriggerEvent("aj-log:server:CreateLog", "shops", "Shop item bought", "green", "**"..GetPlayerName(src) .. "** (citizenid: *"..Player.PlayerData.citizenid.."* | *"..src.."*) bought " .. itemInfo["label"] .. "**, amount: **" .. fromAmount .. " for $"..price)
				if Player.PlayerData.job.name == "police" then
					TriggerEvent("aj-log:server:CreateLog", "shopspolice", "Police Armory", "green", "**"..GetPlayerName(src) .. "** (citizenid: *"..Player.PlayerData.citizenid.."* | *"..src.."*) bought " .. itemInfo["label"] .. "**, amount: **" .. fromAmount .. " for $"..price)
				end
				if Player.PlayerData.job.name == "doctor" then
					TriggerEvent("aj-log:server:CreateLog", "shopsdoc", "EMS Shop", "green", "**"..GetPlayerName(src) .. "** (citizenid: *"..Player.PlayerData.citizenid.."* | *"..src.."*) bought " .. itemInfo["label"] .. "**, amount: **" .. fromAmount .. " for $"..price)
				end
				if Player.PlayerData.job.name == "government" then
					TriggerEvent("aj-log:server:CreateLog", "shopsgov", "Government Armory", "green", "**"..GetPlayerName(src) .. "** (citizenid: *"..Player.PlayerData.citizenid.."* | *"..src.."*) bought " .. itemInfo["label"] .. "**, amount: **" .. fromAmount .. " for $"..price)
				end
...
RegisterNetEvent('aj-inventory:server:SaveStashItems', function(stashId, items)
    MySQL.Async.insert('INSERT INTO stashitems (stash, items) VALUES (:stash, :items) ON DUPLICATE KEY UPDATE items = :items', {
        ['stash'] = stashId,
        ['items'] = json.encode(items)
    })
...
MRFW.Functions.CreateCallback('aj-inventory:server:GetStashItems', function(source, cb, stashId)
...
RegisterNetEvent("inventory:server:spawnOnGround", function(source, itemData, itemAmount) 
	...
	TriggerEvent("aj-log:server:CreateLog", "drop", "New Item Spawn as Drop", "red", "**".. GetPlayerName(source) .. "** (citizenid: *"..Ply.PlayerData.citizenid.."* | id: *"..source.."*) overflowed inventory into drop; name: **"..itemData.name.."**, amount: **" .. itemAmount .. "**")
	TriggerClientEvent("inventory:client:AddDropItem", -1, dropId, source, coords)
end)