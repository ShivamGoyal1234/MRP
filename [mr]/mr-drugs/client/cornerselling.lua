...
RegisterNetEvent('mr-drugs:client:cornerselling', function(data)
    MRFW.Functions.TriggerCallback('mr-drugs:server:cornerselling:getAvailableDrugs', function(result)
        if result ~= nil then
            availableDrugs = result
            if not cornerselling then
                cornerselling = true
                LocalPlayer.state:set("inv_busy", true, true)
                MRFW.Functions.Notify('You started selling drugs')
                startLocation = GetEntityCoords(PlayerPedId())
            else
                cornerselling = false
                LocalPlayer.state:set("inv_busy", false, true)
                MRFW.Functions.Notify('You stopped selling drugs')
            end
        else
            MRFW.Functions.Notify('You aren\'t carrying any drugs with you..', 'error')
            LocalPlayer.state:set("inv_busy", false, true)
        end
    end)
end)
...
RegisterNetEvent('mr-drugs:client:refreshAvailableDrugs', function(items)
    availableDrugs = items
    if #availableDrugs <= 0 then
        MRFW.Functions.Notify('No more drugs left to sell!', 'error')
        cornerselling = false
        LocalPlayer.state:set("inv_busy", false, true)
    end
end)
...
local function toFarAway()
    MRFW.Functions.Notify('Moved Too Far!', 'error')
    LocalPlayer.state:set("inv_busy", false, true)
    cornerselling = false
    hasTarget = false
    busySelling = false
...
local function SellToPed(ped)
    ...
    if hasTarget then
        while pedDist < 1.5 and not IsPedDeadOrDying(ped) do
            ...
            pedDist = #(coords - pedCoords)
            if getRobbed == 18 or getRobbed == 9 then
                TriggerServerEvent('mr-drugs:server:robCornerDrugs', availableDrugs[drugType].item, bagAmount)
                MRFW.Functions.Notify('You have been robbed and lost '..bagAmount..' bags(\'s) '..availableDrugs[drugType].label, 'error')
                stealingPed = ped
                stealData = {
                ...
                if pedDist < 1.5 and cornerselling then
                    ...
                    if IsControlJustPressed(0, 38) then
                        TriggerServerEvent('mr-drugs:server:sellCornerDrugs', availableDrugs[drugType].item, bagAmount, randomPrice)
                        hasTarget = false
                    ...
                    if IsControlJustPressed(0, 47) then
                        MRFW.Functions.Notify('Offer canceled!', 'error')
                        hasTarget = false
            ...
            if IsEntityDead(stealingPed) then
                ...
                if #(pos - pedpos) < 1.5 then
                    ...
                    if IsControlJustReleased(0, 38) then
                        ...
                        Wait(2000)
                        ClearPedTasks(ped)
                        TriggerServerEvent("MRFW:Server:AddItem", stealData.item, stealData.amount)
                        TriggerEvent('inventory:client:ItemBox', MRFW.Shared.Items[stealData.item], "add")
                        stealingPed = nil
                        stealData = {}
                    end
                end
