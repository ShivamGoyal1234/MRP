...
RegisterNetEvent('MRFW:Client:OnPlayerLoaded', function()
    MRFW.Functions.TriggerCallback('mr-drugs:server:RequestConfig', function(DealerConfig)
...
--                                         MRFW.Functions.Progressbar("hospital_revive", "Put person back on their feet..", 5000, false, true, {
...
--                                             MRFW.Functions.Notify("You have helped a person!")
...
--                                         end)
--                                     else
--                                         MRFW.Functions.Notify("There is no one nearby..", "error")
--                                     end
--                                 else
...
function buyDealerStuff()
    ...
    for k, v in pairs(Config.Dealers[currentDealer]["products"]) do
        if MRFW.Functions.GetPlayerData().metadata["dealerrep"] >= Config.Dealers[currentDealer]["products"][k].minrep then
...
function knockDoorAnim(home)
    ...
    local myData = MRFW.Functions.GetPlayerData()
    ...
    if home then
        ...
        knockingDoor = false
        Citizen.Wait(1000)
        MRFW.Functions.Notify('It looks like no one is home..', 'error', 3500)
    end
end

RegisterNetEvent('mr-drugs:client:updateDealerItems')
AddEventHandler('mr-drugs:client:updateDealerItems', function(itemData, amount)
    TriggerServerEvent('mr-drugs:server:updateDealerItems', itemData, amount, currentDealer)
end)

RegisterNetEvent('mr-drugs:client:setDealerItems')
AddEventHandler('mr-drugs:client:setDealerItems', function(itemData, amount, dealer)
    Config.Dealers[dealer]["products"][itemData.slot].amount = Config.Dealers[dealer]["products"][itemData.slot].amount - amount
end)

function requestDelivery()
    while Config.DeliveryLocations == nil do Citizen.Wait(100) end
    ...
    TriggerServerEvent('mr-drugs:server:giveDeliveryItems', amount)
    SetTimeout(2000, function()
        TriggerServerEvent('mr-phone:server:sendNewMail', {
            sender = Config.Dealers[currentDealer]["name"],
            subject = "Delivery Location",
            message = "Here is all info about the delivery, <br>Items: <br> "..amount.."x "..MRFW.Shared.Items[waitingDelivery["itemData"]["item"]]["label"].."<br><br> be in time!!",
            button = {
                enabled = true,
                buttonEvent = "mr-drugs:client:setLocation",
...
function randomDeliveryItemOnRep()
    local ped = PlayerPedId()
    local myRep = MRFW.Functions.GetPlayerData().metadata["dealerrep"]
...
function setMapBlip(x, y)
    SetNewWaypoint(x, y)
    MRFW.Functions.Notify('The route to the delivery location has been set on your map.', 'success');
end

RegisterNetEvent('mr-drugs:client:setLocation')
AddEventHandler('mr-drugs:client:setLocation', function(locationData)
    if activeDelivery == nil then
        activeDelivery = locationData
    else
        setMapBlip(activeDelivery["coords"]["x"], activeDelivery["coords"]["y"])
        MRFW.Functions.Notify('You still have an open delivery...')
        return
    end
        ...
        while true do
            ...
            if activeDelivery ~= nil then
                ...
                if dist < 15 then
                    ...
                    if dist < 1.5 then
                        DrawText3D(activeDelivery["coords"]["x"], activeDelivery["coords"]["y"], activeDelivery["coords"]["z"], '[E] '..activeDelivery["amount"]..'x '..MRFW.Shared.Items[activeDelivery["itemData"]["item"]]["label"]..' deliver.')
...
function deliverStuff(activeDelivery)
    if deliveryTimeout > 0 then
        ...
        Citizen.Wait(500)
        checkPedDistance()
        MRFW.Functions.Progressbar("work_dropbox", "Delivering products..", 3500, false, true, {
            ...
            TriggerEvent('dpemote:custom:animation', {"c"})
            TriggerServerEvent('mr-drugs:server:succesDelivery', activeDelivery, true)
        end, function() -- Cancel
            ClearPedTasks(PlayerPedId())
            MRFW.Functions.Notify("Canceled..", "error")
        end)
    else
        TriggerServerEvent('mr-drugs:server:succesDelivery', activeDelivery, false)
    end
    deliveryTimeout = 0
end
...
function doPoliceAlert()
    ...
    TriggerServerEvent('mr-drugs:server:callCops', streetLabel, pos)
end

RegisterNetEvent('mr-drugs:client:robberyCall')
AddEventHandler('mr-drugs:client:robberyCall', function(msg, streetLabel, coords)
...
RegisterNetEvent('mr-drugs:client:sendDeliveryMail')
AddEventHandler('mr-drugs:client:sendDeliveryMail', function(type, deliveryData)
...
RegisterNetEvent('mr-drugs:client:CreateDealer')
AddEventHandler('mr-drugs:client:CreateDealer', function(dealerName, minTime, maxTime)
    local ped = PlayerPedId()
    ...
    TriggerServerEvent('mr-drugs:server:CreateDealer', DealerData)
end)

RegisterNetEvent('mr-drugs:client:RefreshDealers')
AddEventHandler('mr-drugs:client:RefreshDealers', function(DealerData)
    Config.Dealers = DealerData
end)

RegisterNetEvent('mr-drugs:client:GotoDealer')
AddEventHandler('mr-drugs:client:GotoDealer', function(DealerData)
    local ped = PlayerPedId()

    SetEntityCoords(ped, DealerData["coords"]["x"], DealerData["coords"]["y"], DealerData["coords"]["z"])
    MRFW.Functions.Notify('You have been teleported : '.. DealerData["name"] .. ' Good luck!', 'success')
end)
