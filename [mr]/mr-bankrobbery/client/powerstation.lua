        ...
        if dist < 1.5 then
            if CurrentCops >= Config.MinimumThermitePolice then
                if not Config.PowerStations[closestStation].hit then
                    ...
                    MRFW.Functions.Notify("It seems that the fuses have blown.", "error")
                end
            else
                MRFW.Functions.Notify('Minimum Of '..Config.MinimumThermitePolice..' Police Needed', "error")
            end
        end
...
RegisterNetEvent('mr-bankrobbery:client:SetStationStatus')
AddEventHandler('mr-bankrobbery:client:SetStationStatus', function(key, isHit)
...
RegisterNUICallback('thermitefailed', function()
    MRFW.Functions.TriggerCallback("thermite:server:check", function(success)
        if success then
...
RegisterNUICallback('thermitesuccess', function()
    MRFW.Functions.TriggerCallback("thermite:server:check", function(success)
        if success then
            ...
            local coords = GetEntityCoords(PlayerPedId())
            while time > 0 do 
                MRFW.Functions.Notify("Thermite is going off in " .. time .. "..")
                Citizen.Wait(1000)
                time = time - 1
            end
            local randTime = math.random(10000, 15000)
            ...
            if currentStation ~= 0 then
                MRFW.Functions.Notify("The fuses are broken", "success")
                TriggerServerEvent("mr-bankrobbery:server:SetStationStatus", currentStation, true)
            elseif currentGate ~= 0 then
                MRFW.Functions.Notify("The door is open", "success")
                TriggerServerEvent('nui_doorlock:server:updateState', currentGate, false, nil, false, true, false)
                currentGate = 0
            end
        end
    end)
