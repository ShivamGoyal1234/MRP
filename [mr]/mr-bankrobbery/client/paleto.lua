...
Citizen.CreateThread(function()
    Citizen.Wait(2000)
    local requiredItems = {
        [1] = {name = MRFW.Shared.Items["security_card_01"]["name"], image = MRFW.Shared.Items["security_card_01"]["image"]},
    }
                                    ...
                                    if lockerDist < 0.5 then
                                        ...
                                        if IsControlJustPressed(0, 38) then
                                            if CurrentCops >= Config.MinimumPaletoPolice then
                                                ...
                                                MRFW.Functions.Notify('Minimum Of '..Config.MinimumPaletoPolice..' Police Needed', "error")
                                            end
                                        end
                                    end
                                end
...
RegisterNetEvent('mr-bankrobbery:UseBankcardA')
AddEventHandler('mr-bankrobbery:UseBankcardA', function()
    local ped = PlayerPedId()
    local pos = GetEntityCoords(ped)
    local dist = #(pos - Config.BigBanks["paleto"]["coords"])
        ...
        MRFW.Functions.TriggerCallback('mr-bankrobbery:server:isRobberyActive', function(isBusy)
            if not isBusy then
                if CurrentCops >= Config.MinimumPaletoPolice then
                    if not Config.BigBanks["paleto"]["isOpened"] then 
                        TriggerEvent('inventory:client:requiredItems', requiredItems, false)
                        MRFW.Functions.Progressbar("security_pass", "Validitating card..", math.random(5000, 10000), false, true, {
                            ...
                            TriggerServerEvent('mr-bankrobbery:server:setBankState', "paleto", true)
                            TriggerServerEvent("MRFW:Server:RemoveItem", "security_card_01", 1)
                            ...
                            if not copsCalled then
                                ...
                                if Config.BigBanks["paleto"]["alarm"] then
                                    TriggerServerEvent("mr-bankrobbery:server:callCops", "paleto", 0, streetLabel, pos)
                                    copsCalled = true
                                end
                            end
                        end, function() -- Cancel
                        ...
                        MRFW.Functions.Notify("It looks like the bank is already opened..", "error")
                    end
                else
                    MRFW.Functions.Notify('Minimum Of '..Config.MinimumPaletoPolice..' Police Needed', "error")
                end
            else
                MRFW.Functions.Notify("The security lock is active, the door cannot be opened at the moment..", "error", 5500)
            end
        end)
    end 
end)
...
function OpenPaletoDoor()
    TriggerServerEvent('mr-doorlock:server:updateState', 85, false)
