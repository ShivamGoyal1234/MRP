...
CreateThread(function()
    while true do
        ...
        if NetCheck1 or NetCheck2 then
            TriggerServerEvent('mr-admin:server:GetPlayersForBlips')
        end
    end
end)

RegisterNetEvent('mr-admin:client:toggleBlips', function()
    if not ShowBlips then
        ShowBlips = true
        NetCheck1 = true
        MRFW.Functions.Notify("Blips activated", "success")
    else
        ShowBlips = false
        MRFW.Functions.Notify("Blips deactivated", "error")
    end
end)

RegisterNetEvent('mr-admin:client:toggleNames', function()
    if not ShowNames then
        ShowNames = true
        NetCheck2 = true
        MRFW.Functions.Notify("Names activated", "success")
    else
        ShowNames = false
        MRFW.Functions.Notify("Names deactivated", "error")
    end
end)

RegisterNetEvent('mr-admin:client:Show', function(players)
