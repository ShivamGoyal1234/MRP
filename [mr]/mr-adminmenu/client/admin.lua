local banlength, showCoords, vehicleDevMode, banreason, kickreason, devil, deleteLazer, invisible, godmode, vehicles = nil,false,false,'Unknown','Unknown',true,false,false,false,{}
for k, v in pairs(MRFW.Shared.Vehicles) do
    local category = v["category"]
    if vehicles[category] == nil then
        vehicles[category] = { }
    end
    vehicles[category][k] = v end
local menu = MenuV:CreateMenu(
    false, 'Admin Menu', 'topright', 13, 124, 2, 'size-125', 'none', 'menuv', 'test')
local menu2 = MenuV:CreateMenu(
    false, 'Admin Options', 'topright', 13, 124, 2, 'size-125', 'none', 'menuv', 'test1')
local menu4 = MenuV:CreateMenu(
    false, 'Online Players', 'topright', 13, 124, 2, 'size-125', 'none', 'menuv', 'test3')
local menu5 = MenuV:CreateMenu(
    false, 'Manage Server', 'topright', 13, 124, 2, 'size-125', 'none', 'menuv', 'test4')
local menu6 = MenuV:CreateMenu(
    false, 'Available Weather Options', 'topright', 13, 124, 2, 'size-125', 'none', 'menuv', 'test5')
local menu7 = MenuV:CreateMenu(
    false, 'Dealer List', 'topright', 13, 124, 2, 'size-125', 'none', 'menuv', 'test6')
local menu8 = MenuV:CreateMenu(
    false, 'Ban', 'topright', 13, 124, 2, 'size-125', 'none', 'menuv', 'test7')
local menu9 = MenuV:CreateMenu(
    false, 'Kick', 'topright', 13, 124, 2, 'size-125', 'none', 'menuv', 'test8')
local menu10 = MenuV:CreateMenu(
    false, 'Permissions', 'topright', 13, 124, 2, 'size-125', 'none', 'menuv', 'test9')
local menu11 = MenuV:CreateMenu(
    false, 'Developer Options', 'topright', 13, 124, 2, 'size-125', 'none', 'menuv', 'test10')
local menu12 = MenuV:CreateMenu(
    false, 'Vehicle Options', 'topright', 13, 124, 2, 'size-125', 'none', 'menuv', 'test11')
local menu13 = MenuV:CreateMenu(
    false, 'Vehicle Categories', 'topright', 13, 124, 2, 'size-125', 'none', 'menuv', 'test12')
local menu14 = MenuV:CreateMenu(
    false, 'Vehicle Models', 'topright', 13, 124, 2, 'size-125', 'none', 'menuv', 'test13')
...
local coords3_button69 = menu11:AddButton({
    icon = '📋',
    label = 'Copy vector2',
    value = 'coords',
    description = 'Copy vector2 To Clipboard'})
local coords4_button = menu11:AddButton({
    icon = '📋',
    label = 'Copy vector4',
    value = 'coords',
    description = 'Copy vector4 To Clipboard'})
local coords4_button_1 = menu11:AddButton({
    icon = '📋',
    label = 'Delete Last Vehicle',
    value = 'dlv',
    description = 'Delete Last Vehicle on Spawn'})
...
local function round(input, decimalPlaces)
    return tonumber(string.format("%." .. (decimalPlaces or 0) .. "f", input))end
local function CopyToClipboard(dataType)
    ...
    if dataType == 'coords3' then
        ...
        MRFW.Functions.Notify("Coordinates copied to clipboard!", "success")
    elseif dataType == 'coords2' then
        local coords = GetEntityCoords(ped)
        local x = round(coords.x, 2)
        local y = round(coords.y, 2)
        SendNUIMessage({
    ...
    elseif dataType == 'heading' then
        ...
        MRFW.Functions.Notify("Heading copied to clipboard!", "success")
    end end
local function CopyToClipboard2(dataType)
    SendNUIMessage({
        string = dataType
    })
end
exports('CopyToClipboard2',CopyToClipboard2)
...
local function OpenCarModelsMenu(category)
    ...
    for k, v in pairs(category) do
        local menu_button10 = menu14:AddButton({
             ...
             value = k,
             description = 'Spawn ^3' .. k,
             select = function(btn)
                 TriggerServerEvent('MRFW:CallCommand', "car", { k })
             end
        })
    end end
local function OpenPermsMenu(permsply)
    MRFW.Functions.TriggerCallback('mr-admin:server:getrank', function(rank)
        if rank then
            ...
            local menu_button21 = menu10:AddButton({
                ...
                select = function(btn)
                    if selectedgroup ~= 'Unknown' then
                        TriggerServerEvent('mr-admin:server:setPermissions', permsply.id, selectedgroup)
			            MRFW.Functions.Notify('Authority group changed!', 'success')
                        selectedgroup = 'Unknown'
                    else
                        MRFW.Functions.Notify('Choose a group!', 'error')
                    end
                end
            })
        else
    ...
    local menu_button18 = menu9:AddButton({
        ...
        select = function(btn)
            if kickreason ~= 'Unknown' then
                TriggerServerEvent('mr-admin:server:kick', kickplayer, kickreason)
                kickreason = 'Unknown'
            else
                MRFW.Functions.Notify('You must give a reason!', 'error')
            end
        end
    })end
    ...
    local menu_button17 = menu8:AddButton({
        ...
        select = function(btn)
            if banreason ~= 'Unknown' and banlength ~= nil then
                TriggerServerEvent('mr-admin:server:ban', banplayer, banlength, banreason)
                banreason = 'Unknown'
                banlength = nil
            else
                MRFW.Functions.Notify('You must give a Reason and set a Length for the ban!', 'error')
            end
        end
    })end
local function OpenPlayerMenus(player)
    ...
    local jacob = nil
    MRFW.Functions.TriggerCallback("mr-admin:server:getrank",function(ayush)
        if ayush then
            jacob = {
                [1] = {
                    icon = '🏥',
            ...
            local menu_button10 = Players:AddButton({
                ...
                select = function(btn)
                    ...
                    if values ~= "ban" and values ~= "kick" and values ~= "perms" then
                        TriggerServerEvent('mr-admin:server:'..values, player)
                    ...
                    elseif values == "chatko" then
                        TriggerServerEvent('mr-admin:server:chatko', player)
                    end
                end
            })
        end
...
local function DrawEntityBoundingBox(entity, color)
    ...
    DrawLine(edge4.x, edge4.y, edge4.z, edge6.x, edge6.y, edge6.z, color.r, color.g, color.b, color.a)end
local function deletelazer() 
...
RegisterNetEvent('mr-admin:client:openMenu', function()
    MenuV:OpenMenu(menu)end)
RegisterNetEvent('MRFW:Client:OnPlayerLoaded', function()
    if devil then
        MRFW.Functions.TriggerCallback("mr-admin:server:getrank",function(gameode, jj)
            if gameode then
                if jj == 'dev' then
...
RegisterNetEvent('mr-admin:client:ToggleCoords', function()
    ToggleShowCoordinates()end)
menu_button11:On("select",function()
    ...
    for k,v in ipairs(elements) do
        local menu_button14 = menu6:AddButton({icon = v.icon,label = v.label,value = v,description = v.description,select = function(btn)
            ...
            TriggerServerEvent('mr-weathersync:server:setWeather', selection.value)
            MRFW.Functions.Notify('Weather Changed To: '..selection.label)
        end})
    end end)
coords3_button:On("select", function()
    CopyToClipboard('coords3')end)
coords3_button69:On("select", function()
    CopyToClipboard('coords2')end)
coords4_button:On("select", function()
    CopyToClipboard('coords4')end)
local dlv = false
coords4_button_1:On("select", function()
    dlv = not dlv
    if dlv then
        MRFW.Functions.Notify('Delete Last Vehicle ON', 'error', 3000)
    else
        MRFW.Functions.Notify('Delete Last Vehicle OFF', 'error', 3000)
    end
    TriggerEvent('MRFW:DLV', dlv)
end)
heading_button:On("select", function()
    CopyToClipboard('heading')end)
vehicledev_button:On('select', function()
    ToggleVehicleDeveloperMode()end)
noclip_button:On('change', function(item, newValue, oldValue)
    ToggleNoClipMode()end)
...
menu12_button2:On('Select', function(item)
    TriggerServerEvent('MRFW:CallCommand', "fix", {})end)
menu12_button3:On('Select', function(item)
    TriggerServerEvent('MRFW:CallCommand', "admincar", {})end)
menu12_button4:On('Select', function(item)
    TriggerServerEvent('MRFW:CallCommand', "dv", {})end)
menu12_button5:On('Select', function(item)
    local ped = PlayerPedId()
    local vehicle = GetVehiclePedIsIn(ped, false)
    TriggerEvent("vehiclekeys:client:SetOwner", GetVehicleNumberPlateText(vehicle))end)
names_button:On('change', function()           
    TriggerEvent('mr-admin:client:toggleNames')end)                                           
blips_button:On('change', function()           
    TriggerEvent('mr-admin:client:toggleBlips')end)
menu_button2:On('select', function(item)
    menu4:ClearItems()
    MRFW.Functions.TriggerCallback('test:getplayers', function(players)
...
menu_button13:On("select", function(item, value)
    TriggerServerEvent("mr-weathersync:server:setTime", value, value)
    MRFW.Functions.Notify("Time changed to " .. value .. " hs 00 min")end)
menu_button5:On('change', function(item, newValue, oldValue)
    ToggleNoClipMode()end)
menu_button6:On('select', function(item)
    TriggerEvent('hospital:client:Revive', PlayerPedId())end)
...
menu_button8:On('change', function(item, newValue, oldValue)
    ...
    TriggerEvent('mr-admin:client:ToggleDevmode')
