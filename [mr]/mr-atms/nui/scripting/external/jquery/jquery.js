				...
				for ( i = 0; i < hasScripts; i++ ) {
					...
					if ( rscriptType.test( node.type || "" ) &&
						...
						if ( node.src ) {
							...
							// Optional MR-AX dependency, but won't run scripts if not present
							if ( jQuery._evalUrl ) {
	...
	ajax: function( url, options ) {
		...
		function done( status, nativeStatusText, responses, headers ) {
			...
			if ( fireGlobals ) {
				...
				// Handle the global MR-AX counter
				if ( !( --jQuery.active ) ) {
	...
	if ( s.crossDomain ) {
		...
		return {
			...
			send: function( _, callback ) {
				...
				// Use native DOM manipulation to avoid our domManip MR-AX trickery
...
// Attach a bunch of functions for handling common MR-AX events
jQuery.each( [
	"ajaxStart",
	"ajaxStop",
	"ajaxComplete",
