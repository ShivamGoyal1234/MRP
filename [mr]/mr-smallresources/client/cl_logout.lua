...
CreateThread(function()
    while true do
        if LocalPlayer.state['isLoggedIn'] then
            ...
            for k, v in pairs(Config.logout["logout"]) do
                if #(pos - vector3(v.x, v.y, v.z)) < 15 then
                    in_range = true
                    if #(pos - vector3(v.x, v.y, v.z)) < 0.35 then
                        DrawText3Ds(v.x, v.y, v.z, "~g~[E]~w~ Logout")
                        if IsControlJustPressed(0, 38) then -- E
                            TriggerServerEvent('mr-houses:server:LogoutLocation')
                        end
                    elseif #(pos - vector3(v.x, v.y, v.z)) < 5 then
                        DrawText3Ds(v.x, v.y, v.z, "~r~[E]~w~ Logout")
                    elseif #(pos - vector3(v.x, v.y, v.z)) < 7 then
                        DrawText3Ds(v.x, v.y, v.z, "Logout")
                    end
                end
            end
...
-- 				TriggerServerEvent('mr-stations:server:LogoutLocation')
-- 			end			
-- 		end
-- 	end
-- end)
