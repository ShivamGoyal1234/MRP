...
local function DoLimbAlert()
    if not isDead and not InLaststand then
        if #injured > 0 then
            ...
            MRFW.Functions.Notify(limbDamageMsg, "primary", 5000)
        end
    end
end
...
local function ResetAll()
    ...
    TriggerServerEvent("MRFW:Server:SetMetaData", "hunger", 100)
    TriggerServerEvent("MRFW:Server:SetMetaData", "thirst", 100)
end

local function ResetAllcheckin()
    isBleeding = 0
    bleedTickTimer = 0
    advanceBleedTimer = 0
    fadeOutTimer = 0
    ...
    MRFW.Functions.GetPlayerData(function(PlayerData)
        if PlayerData.metadata["hunger"] < 10 then
            TriggerServerEvent("MRFW:Server:SetMetaData", "hunger", 20)
        end
    end)

   MRFW.Functions.GetPlayerData(function(PlayerData)
        if PlayerData.metadata["thirst"] < 10 then
            TriggerServerEvent("MRFW:Server:SetMetaData", "thirst", 20)
        end
    end)
end
...
RegisterNetEvent('hospital:client:ambulanceAlert', function(coords, text)
    ...
    MRFW.Functions.Notify({text = text, caption = street1name.. ' ' ..street2name}, 'ambulance')
...
RegisterNetEvent('hospital:client:Revive', function()
    ...
    MRFW.Functions.Notify("You are completely healthy again!")
end)

RegisterNetEvent('hospital:client:HalfRevive', function()
    local player = PlayerPedId()
    ...
    MRFW.Functions.Notify("You're Healed!")

end)

RegisterNetEvent('hospital:client:Revivecheckin', function()
    local player = PlayerPedId()
    ...
    --TriggerServerEvent('mr-hud:server:RelieveStress', 100)
    TriggerServerEvent("hospital:server:SetDeathStatus", false)
    TriggerServerEvent("hospital:server:SetLaststandStatus", false)

    MRFW.Functions.Notify("You're back on top!")
end)
...
RegisterNetEvent('hospital:client:HealInjuries', function(type)
    ...
    TriggerServerEvent("hospital:server:RestoreWeaponDamage")
    MRFW.Functions.Notify("Your wounds have been healed!")
end)
...
RegisterNetEvent('hospital:client:SendToBed', function(id, data, isRevive)
    ...
    CreateThread(function ()
        Wait(5)
        if isRevive then
            MRFW.Functions.Notify("You are being helped..")
...
RegisterNetEvent('hospital:client:SendToBed2', function(id, data)
    bedOccupying = id
    bedOccupyingData = data
    SetBedCam()
    CreateThread(function ()
        Wait(5)
        local player = PlayerPedId()
        MRFW.Functions.Notify("Wait For the Doctor..")
        Wait(Config.AIHealTimer * 1000)
        --TriggerEvent("hospital:client:Revive")
        canLeaveBed = true   
    end)
...
RegisterNetEvent('hospital:client:SendToBedcheckin', function(id, data, isRevive)
    bedOccupying = id
    bedOccupyingData = data
    SetBedCam()
    CreateThread(function ()
        ...
        local player = PlayerPedId()
        if isRevive then
            MRFW.Functions.Notify("You'll be helped..")
            Wait(Config.AIHealTimer * 1000)
...
RegisterNetEvent('hospital:client:Sendcheckemail', function()
    SetTimeout(math.random(2500, 4000), function()
        local gender = "Mr."
        if MRFW.Functions.GetPlayerData().charinfo.gender == 1 then
            gender = "Mrs"
...
RegisterNetEvent('hospital:checkin', function(data)
    if onDutyasDoc then
        TriggerServerEvent("hospital:server:SendDoctorAlert")
        TriggerEvent('dpemote:custom:animation', {"notepad"})
        MRFW.Functions.Progressbar("hospital_checkin", "Calling Doctors..", 2000, false, true, {
            ...
            if bedId ~= nil then
                ...
                MRFW.Functions.Notify("Beds occupied..", "error")
            end
        end, function() -- Cancel
            TriggerEvent('dpemote:custom:animation', {"c"})
            MRFW.Functions.Notify("Not checked in!", "error")
        end)
    else
        TriggerEvent('dpemote:custom:animation', {"notepad"})
        MRFW.Functions.Progressbar("hospital_checkin", "Checking in..", 2000, false, true, {
            disableMovement = true,
            disableCarMovement = true,
            disableMouse = false,
            disableCombat = true,
            ...
            if bedId then
                ...
                MRFW.Functions.Notify("Beds are occupied..", "error")
            end
        end, function() -- Cancel
            TriggerEvent('dpemote:custom:animation', {"c"})
            MRFW.Functions.Notify("Checking in failed!", "error")
        end)
    end
end)
