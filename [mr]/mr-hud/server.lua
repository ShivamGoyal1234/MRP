...
MRFW.Commands.Add('cash', 'Check Cash Balance', {}, false, function(source, args)
    local Player = MRFW.Functions.GetPlayer(source)
    local cashamount = Player.PlayerData.money.cash
    TriggerClientEvent('hud:client:ShowAccounts', source, 'cash', cashamount)
end)

MRFW.Commands.Add('bank', 'Check Bank Balance', {}, false, function(source, args)
    local Player = MRFW.Functions.GetPlayer(source)
    local bankamount = Player.PlayerData.money.bank
    TriggerClientEvent('hud:client:ShowAccounts', source, 'bank', bankamount)
end)

MRFW.Commands.Add("dev", "Enable/Disable developer Mode", {}, false, function(source, args)
    TriggerClientEvent("mr-admin:client:ToggleDevmode", source)
end, 'admin')
...
RegisterNetEvent('hud:server:GainStress', function(amount)
    ...
    TriggerClientEvent('MRFW:Notify', src, Lang:t("notify.stress_gain"), 'error', 1500)
end)
...
RegisterNetEvent('hud:server:RelieveStress', function(amount)
    ...
    Player.Functions.SetMetaData('stress', newStress)
    TriggerClientEvent('hud:client:UpdateStress', src, newStress)
    TriggerClientEvent('MRFW:Notify', src, Lang:t("notify.stress_removed"))
end)

RegisterNetEvent('hud:server:saveUIData', function(data)
    local src = source
	-- Check Permissions
    if not MRFW.Functions.HasPermission(src, 'admin') and not IsPlayerAceAllowed(src, 'command') then
		return
	end
...
MRFW.Functions.CreateCallback('hud:server:getMenu', function(source, cb)
    cb(Config.Menu)
end) 

MRFW.Functions.CreateCallback('hud:server:getRank', function(source, cb)
    local src = source
    if MRFW.Functions.HasPermission(src, 'admin') or IsPlayerAceAllowed(src, 'command') then
        cb(true)
    else
        cb(false)
    end
