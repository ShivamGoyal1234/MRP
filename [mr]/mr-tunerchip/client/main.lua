...
RegisterNUICallback('save', function(data)
    MRFW.Functions.TriggerCallback('mr-tunerchip:server:HasChip', function(HasChip)
        if HasChip then
            local ped = PlayerPedId()
            ...
            TriggerServerEvent('mr-tunerchip:server:TuneStatus', MRFW.Functions.GetPlate(veh), true)
        end
    end)
end)

RegisterNetEvent('mr-tunerchip:client:TuneStatus')
AddEventHandler('mr-tunerchip:client:TuneStatus', function()
    local ped = PlayerPedId()
    ...
    if vehModel ~= 0 then
        MRFW.Functions.TriggerCallback('mr-tunerchip:server:GetStatus', function(status)
            if status then
...
RegisterNetEvent('mr-tunerchip:client:checkvehicle')
AddEventHandler('mr-tunerchip:client:checkvehicle', function(slot, info)
    local ped = PlayerPedId()
    if IsPedInAnyVehicle(ped, false) then
        TriggerServerEvent('mr-tunerchip:server:checkvehicle', slot, info)
    else
...
RegisterNetEvent('mr-tunerchip:client:openChip')
AddEventHandler('mr-tunerchip:client:openChip', function(item)
    local ped = PlayerPedId()
