...
AddEventHandler('MRFW:Client:OnPlayerLoaded', function()
    MRFW.Functions.TriggerCallback('nitrous:GetNosLoadedVehs', function(vehs)
        VehicleNitrous = vehs
    end)
end)
    ...
    if not NitrousActivated then
        if IsInVehicle and not IsThisModelABike(GetEntityModel(GetVehiclePedIsIn(ped))) then
            if GetPedInVehicleSeat(veh, -1) == ped then
                MRFW.Functions.Progressbar("use_nos", "Connecting NOS...", math.random(5000, 10000), false, true, {
                    disableMovement = false,
                    disableCarMovement = false,
                    disableMouse = false,
                    disableCombat = true,
                }, {}, {}, {}, function() -- Done
                    TriggerEvent("inventory:client:ItemBox", MRFW.Shared.Items['nitrous'], "remove")
                    TriggerServerEvent("MRFW:Server:RemoveItem", 'nitrous', 1)
                    local CurrentVehicle = GetVehiclePedIsIn(PlayerPedId())
                    local Plate = trim(GetVehicleNumberPlateText(CurrentVehicle))
                    TriggerServerEvent('nitrous:server:LoadNitrous', Plate)
                end)
            else
                MRFW.Functions.Notify("You cannot do that from this seat!", "error")
            end
        else
            MRFW.Functions.Notify('You\'re Not In A Car', 'error')
        end
    else
        MRFW.Functions.Notify('You Already Have NOS Active', 'error')
    end
end)
                        ...
                        Citizen.CreateThread(function()
                            while NitrousActivated do
                                if VehicleNitrous[Plate].level - 1 ~= 0 then
                                    TriggerServerEvent('nitrous:server:UpdateNitroLevel', Plate, (VehicleNitrous[Plate].level - 0.2))
