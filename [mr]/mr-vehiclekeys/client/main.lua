    ...
    while true do
        ...
        if LocalPlayer.state.isLoggedIn then
            ...
            if entering ~= 0 and not isBlacklistedVehicle(entering) then
                sleep = 2000
                local plate = MRFW.Functions.GetPlate(entering)
                ...
                if driver ~= 0 and not IsPedAPlayer(driver) and not HasKeys(plate) and not carIsImmune then
                    if IsEntityDead(driver) then
                        if not isTakingKeys then
                            ...
                            MRFW.Functions.Progressbar("steal_keys", "Taking keys from body...", 2500, false, false, {
                                disableMovement = false,
                                disableCarMovement = true,
                                disableMouse = false,
                                disableCombat = true
                            }, {}, {}, {}, function() -- Done
                                TriggerServerEvent('mr-vehiclekeys:server:AcquireVehicleKeys', plate)
                                isTakingKeys = false
                            end, function()
                                isTakingKeys = false
                            end)
...
AddEventHandler('onResourceStart', function(resourceName)
	if resourceName == GetCurrentResourceName() and MRFW.Functions.GetPlayerData() ~= {} then
		GetKeys()
	end
end)
...
RegisterNetEvent('mr-vehiclekeys:client:AddKeys', function(plate)
    KeysList[plate] = true

    local ped = PlayerPedId()
    if IsPedInAnyVehicle(ped, false) then
        local vehicle = GetVehiclePedIsIn(ped)
        local vehicleplate = MRFW.Functions.GetPlate(vehicle)
...
RegisterNetEvent('mr-vehiclekeys:client:RemoveKeys', function(plate)
    KeysList[plate] = nil
end)

RegisterNetEvent('mr-vehiclekeys:client:ToggleEngine', function()
    local EngineOn = GetIsVehicleEngineRunning(GetVehiclePedIsIn(PlayerPedId()))
    local vehicle = GetVehiclePedIsIn(PlayerPedId(), true)
    if HasKeys(MRFW.Functions.GetPlate(vehicle)) then
        if EngineOn then
            SetVehicleEngineOn(vehicle, false, false, true)
        else
...
RegisterNetEvent('mr-vehiclekeys:client:GiveKeys', function(id)
    local targetVehicle = GetVehicle()

    if targetVehicle then
        local targetPlate = MRFW.Functions.GetPlate(targetVehicle)
            ...
            if id ~= nil then -- Give keys to specific ID
                ...
                if IsPedSittingInVehicle(PlayerPedId(), targetVehicle) then -- Give keys to everyone in vehicle
                    ...
                    for p=1,#otherOccupants do
                        TriggerServerEvent('mr-vehiclekeys:server:GiveVehicleKeys', GetPlayerServerId(NetworkGetPlayerIndexFromPed(otherOccupants[p])), targetPlate)
                    end
                else -- Give keys to closest player
                    GiveKeys(GetPlayerServerId(MRFW.Functions.GetClosestPlayer()), targetPlate)
                end
            end
        else
            MRFW.Functions.Notify("You don't have keys to this vehicle.", 'error')
        end
    end
end)
...
function GiveKeys(id, plate)
    ...
    if distance < 1.5 and distance > 0.0 then
        TriggerServerEvent('mr-vehiclekeys:server:GiveVehicleKeys', id, plate)
    else
        MRFW.Functions.Notify('There is nobody nearby to hand keys to.','error')
    end
end

function GetKeys()
    MRFW.Functions.TriggerCallback('mr-vehiclekeys:server:GetVehicleKeys', function(keysList)
        KeysList = keysList
    end)
end
...
function ToggleVehicleLocks(veh)
    if veh then
        if not isBlacklistedVehicle(veh) then
            if HasKeys(MRFW.Functions.GetPlate(veh)) then
                local ped = PlayerPedId()
                ...
                if vehLockStatus == 1 then
                    ...
                    MRFW.Functions.Notify("Vehicle locked!", "primary")
                else
                    TriggerServerEvent("InteractSound_SV:PlayWithinDistance", 5, "unlock", 0.3)
                    SetVehicleDoorsLocked(veh, 1)
                    MRFW.Functions.Notify("Vehicle unlocked!", "success")
                end
...
function LockpickDoor(isAdvanced)
    ...
    if HasKeys(MRFW.Functions.GetPlate(vehicle)) then return end
...
function lockpickFinish(success)
    ...
    if success then
        ...
        if GetPedInVehicleSeat(vehicle, -1) == PlayerPedId() then
            TriggerServerEvent('mr-vehiclekeys:server:AcquireVehicleKeys', MRFW.Functions.GetPlate(vehicle))
        else
            MRFW.Functions.Notify('You managed to pick the door lock open!', 'success')
...
function Hotwire(vehicle, plate)
    ...
    MRFW.Functions.Progressbar("hotwire_vehicle", "Searching for the car keys...", hotwireTime, false, true, {
        ...
        if (math.random() <= Config.HotwireChance) then
            ...
            MRFW.Functions.Notify("You fail to find the keys and get frustrated.", "error")
        end
        IsHotwiring = false
    end, function() -- Cancel
        StopAnimTask(ped, "anim@amb@clubhouse@tutorial@bkr_tut_ig3@", "machinic_loop_mechandplayer", 1.0)
...
function CarjackVehicle(target)
    ...
    MRFW.Functions.Progressbar("rob_keys", "Attempting Carjacking..", Config.CarjackingTime, false, true, {}, {}, {}, {}, function()
