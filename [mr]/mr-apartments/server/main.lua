...
RegisterNetEvent('mr-apartments:server:SetInsideMeta', function(house, insideId, bool)
    local src = source
...
RegisterNetEvent('mr-apartments:EnterBucket', function()
    local src = source
    local randbucket = (math.random(11111111,99999999))
    SetPlayerRoutingBucket(src, randbucket)
end)

RegisterNetEvent('mr-apartments:EnterBucketguest', function(bucket)
    local src = source
    SetPlayerRoutingBucket(src, bucket)
end)

RegisterNetEvent('mr-apartments:returnBucket', function()
    local src = source
...
RegisterNetEvent('apartments:server:CreateApartment', function()
    local src = source
    local Player = MRFW.Functions.GetPlayer(src)
    ...
    if bank >= 1000 then
        ...
        TriggerClientEvent("apartments:client:CreateApartment", src)
    elseif cash >= 1000 then
        Player.Functions.RemoveMoney('cash', 1000)
        TriggerClientEvent("apartments:client:CreateApartment", src)
    else
        TriggerClientEvent("MRFW:Notify", src, "You don't have enoush money", "error", 3000)
    end
end)

RegisterNetEvent('apartments:server:CreateApartmentConfirm', function(type, label)
    local src = source
    local Player = MRFW.Functions.GetPlayer(src)
    ...
    TriggerClientEvent('MRFW:Notify', src, "You got a apartment ("..label..")")
    -- TriggerClientEvent("apartments:client:SpawnInApartment", src, apartmentId, type)
    TriggerClientEvent("apartments:client:SetHomeBlip", src, type)
end)

-- MRFW.Commands.Add('aptest', 'help text here', {}, false, function(source, args)
--     TriggerEvent('apartments:server:CreateApartment', 'apartment1', 'alta Street')
-- end)
...
RegisterNetEvent('apartments:server:UpdateApartment', function(type, label)
    ...
    TriggerClientEvent('MRFW:Notify', src, "You have changed apartments")
    TriggerClientEvent("apartments:client:SetHomeBlip", src, type)
end)
...
RegisterNetEvent('apartments:server:OpenDoor', function(target, apartmentId, apartment)
    ...
    if OtherPlayer ~= nil then
        TriggerClientEvent('apartments:client:SpawnInApartment', OtherPlayer.PlayerData.source, apartmentId, apartment, bucket)
    end
end)
...
MRFW.Functions.CreateCallback('apartments:GetAvailableApartments', function(source, cb, apartment)
    local apartments = {}
...
MRFW.Functions.CreateCallback('apartments:GetApartmentOffset', function(source, cb, apartmentId)
    local retval = 0
...
MRFW.Functions.CreateCallback('apartments:GetApartmentOffsetNewOffset', function(source, cb, apartment)
...
MRFW.Functions.CreateCallback('apartments:GetOwnedApartment', function(source, cb, cid)
    if cid ~= nil then
...
MRFW.Functions.CreateCallback('apartments:IsOwner', function(source, cb, apartment)
	local src = source
    local Player = MRFW.Functions.GetPlayer(src)
    if Player ~= nil then
        local result = MySQL.Sync.fetchAll('SELECT * FROM apartments WHERE citizenid = ?', { Player.PlayerData.citizenid })
...
MRFW.Functions.CreateCallback('apartments:GetOutfits', function(source, cb)
	local src = source
	local Player = MRFW.Functions.GetPlayer(src)
