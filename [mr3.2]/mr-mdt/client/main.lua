...
RegisterNetEvent('mdt:client:dashboardWarrants', function()
    MRFW.Functions.TriggerCallback("mdt:server:getWarrants", function(data)
        if data then
...
RegisterNUICallback("searchProfiles", function(data, cb)
    ...
    MRFW.Functions.TriggerCallback('mdt:server:SearchProfile', function(result)
        p:resolve(result)
    end, data.name)
...
RegisterNUICallback("getProfileData", function(data, cb)
    ...
    local getProfileDataPromise = function(data)
        ...
        p = promise.new()
        MRFW.Functions.TriggerCallback('mdt:server:GetProfileData', function(result)
            p:resolve(result)
        end, data)
...
RegisterNUICallback('SetHouseLocation', function(data, cb)
    ...
    MRFW.Functions.Notify('GPS has been set!', 'success')
end)
...
RegisterNUICallback("searchVehicles", function(data, cb)
    ...
    MRFW.Functions.TriggerCallback('mdt:server:SearchVehicles', function(result)
        p:resolve(result)
    end, data.name)
...
RegisterNUICallback("saveVehicleInfo", function(data, cb)
    ...
    if JobType == 'police' and impound.impoundChanged == true then
        if impound.inpoundActive then
            ...
            if found == 0 then
                MRFW.Functions.Notify('Vehicle not found!', 'error')
...
RegisterNUICallback("toggleDuty", function(data, cb)
    ...
    TriggerEvent("mr-policejob:ToggleDuty")
    cb(true)
end)
...
RegisterNetEvent('mdt:client:setRadio', function(radio)
    if type(tonumber(radio)) == "number" then
        ...
        MRFW.Functions.Notify("You have set your radio frequency to "..radio..".", "success")
    else
        MRFW.Functions.Notify("Invalid Station(Please enter a number)", "error")
    end
end)
...
RegisterNetEvent('mdt:client:sig100', function(radio, type)
    ...
    if (job == "police" or job == "doctor") and duty == 1 then
        if type == true then
...
RegisterNetEvent('mdt:client:callDetach', function(callid, sentData)
    local job = PlayerData.job.name
    if job == "police" or job == 'doctor' then SendNUIMessage({ type = "callDetach", callid = callid, data = tonumber(sentData) }) end
end)
RegisterNetEvent('mdt:client:callAttach', function(callid, sentData)
    local job = PlayerData.job.name
    if job == "police" or job == 'doctor' then
