...
RegisterNetEvent('mdt:server:openMDT', function()
	...
	for k,v in pairs(Players) do
		...
		if player ~= nil then
			local Radio = Player(player.PlayerData.source).state.radioChannel or 0
			if player.PlayerData.job.name == 'police' or player.PlayerData.job.name == 'doctor' or player.PlayerData.job.name == 'doj' then
...
MRFW.Functions.CreateCallback('mdt:server:SearchProfile', function(source, cb, sentData)
...
MRFW.Functions.CreateCallback("mdt:server:getWarrants", function(source, cb)
    local WarrantData = {}
...
MRFW.Functions.CreateCallback('mdt:server:OpenDashboard', function(source, cb)
...
MRFW.Functions.CreateCallback('mdt:server:GetProfileData', function(source, cb, sentId)
...
RegisterNetEvent('mdt:server:getAllBolos', function()
	...
	local Player = MRFW.Functions.GetPlayer(src)
	local JobType = GetJobType(Player.PlayerData.job.name)
	if JobType == 'police' or JobType == 'doctor' then
...
RegisterNetEvent('mdt:server:getAllReports', function()
	...
	if Player then
		local JobType = GetJobType(Player.PlayerData.job.name)
		if JobType == 'police' or JobType == 'doj' or JobType == 'doctor' then
			if JobType == 'doj' then JobType = 'police' end
...
MRFW.Functions.CreateCallback('mdt:server:SearchVehicles', function(source, cb, sentData)
	if not sentData then  return cb({}) end
	local src = source
	local PlayerData = GetPlayerData(src)
					...
					if impound.impoundActive then
						...
						if vehicle.impoundid ~= nil then
							...
							if result then
								...
								TriggerClientEvent('mr-mdt:client:TakeOutImpound', src, result)
							end
						end
					end
				end
...
RegisterNetEvent('mdt:server:setCallsign', function(cid, newcallsign)
	local Player = MRFW.Functions.GetPlayerByCitizenId(cid)
...
RegisterNetEvent('mdt:server:setWaypoint', function(callid)
	...
	if JobType == 'police' or JobType == 'doctor' then
		if callid then
			local calls = exports['mr-dispatch']:GetDispatchCalls()
...
RegisterNetEvent('mdt:server:setWaypoint:unit', function(cid)
	local src = source
	local Player = MRFW.Functions.GetPlayerByCitizenId(cid)
	if Player.PlayerData.job.onduty and Player.PlayerData.metadata['ptracker'] and Player.Functions.GetItemByName("tracker") then
...
RegisterNetEvent('mdt:server:setRadio', function(cid, newRadio)
	...
	if Player.PlayerData.citizenid ~= cid then
		TriggerClientEvent("MRFW:Notify", src, 'You can only change your radio!', 'error')
		return
	else
		...
		if radio ~= nil then
			...
			TriggerClientEvent("MRFW:Notify", src, 'You do not have a radio!', 'error')
		end
	end
