...
RegisterNetEvent("mr-camera:client:use-camera", function()
    if not active then
        active = true
                    ...
                    while active and not IsEntityDead(lPed) and (GetVehiclePedIsIn(lPed) == vehicle) and true do
                        ...
                        elseif IsControlJustPressed(1, 176) then -- TAKE.. PIC
                            if not presstake then
                                presstake = true
				MRFW.Functions.TriggerCallback("mr-camera:server:webhook", function(hook)
					if hook then
						exports['screenshot-basic']:requestScreenshotUpload(tostring(hook), "files[]", function(data)
                                    			local image = json.decode(data)
                                FullClose()
                                MRFW.Functions.Progressbar("makingphoto", "Collecting Photo..", 1500, false, true, {
                                }, {}, {}, {}, function()
                                end)
                        TriggerServerEvent("mr-camera:server:add-photo-item", json.encode(image.attachments[1].proxy_url))
                                		end)						
					end
				end)
                            end
...
RegisterNetEvent("mr-camera:client:use-photo", function(url)
