...
function SetPetrolCanDurability(percentageFull)
	...
	local ammo
	if CurrentWeaponData.info.ammo ~= nil then
		ammo = CurrentWeaponData.info.ammo - 40
		CurrentWeaponData.info.ammo = CurrentWeaponData.info.ammo - 40
	else
		ammo = 4000 - 40
...
function SetPetrolCanDurability2(percentageFull)
	SetPedAmmo(PlayerPedId(),`weapon_petrolcan`, percentageFull)
	TriggerServerEvent("weapons:server:UpdateWeaponAmmo",CurrentWeaponData,percentageFull)
end
