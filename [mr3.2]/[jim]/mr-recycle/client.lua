...
Citizen.CreateThread(function()
	exports['mr-eye']:AddBoxZone("RecyclingEnter", vector3(746.82, -1398.93, 26.55), 0.4, 1.6, { name="RecyclingEnter", debugPoly=false, minZ=25.2, maxZ=28.0 },
    { options = { { event = "expand:recyling:EnterTradeWarehouse", icon = "fas fa-recycle", label = "Enter Warehouse", }, },
					distance = 1.5
    })
	exports['mr-eye']:AddBoxZone("RecyclingExit", vector3(991.97, -3097.81, -39.0), 1.6, 0.4, { name="RecyclingExit", debugPoly=false, useZ=true, },
    { options = { { event = "expand:recyling:ExitTradeWarehouse", icon = "fas fa-recycle", label = "Exit Warehouse", }, },
					distance = 1.5
    })
    exports['mr-eye']:AddCircleZone("recycleduty", vector3(994.64,-3100.07,-39.0), 2.0, { name="recycleduty", debugPoly=false, useZ=true, },
    { options = { { event = "recycle:dutytoggle", icon = "fas fa-hard-hat", label = "Toggle Recycling Duty", }, },
					distance = 1.5
    })
    exports['mr-eye']:AddCircleZone("tradeitems", Config.Locations['Trade'].location, 2.0, { name="tradeitems", debugPoly=false, useZ=true, },
    { options = { { event = "jim-recycle:Trade:Menu", icon = "fas fa-box", label = "Trade Materials", }, },
					distance = 1.5
    })
	exports['mr-eye']:AddBoxZone("Stash", vector3(997.26, -3091.95, -39.0), 1.3, 0.3, { name="Stash", debugPoly=false, useZ=true, },
    { options = { { event = "jim-recycle:stash",  icon = "fas fa-box", label = "Recycle Storage",stash = "Center", job = "recycle" }, },
					distance = 1.5
    })
	--Dumpster Third Eye
	exports['mr-eye']:AddTargetModel(dumpsters, { options = { { event = "jim-recycle:Dumpsters:Search", icon = "fas fa-dumpster", label = "Search Trash", }, },
					distance = 1.5
    })
end)

RegisterNetEvent('jim-recycle:stash', function()
	...
	if Config.EnableOpeningHours then
		...
		if ClockTime >= Config.OpenHour and ClockTime <= Config.CloseHour - 1 then
			if (ClockTime >= Config.OpenHour and ClockTime < 24) or (ClockTime <= Config.CloseHour -1 and ClockTime > 0) then
				...
				TriggerEvent("MRFW:Notify", "We're currently closed, we're open from "..Config.OpenHour..":00am till "..Config.CloseHour..":00pm", "error")
			end
		else
			TriggerEvent("MRFW:Notify", "We're currently closed, we're open from 9:00am till 21:00pm", "error")
		end
	else
		renderPropsWhereHouse()
		DoScreenFadeOut(500)
...
AddEventHandler('expand:recyling:ExitTradeWarehouse', function()
	if onDuty then
		MRFW.Functions.Notify('Go off duty first', 'error', 3000)
	else
                ...
                if carryPackage == nil then
                    if GetDistanceBetweenCoords(pos.x, pos.y, pos.z, packagePos.x,packagePos.y,packagePos.z, true) < 2.3 then
                        ...
                        if IsControlJustReleased(0, 38) then
                            ...
                            MRFW.Functions.Progressbar("pickup_reycle_package", "Picking up the junk..", 5000, false, true, {
                    ...
                    if GetDistanceBetweenCoords(pos.x, pos.y, pos.z, Config['delivery'].DropLocation.x, Config['delivery'].DropLocation.y, Config['delivery'].DropLocation.z, true) < 2.0 then
                        ...
                        if IsControlJustReleased(0, 38) then
                            DropPackage()
                            ScrapAnim()
                            MRFW.Functions.Progressbar("deliver_reycle_package", "Packing into recyclable box..", 5000, false, true, {
                                disableMovement = true,
                                disableCarMovement = true,
                                disableMouse = false,
                                disableCombat = true,
...
AddEventHandler('recycle:dutytoggle', function()
    onDuty = not onDuty
    if onDuty then
		TriggerEvent('MRFW:Notify', 'You went on duty', 'success')
    else
        TriggerEvent('MRFW:Notify', 'You went off duty', 'error')
    end
end)
...
-- 		{ header = MRFW.Shared.Items["copper"].label, params = { event = "jim-recycle:SellAnim", args = 'copper' } },
-- 		{ header = MRFW.Shared.Items["plastic"].label, params = { event = "jim-recycle:SellAnim", args = 'plastic' } },
-- 		{ header = MRFW.Shared.Items["aluminum"].label, params = { event = "jim-recycle:SellAnim", args = 'aluminum' } },
-- 		{ header = MRFW.Shared.Items["metalscrap"].label, params = { event = "jim-recycle:SellAnim", args = 'metalscrap' }  },
-- 		{ header = MRFW.Shared.Items["steel"].label, params = { event = "jim-recycle:SellAnim", args = 'steel' } },
-- 		{ header = MRFW.Shared.Items["glass"].label, params = { event = "jim-recycle:SellAnim", args = 'glass' } },
-- 		{ header = MRFW.Shared.Items["iron"].label, params = { event = "jim-recycle:SellAnim", args = 'iron' } },
-- 		{ header = MRFW.Shared.Items["rubber"].label, params = { event = "jim-recycle:SellAnim", args = 'rubber' } },
...
RegisterNetEvent('jim-recycle:Trade:Menu', function()
    exports['mr-menu']:openMenu({
		{ header = "Material Trading", txt = "Trade collected materials", isMenuHeader = true },
 		{ header = "", txt = "✘ Close", params = { event = "jim-recycle:SellAnim", args = -2 } },
		{ header = "Trade 1 Materials", params = { event = "jim-recycle:TradeAnim", args = 1 } },
		{ header = "Trade 10 Materials", params = { event = "jim-recycle:TradeAnim", args = 2 } },
		{ header = "Trade 100 Materials", params = { event = "jim-recycle:TradeAnim", args = 3 } },
    })
end)
...
-- 		{ header = "", txt = "✘ Close", params = { event = "jim-recycle:SellAnim", args = -2 } },
-- 		{ header = "Sell "..MRFW.Shared.Items["bottle"].label, params = { event = "jim-recycle:SellAnim", args = 'bottle' } },
-- 		{ header = "Sell "..MRFW.Shared.Items["can"].label, params = { event = "jim-recycle:SellAnim", args = 'can' } },
--     })
-- end)
                ...
                if dist < 1.8 then
                    ...
                    for i = 1, #searched do
                        ...
                        if i == #searched and dumpsterFound then
							MRFW.Functions.Notify('Already Searched.', 'error')
