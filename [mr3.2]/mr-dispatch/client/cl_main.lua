...
RegisterNetEvent('dispatch:manageNotifs', function(sentSetting)
    ...
    if wantedSetting == "on" then
        ...
        MRFW.Functions.Notify("Dispatch enabled", "success")
    elseif wantedSetting == "off" then
        disableNotis = true
        disableNotifSounds = true
        MRFW.Functions.Notify("Dispatch disabled", "success")
    elseif wantedSetting == "mute" then
        disableNotis = false
        disableNotifSounds = true
        MRFW.Functions.Notify("Dispatch muted", "success")
    else
        MRFW.Functions.Notify('Please choose to have dispatch as "on", "off" or "mute".', "success")
				...
				if sNotificationData.origin ~= nil then
					if sNotificationData.dispatchCode == "911" then
						if PlayerJob.name == 'police' and PlayerJob.onduty then
							if sNotificationData.name == 'Anonymous' then
								TriggerEvent('chat:addMessage', {
									template = "<div class='chat-message error'><strong>Anonymous 911 Call</strong>: {0}</div>",
									args = {sNotificationData.information}
								})
							else
								TriggerEvent('chat:addMessage', {
									template = "<div class='chat-message error'><strong>911 Call From {0} (PhoneNo.{1})(ID:{2})</strong>: {3}</div>",
									args = {sNotificationData.name, sNotificationData.number, sNotificationData.source, sNotificationData.information}
								})
							end
						end
					elseif sNotificationData.dispatchCode == "311" then
						if PlayerJob.name == 'police' or PlayerJob.name == 'doctor' and PlayerJob.onduty then
							if sNotificationData.name == 'Anonymous' then
								TriggerEvent('chat:addMessage', {
									template = "<div class='chat-message error'><strong>Anonymous 311 Call</strong>: {0}</div>",
									args = {sNotificationData.information}
								})
							else
								TriggerEvent('chat:addMessage', {
									template = "<div class='chat-message error'><strong>311 Call From {0} (PhoneNo.{1})(ID:{2})</strong>: {3}</div>",
									args = {sNotificationData.name, sNotificationData.number, sNotificationData.source, sNotificationData.information}
								})
							end
						end
...
RegisterNetEvent("mr-dispatch:client:AddCallBlip")
AddEventHandler("mr-dispatch:client:AddCallBlip", function(coords, data)
