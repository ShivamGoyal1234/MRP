...
MRFW.Functions.CreateCallback('jacob:get:coke', function(source, cb)
    cb(hiddenstart, hiddenprocess, boarspawn)
end)
MRFW.Functions.CreateCallback('jacob:get:craft', function(source, cb)
...
MRFW.Functions.CreateUseableItem('coke', function(source, item)
...
-- MRFW.Functions.CreateCallback('coke:processcoords', function(source, cb)
--     cb(hiddenprocess)
-- end)

-- MRFW.Functions.CreateCallback('coke:startcoords', function(source, cb)
--     cb(hiddenstart)
-- end)

--[[MRFW.Functions.CreateCallback('coke:pay' , function(source, cb)
	local src = source
	...
	if cashamount >= amount then
		...
		TriggerClientEvent("inventory:client:ItemBox", source, MRFW.Shared.Items['goldbar'], "remove")
    	cb(true)
	else
		TriggerClientEvent("MRFW:Notify", src, "You dont have enough Money to Start", "error", 4000)
		cb(false)
	end
end)]]
...
AddEventHandler("coke:processed", function(x,y,z)
	...
	local pick = Config.randBrick
		...
		if 	TriggerClientEvent("MRFW:Notify", src, "Made a Coke Bag!!", "Success", 8000) then
			...
			TriggerClientEvent("inventory:client:ItemBox", source, MRFW.Shared.Items['coke_brick'], "remove")
			TriggerClientEvent("inventory:client:ItemBox", source, MRFW.Shared.Items['bakingsoda'], "remove")
			TriggerClientEvent("inventory:client:ItemBox", source, MRFW.Shared.Items['empty_bag'], "remove")
			TriggerClientEvent("inventory:client:ItemBox", source, MRFW.Shared.Items['cokebaggy'], "add")
		end
	end)

MRFW.Functions.CreateCallback('coke:process', function(source, cb)
	local src = source
	local Player = MRFW.Functions.GetPlayer(src)
		...
		for k, v in pairs(Player.PlayerData.items) do
		    if Player.Playerdata.items[k] ~= nil then
				if Player.Playerdata.items[k].name == "coke_brick" and Player.Playerdata.items[k].name == "bakingsoda" and Player.Playerdata.items[k].name == "highqualityscales" and Player.Playerdata.items[k].name == "id_card" and Player.Playerdata.items[k].name == "empty_bag" and Player.Playerdata.items[k].name == "lighter"  then
					...
					TriggerClientEvent("MRFW:Notify", src, "You are missing something", "error", 10000)
					cb(false)
				end
	        end
		end
...
MRFW.Functions.CreateCallback('coke:itemTake', function(source, cb)
    local src = source
...
MRFW.Functions.CreateCallback('coke:ingredient', function(source, cb)
    local src = source
    local Ply = MRFW.Functions.GetPlayer(src)
...
AddEventHandler("coke:paid", function(x,y,z)
	...
	if cashamount >= amount then
		TriggerClientEvent("MRFW:Notify", src, "Yeah! Here you go", "Success", 8000) 
		Player.Functions.RemoveMoney('bank', amount)
		Player.Functions.RemoveItem('greychip', 1)
		TriggerClientEvent("inventory:client:ItemBox", source, MRFW.Shared.Items['greychip'], "remove")
...
AddEventHandler("coke:paid", function(x,y,z)
	...
	if cashamount >= amount then
		...
		-- TriggerClientEvent("inventory:client:ItemBox", source, MRFW.Shared.Items['blackmoney'], "remove")
		Player.Functions.RemoveItem('greychip', 1)
		TriggerClientEvent("inventory:client:ItemBox", source, MRFW.Shared.Items['greychip'], "remove")
...
AddEventHandler("coke:GiveItem", function()
	...
	if fcashamount >= famount then
		...
		TriggerClientEvent('inventory:client:ItemBox', src, MRFW.Shared.Items['coke_brick'], "add")
