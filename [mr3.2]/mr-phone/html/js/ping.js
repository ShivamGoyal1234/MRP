$(document).on('click', '#ping-send', function(e){
    ...
    if (IDPlayer >= 1){
        $.post('https://mr-phone/SendPingPlayer', JSON.stringify({
            id: IDPlayer
        }));
        $("#channel").val("");
    }
...
$(document).on('click', '#ping-accept', function(e){
    e.preventDefault();
    $.post('https://mr-phone/AcceptPingPlayer', JSON.stringify({}));
});

$(document).on('click', '#ping-reject', function(e){
    e.preventDefault();
    $.post('https://mr-phone/rejectPingPlayer', JSON.stringify({}));
});
