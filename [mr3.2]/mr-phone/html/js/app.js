...
$(document).on('click', '.phone-application', function(e){
    ...
    if (AppObject.length !== 0) {
        if (CanOpenApp) {
            if (QB.Phone.Data.currentApplication == null) {
                if (PressedApplication == 'darkweb' || PressedApplication == 'racing') {
                    $.post('https://mr-phone/checkforvpn',JSON.stringify({}),function(has){
                        if (has){
                            ...
                            if (PressedApplication == "settings") {
                                $("#mySerialNumber").text("mr-" + QB.Phone.Data.PlayerData.metadata["phonedata"].SerialNumber);
                            } else if (PressedApplication == "twitter") {
                                $.post('https://mr-phone/GetMentionedTweets', JSON.stringify({}), function(MentionedTweets){
                                    QB.Phone.Notifications.LoadMentionedTweets(MentionedTweets)
                                })
                                $.post('https://mr-phone/GetHashtags', JSON.stringify({}), function(Hashtags){
                                    QB.Phone.Notifications.LoadHashtags(Hashtags)
                                })
                                if (QB.Phone.Data.IsOpen) {
                                    $.post('https://mr-phone/GetTweets', JSON.stringify({}), function(Tweets){
                                        QB.Phone.Notifications.LoadTweets(Tweets);
                                    });
                                }
                            } else if (PressedApplication == 'darkweb') {
                                $.post('https://mr-phone/GetMentionedAnons', JSON.stringify({}), function(MentionedAnons){
                                    QB.Phone.Notifications.LoadMentionedAnons(MentionedAnons)
                                })
                                $.post('https://mr-phone/GetHashtags-dark', JSON.stringify({}), function(Hashtags){
                                    QB.Phone.Notifications.LoadHashtags_dark(Hashtags)
                                })
                                if (QB.Phone.Data.IsOpen) {
                                    $.post('https://mr-phone/GetAnons', JSON.stringify({}), function(Anons){
                                ...
                                $.post('https://mr-phone/GetBankContacts', JSON.stringify({}), function(contacts){
                                    QB.Phone.Functions.LoadContactsWithNumber(contacts);
                                });
                                $.post('https://mr-phone/GetInvoices', JSON.stringify({}), function(invoices){
                                    QB.Phone.Functions.LoadBankInvoices(invoices);
                                });
                            } else if (PressedApplication == "whatsapp") {
                                $.post('https://mr-phone/GetWhatsappChats', JSON.stringify({}), function(chats){
                                    QB.Phone.Functions.LoadWhatsappChats(chats);
                                });
                            } else if (PressedApplication == "phone") {
                                $.post('https://mr-phone/GetMissedCalls', JSON.stringify({}), function(recent){
                                    QB.Phone.Functions.SetupRecentCalls(recent);
                                });
                                $.post('https://mr-phone/GetSuggestedContacts', JSON.stringify({}), function(suggested){
                                    QB.Phone.Functions.SetupSuggestedContacts(suggested);
                                });
                                $.post('https://mr-phone/ClearGeneralAlerts', JSON.stringify({
                                    app: "phone"
                                }));
                            } else if (PressedApplication == "mail") {
                                $.post('https://mr-phone/GetMails', JSON.stringify({}), function(mails){
                                ...
                                $.post('https://mr-phone/LoadAdverts', JSON.stringify({}), function(Adverts){
                                    QB.Phone.Functions.RefreshAdverts(Adverts);
                                })
                            } else if (PressedApplication == "garage") {
                                $.post('https://mr-phone/SetupGarageVehicles', JSON.stringify({}), function(Vehicles){
                                    SetupGarageVehicles(Vehicles);
                                })
                            } else if (PressedApplication == "crypto") {
                                $.post('https://mr-phone/GetCryptoData', JSON.stringify({
                                    crypto: "qbit",
                                }), function(CryptoData){
                                    SetupCryptoData(CryptoData);
                                })
            
                                $.post('https://mr-phone/GetCryptoTransactions', JSON.stringify({}), function(data){
                                    RefreshCryptoTransactions(data);
                                })
                            } else if (PressedApplication == "racing") {
                                $.post('https://mr-phone/GetAvailableRaces', JSON.stringify({}), function(Races){
                                    SetupRaces(Races);
                                });
                            } else if (PressedApplication == "houses") {
                                $.post('https://mr-phone/GetPlayerHouses', JSON.stringify({}), function(Houses){
                                    SetupPlayerHouses(Houses);
                                });
                                $.post('https://mr-phone/GetPlayerKeys', JSON.stringify({}), function(Keys){
                                ...
                                $.post('https://mr-phone/GetCurrentLawyers', JSON.stringify({}), function(data){
                                    SetupLawyers(data);
                                });
                            } else if (PressedApplication == "store") {
                                $.post('https://mr-phone/SetupStoreApps', JSON.stringify({}), function(data){
                                    SetupAppstore(data);
                                });
                            } else if (PressedApplication == "trucker") {
                                $.post('https://mr-phone/GetTruckerData', JSON.stringify({}), function(data){
                                    SetupTruckerInfo(data);
                                });
                            }
                            else if (PressedApplication == "gallery") {
                                $.post('https://mr-phone/GetGalleryData', JSON.stringify({}), function(data){
    ...
    else if (PressedApplication == 'duty') {
        $.post('https://mr-phone/jacob_duty');
        QB.Phone.Functions.Close();
    } else if (PressedApplication == 'vpn') {
        $.post('https://mr-phone/toggelVPN');
        QB.Phone.Functions.Close();
    } else {
...
$(document).on('click', '.mykeys-key', function(e){
    ...
    $.post('https://mr-phone/SetHouseLocation', JSON.stringify({
        HouseData: KeyData
    }))
});
...
$(document).on('click', '.phone-take-camera-button', function(event){
    ...
    $.post('https://mr-phone/TakePhoto', JSON.stringify({}),function(url){
    })
    QB.Phone.Functions.Close();
});
...
$(document).on('click', '.phone-silent-button', function(event){
    event.preventDefault();
    $.post('https://mr-phone/phone-silent-button', JSON.stringify({}),function(Data){
        if(Data){
...
QB.Phone.Functions.Open = function(data) {
    ...
    QB.Phone.Notifications.LoadAnons(data.Anons);
...
QB.Phone.Notifications.Add = function(icon, title, text, color, timeout) {
    $.post('https://mr-phone/HasPhone', JSON.stringify({}), function(HasPhone){
        if (HasPhone) {
    ...
    window.addEventListener('message', function(event) {
        switch(event.data.action) {
            ...
            case "UpdateMentionedTweets":
                ...
                break;
            case "UpdateMentionedAnons":
                QB.Phone.Notifications.LoadMentionedAnons(event.data.Anons);
                break;
            case "UpdateBank":
            ...
            case "RefreshWhatsappAlerts":
                ...
                break;
            case "UpdateHashtags-dark":
                QB.Phone.Notifications.LoadHashtags_dark(event.data.Hashtags);
                break;
            ...
            case "UpdateAnons":
                if (QB.Phone.Data.currentApplication == "darkweb") {
                    QB.Phone.Notifications.LoadAnons(event.data.Anons);
                }
                break;
            case "AddPoliceAlert":
