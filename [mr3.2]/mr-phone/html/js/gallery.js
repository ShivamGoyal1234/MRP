...
$(document).on('click', '#delete-button', function(e){
    ...
    setTimeout(() => {
        $.post('https://mr-phone/DeleteImage', JSON.stringify({image:source}), function(Hashtags){
            setTimeout(()=>{
                $('#return-button').click()
...
$(document).on('click', '#tweet-button', function(e){
    ...
    if (TweetMessage != "") {
        ...
        $.post('https://mr-phone/PostNewTweet', JSON.stringify({
...
$(document).on('click', '#dark-button', function(e){
    e.preventDefault();
    var DarkMessage = $("#new-textarea").val();
    var imageURL = postImageUrl
    if (DarkMessage != "") {
        var CurrentDate = new Date();
        $.post('https://mr-phone/PostNewAnon', JSON.stringify({
            Message: DarkMessage,
            Date: CurrentDate,
            Picture: QB.Phone.Data.MetaData.profilepicture,
            url: imageURL
        ...
        var AnonMessage = $("#new-textarea").val(' ');
        ...
        QB.Phone.Notifications.Add("fas fa-hat-cowboy", "Darkweb", "Fill a message!", "#0F0F0F");
    };
    $('#anon-new-url').val("");
...
$(document).on('click', '#advert-button', function(e){
    ...
    if (Advert !== "") {
        ...
        if (!picture){
            $.post('https://mr-phone/PostAdvert', JSON.stringify({
                message: Advert,
                url: null
            }));
            returnDetail()
