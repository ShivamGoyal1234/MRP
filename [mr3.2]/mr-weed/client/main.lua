...
-- 				--MRFW.Functions.Draw2DText('Press ~g~[ E ]~w~ to Process Cannabis')
-- 			end

-- 			if IsControlJustReleased(0, 38) and not isProcessing then
-- 				TriggerServerEvent('mr-weed:processCannabis')
-- 				-- 	if result then
-- 				-- 		ProcessWeed()
-- 				-- 	else
-- 				-- 		MRFW.Functions.Notify('You do not have enough Cannabis to process..', 'error')
-- 				-- 	end
-- 				-- end, 'cannabis')
-- 			end
-- 		else
...
Citizen.CreateThread(function()
	while true do
		...
		if GetDistanceBetweenCoords(coords, Config.CircleZones.WeedProcessing.coords, true) < 1 then
			...
			if IsControlJustReleased(0, 38) and not isProcessing then
				MRFW.Functions.TriggerCallback('weed:ingredient', function(HasItem, type)
					if HasItem then
						ProcessWeed()
					else
						MRFW.Functions.Notify('You dont have enough Materials', 'error')
					end
				end)
			end
		else
...
Citizen.CreateThread(function()
	while true do
		...
		if GetDistanceBetweenCoords(coords, Config.CircleZones.WeedProcessing2.coords, true) < 1 then
			...
			if IsControlJustReleased(0, 38) and not isProcessing then
				MRFW.Functions.TriggerCallback('weed:ingredient2', function(HasItem, type)
					if HasItem then
...
function ProcessWeed()
	...
	MRFW.Functions.Progressbar("search_register", "Processing..", 15000, false, true, {
		...
		while timeLeft > 0 do
			...
			if GetDistanceBetweenCoords(GetEntityCoords(playerPed), Config.CircleZones.WeedProcessing.coords, false) > 4 then
				TriggerServerEvent('mr-weed:cancelProcessing')
				break
			end
		end
...
function ProcessWeed2()
	...
	MRFW.Functions.Progressbar("search_register", "Processing..", 15000, false, true, {
		...
		TriggerServerEvent("mr-weed:server:process")
		--TriggerServerEvent('mr-weed:processCannabis2')
		local timeLeft = Config.Delays.WeedProcessing / 1000
	...
	while true do
		...
		if nearbyObject and IsPedOnFoot(playerPed) then
			...
			if IsControlJustReleased(0, 38) and not isPickingUp then
				...
				MRFW.Functions.Notify("रूको ज़रा सबर करो", "error", 10000)
				MRFW.Functions.Progressbar("search_register", "Picking up Cannabis..", 10000, false, true, {
					disableMovement = true,
					disableCarMovement = true,
					disableMouse = false,
					disableCombat = true,
					...
					TriggerServerEvent('mr-weed:pickedUpCannabis')
					TriggerServerEvent('mr-weed:weed')
...
function SpawnWeedPlants()
	--[[while spawnedWeeds < 25 do
		...
		MRFW.Functions.SpawnLocalObject('prop_weed_02', weedCoords, function(obj)
    ...
    while b < random do
        ...
        local E = CreateObject(-305885281, D.x + math.random(-15,15), D.y + math.random(-15,15), DD, false, false, true)
