...
RegisterNetEvent('mr-lumberjack:sellItems', function()
    ...
    if Player.PlayerData.items ~= nil and next(Player.PlayerData.items) ~= nil then
        ...
        TriggerClientEvent('MRFW:Notify', source, Config.Alerts["successfully_sold"])
    else 
		TriggerClientEvent('MRFW:Notify', source, Config.Alerts["error_sold"])
	end
end)

RegisterNetEvent('mr-lumberjack:BuyAxe', function()
    local source = source
    local Player = MRFW.Functions.GetPlayer(tonumber(source))
    ...
    if not axe then
        ...
        TriggerClientEvent('inventory:client:ItemBox', source, MRFW.Shared.Items['weapon_battleaxe'], "add")
        Player.Functions.RemoveMoney("cash", TRAxeClassicPrice)
        TriggerClientEvent('MRFW:Notify', source, Config.Alerts["axe_bought"])
    elseif axe then
        TriggerClientEvent('MRFW:Notify', source, Config.Alerts["axe_check"])
    end
end)

MRFW.Functions.CreateCallback('mr-lumberjack:axe', function(source, cb)
    local Player = MRFW.Functions.GetPlayer(source)
    if Player ~= nil then
...
RegisterServerEvent('mr-lumberjack:recivelumber', function()
    ...
    TriggerClientEvent('inventory:client:ItemBox', source, MRFW.Shared.Items['tree_lumber'], "add", lumber)
    Player.Functions.AddItem('tree_bark', bark)
    TriggerClientEvent('inventory:client:ItemBox', source, MRFW.Shared.Items['tree_bark'], "add", bark)
end)

MRFW.Functions.CreateCallback('mr-lumberjack:lumber', function(source, cb)
    local Player = MRFW.Functions.GetPlayer(source)
    if Player ~= nil then
...
RegisterServerEvent('mr-lumberjack:lumberprocessed', function()
    ...
    if not lumber then 
        TriggerClientEvent('MRFW:Notify', source, Config.Alerts['error_lumber'])
        return false
    end
    ...
    if not Player.Functions.RemoveItem('tree_lumber', amount) then 
        TriggerClientEvent('MRFW:Notify', source, Config.Alerts['itemamount'])
        return false 
    end

    TriggerClientEvent('inventory:client:ItemBox', source, MRFW.Shared.Items['tree_lumber'], "remove")
    TriggerClientEvent('MRFW:Notify', source, Config.Alerts["lumber_processed_trade"] ..TradeAmount.. Config.Alerts["lumber_processed_lumberamount"] ..TradeRecevied.. Config.Alerts["lumber_processed_received"])
    Wait(750)
    Player.Functions.AddItem('wood_plank', TradeRecevied)
    TriggerClientEvent('inventory:client:ItemBox', source, MRFW.Shared.Items['wood_plank'], "add", TradeRecevied)
end)