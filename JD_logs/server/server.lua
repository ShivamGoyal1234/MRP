...
-- 						return deferrals.done("\nPrefech | Global Banned.\nReason: "..one.."\nUUID: "..two.."\nTo appeal this ban please join our discord: https://discord.gg/j3NvcCQB3p")
-- 					else
-- 						debugLog('Code: AC1002')
...
AddEventHandler("playerJoining", function(source, oldID)
	...
	if ids.license then
		...
		if loadedFile[ids.license] ~= nil then
			if loadedFile[ids.license] ~= GetPlayerName(source) then
				for _, i in ipairs(GetPlayers()) do
					if IsPlayerAceAllowed(i, cfgFile.nameChangePerms) then
						TriggerClientEvent('chat:addMessage', i, {
							...
							args = { lang['DefaultLogs'].NameChangeChat:format(GetPlayerName(source), loadedFile[ids.license]) }
						})
					end
				end
				ServerFunc.CreateLog({EmbedMessage = lang['DefaultLogs'].NameChange:format(GetPlayerName(source), loadedFile[ids.license]), player_id = source, channel = 'nameChange'})
			end
		end
		loadedFile[ids.license] = GetPlayerName(source)
