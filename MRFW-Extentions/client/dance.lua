...
RegisterNetEvent('nmsh-dances:dance')
AddEventHandler('nmsh-dances:dance', function(pDance)
    if not isHandcuffed and not isSoftCuffed then
        local noAnimations = #animations

        if pDance == -1 then
            pDance = math.random(noAnimations)
            MRFW.Functions.Notify("You randomly selected dance")
        end

        if pDance > noAnimations or pDance <= 0 then
            MRFW.Functions.Notify("There are only 3800 dances, select a number inbetween or random (no input)")
            return
        end

        if animations[pDance] and animations[pDance].disabled then
            MRFW.Functions.Notify("This dance is disabled", "error", 3000)
            return
        end
