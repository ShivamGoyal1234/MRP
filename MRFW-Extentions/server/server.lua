...
MRFW.Functions.CreateUseableItem('labkey2' , function(source, item)
    local src = source
    ...
    if dist <= 1 then
        ...
        -- else
        --     TriggerClientEvent('MRFW:Notify', src, 'You are missing something', 'error')
        -- end
    end
end)

MRFW.Functions.CreateUseableItem('labkey3' , function(source, item)
    local src = source
    local Player  = MRFW.Functions.GetPlayer(src)
    local ped = GetPlayerPed(src)
    local pCoords  = GetEntityCoords(ped)
...
MRFW.Functions.CreateUseableItem('labkey5' , function(source, item)
    local src = source
    local Player  = MRFW.Functions.GetPlayer(src)
    local ped = GetPlayerPed(src)
    local pCoords  = GetEntityCoords(ped)
...
MRFW.Functions.CreateUseableItem('labkey6' , function(source, item)
    local src = source
    local Player  = MRFW.Functions.GetPlayer(src)
    local ped = GetPlayerPed(src)
    local pCoords  = GetEntityCoords(ped)
...
MRFW.Functions.CreateUseableItem("lockpick", function(source, item)
    ...
    if item.info.uses > 0 then
        ...
        TriggerClientEvent("MRFW:Notify", source, "Broken Lockpick", "error", 3000)
    end
end)

MRFW.Functions.CreateUseableItem("advancedlockpick", function(source, item)
    local Player = MRFW.Functions.GetPlayer(source)
    if item.info.uses > 0 then
...
MRFW.Functions.CreateCallback('jacob:get:it', function(source, cb)
    -- print(illegel_teleports)
    cb(illegel_teleports)
end)

MRFW.Functions.CreateCallback('jacob:get:c', function(source, cb)
    -- print(illegel_teleports)
    cb(crafting)
end)
...
RegisterNetEvent("ac:server:checkMyResources", function(givenList)
    for _, resource in ipairs(givenList) do
        if not validResourceList[resource] then
            -- bad client!
            local src = source
            TriggerEvent("mr-log:server:CreateLog", "anticheat", "Player kicked!", "red", "** @everyone " ..GetPlayerName(src).. "** has more resources than server.")
...
MRFW.Commands.Add('allowheat', 'Grant PD Officer Heat Level', {{name='id',help='ID of player'},{name='Allow',help='true or false'}}, false, function(source, args)
    local src = source
    local Player = MRFW.Functions.GetPlayer(src)
    if Player then
        if Player.PlayerData.job.name =='police' and Player.PlayerData.job.grade.level >=11 then
            ...
            if tPlayer then
                ...
                elseif args[2] == 'false' then
                    ...
                    TriggerClientEvent("MRFW:Notify", src, "only true or false", "error", 5000)
                end
            else
                TriggerClientEvent("MRFW:Notify", src, "Player is not online", "error", 5000)
            end
        else
            TriggerClientEvent("MRFW:Notify", src, "You Don't have enough perms", "error", 5000)
        end
    end
end, 'police')

MRFW.Commands.Add('allowairone', 'Grant PD Officer Airone', {{name='id',help='ID of player'},{name='Allow',help='true or false'}}, false, function(source, args)
    local src = source
    local Player = MRFW.Functions.GetPlayer(src)
    if Player then
        if Player.PlayerData.job.name =='police' and Player.PlayerData.job.grade.level >=11 then
