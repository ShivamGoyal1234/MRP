...
RegisterNetEvent("mr-justice:client:showLawyerLicense")
AddEventHandler("mr-justice:client:showLawyerLicense", function(sourceId, data)
...
RegisterNetEvent("mr-justice:client:showMayorPass")
AddEventHandler("mr-justice:client:showMayorPass", function(sourceId, data)
    local sourcePos = GetEntityCoords(GetPlayerPed(GetPlayerFromServerId(sourceId)), false)
    local pos = GetEntityCoords(PlayerPedId(), false)
    if (GetDistanceBetweenCoords(pos.x, pos.y, pos.z, sourcePos.x, sourcePos.y, sourcePos.z, true) < 2.0) then
        TriggerEvent('chat:addMessage', {
...
AddEventHandler('covidvac:client:vaccination', function(source)
    TriggerEvent("inventory:client:ItemBox", MRFW.Shared.Items["covidvac"], "remove")
    TriggerServerEvent("MRFW:Server:RemoveItem", "covidvac", 1)    
end)
