...
Citizen.CreateThread(function()
    ...
    MRFW.Functions.TriggerCallback('jacob:get:wash', function(t, p, s)
		tpLocations       = t
        password          = p
        target            = s
	end)
...
RegisterNetEvent('JcaobMoneyWash')
AddEventHandler('JcaobMoneyWash', function()
    -- print('ok')
...
Citizen.CreateThread(function()
	while tpLocations == nil do Wait(100)end
	...
	while true do 
        ...
        for location, val in pairs(tpLocations) do
            ...
            --                 TriggerServerEvent("InteractSound_SV:PlayOnSource", "houses_door_close", 0.1)
            --             else
            --                 MRFW.Functions.Notify("Wrong passcode, get it from drug dealers!", "error")
            --             end
            --         end
            --     end
            -- end
            ...
            if GetDistanceBetweenCoords(pos.x, pos.y, pos.z, washLocations.pOne.x, washLocations.pOne.y, washLocations.pOne.z, true) < 0.5 then
                if GetDistanceBetweenCoords(pos.x, pos.y, pos.z, washLocations.pOne.x, washLocations.pOne.y, washLocations.pOne.z, true) < 0.5 then      
                    if not washing and not collect then      
                        ...
                        if IsControlJustReleased(1, Keys['E']) then
                            MRFW.Functions.TriggerCallback('mr-moneywash:server:checkWash', function(HasItem, tamount)
                                if HasItem then
                                    -- TriggerServerEvent('mr-moneywash:server:getItem', amt)
                                    WashAnimation()
                                    MRFW.Functions.Progressbar("bills_wash", "Putting Black Money into the washer...", math.random(2000, 5000), false, true, {
                                    ...
                                    flags = 16,
                                        ...
                                        TriggerServerEvent('mr-moneywash:server:checkInv', amt)
                                    ...
                                    MRFW.Functions.Notify("Where is black money?..", "error")
                                end
                            end)                       
                        end   
                    end
...
--                     DrawText3D(washLocations.pOne.x, washLocations.pOne.y, washLocations.pOne.z, "~g~E~w~ - Start Washer")
--                         if IsControlJustReleased(1, Keys['E']) then
--                             MRFW.Functions.TriggerCallback('mr-moneywash:server:checkWash', function(HasItem, type)
--                                 if HasItem then
--                                     TriggerServerEvent('mr-moneywash:server:getItem', amt)
--                                     WashAnimation()
--                                     MRFW.Functions.Progressbar("bills_wash", "Putting Black Money into the washer...", math.random(2000, 5000), false, true, {
...
RegisterNetEvent('mr-moneywash:client:washTimer')
AddEventHandler('mr-moneywash:client:washTimer', function()
        Citizen.Wait(0)
        local pos = GetEntityCoords(PlayerPedId())
        local ped = PlayerPedId()
        local inRange = false
            ...
            if GetDistanceBetweenCoords(pos.x, pos.y, pos.z, collectCoordX,collectCoordY, collectCoordZ, true) < 0.5 then
                ...
                if GetDistanceBetweenCoords(pos.x, pos.y, pos.z, collectCoordX,collectCoordY, collectCoordZ, true) < 0.5 then      
                    if IsControlJustReleased(1, Keys['E']) and inRange then
                        WashAnimation()
                        MRFW.Functions.Progressbar("bills_collect", "Collecting clean bill\'s from washer...", math.random(5000, 10000), false, true, {
                            disableMovement = false,
                            disableCarMovement = false,
                            disableMouse = false,
                            disableCombat = true,
                            ...
                            TriggerServerEvent('mr-moneywash:server:giveMoney', amt)
...
RegisterNetEvent('mr-moneywash:client:startTimer')
AddEventHandler('mr-moneywash:client:startTimer', function(amt)
    ...
    TriggerEvent('mr-moneywash:client:washTimer')
        ...
        while washing do
            ...
            if timer <= 0 then
                washing = false
                collect = true
                MRFW.Functions.Notify('Your clean cash is ready to be collected.', 'success', 50000)
