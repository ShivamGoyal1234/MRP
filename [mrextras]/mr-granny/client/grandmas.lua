                ...
                if distance < 3 then
                    ...
                    if IsControlJustReleased(0, Keys['E']) then
                        if (GetEntityHealth(PlayerPedId()) <= 200) then
                            ...
                            MRFW.Functions.Progressbar("check-", "Checking in..", 5000, false, true, {
                                disableMovement = true,
                                disableCarMovement = true,
                                disableMouse = false,
                                disableCombat = true,
                            }, {}, {}, {}, function()
                                MRFW.Functions.Notify("You're getting treated! ..", "success")
                                Citizen.Wait(10000)
                                TriggerEvent('hospital:client:Revive')
                                TriggerServerEvent('mr-grandmas:server:HealSomeShit')
                                ClearPedTasks(PlayerPedId())
                            end, function() -- Cancel
                                ClearPedTasks(PlayerPedId())
                            end) 
                        else
                            MRFW.Functions.Notify("You do not need medical attention ..", "error")
                        end
                    end
                end
            else
