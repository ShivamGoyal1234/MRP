...
--
		if dist <= 0.5 then
			...
			TriggerServerEvent("mr-chickenjob:startChicken")
			LapChicken()
			end			
		end
	end
...
if share == true then
	...
	MRFW.Functions.Notify("Take Alived Chiken To Process Area ..", "primary")
	TriggerServerEvent("mr-chickenjob:getNewChicken")
	end
...
function hewassplashed()
	...
	if chanceofsplashes <= 60 then
			MRFW.Functions.Notify("You managed to catch 1 chicken!", "success")
			if Caught1 == 1 then
		...
		MRFW.Functions.Notify("The chicken escaped your arms!", "error")
	end
end
------------------DEVELOPED BY mr-----------------------------------------

function packingg(stanowisko)
	local inventory =  MRFW.Functions.GetPlayerData()
	...
	MRFW.Functions.TriggerCallback('MRFW:HasItem', function(result)
		if result then
			...
			MRFW.Functions.Progressbar("wash-", "Packing..", 20000, false, true, {
			...
			MRFW.Functions.TriggerCallback('MRFW:HasItem', function(result4)
				if result4 then
					TriggerServerEvent("mr-chickenjob:getpackedChicken",2)
					MRFW.Functions.Notify("Keep packing the chicken or go to the vehicle and store it.", "primary")
				else
					-- print("badhe teej bannre the na xD")
					MRFW.Functions.Notify("You have nothing to pack!", "error")
...
function packed(stanowisko)
	...
	LoadDict('anim@heists@box_carry@')
		...
		MRFW.Functions.Notify("You stopped packing!", "error")
...
function portioning(position)
	...
	MRFW.Functions.TriggerCallback('MRFW:HasItem', function(result)
		if result then
			...
			MRFW.Functions.Progressbar("Cut-", "Slaughtering..", 15000, false, true, {
				disableMovement = true,
				disableCarMovement = true,
				disableMouse = false,
				disableCombat = true,
			...
			MRFW.Functions.TriggerCallback('MRFW:HasItem', function(result1)
				if result1 then	
					MRFW.Functions.Notify("Now Pack slaughtered chicken!", "primary")
					TriggerServerEvent("mr-chickenjob:getcutChicken", 1)
				else
					-- print("badhe teej bannre the na xD")
					MRFW.Functions.Notify("You dont have any chickens!", "error")
					FreezeEntityPosition(PlayerPedId(),false)
				end
			end, 'alivechicken')
			FreezeEntityPosition(PlayerPedId(),false)
...
function packedsell()
	...
	MRFW.Functions.TriggerCallback('MRFW:HasItem', function(result)
		if result then
			...
			MRFW.Functions.Progressbar("Cut-", "Selling..", 10000, false, true, {
				disableMovement = true,
				disableCarMovement = true,
				disableMouse = false,
				disableCombat = true,
				...
				MRFW.Functions.TriggerCallback('MRFW:HasItem', function(result1)
					if result1 then	
						TriggerServerEvent("mr-chickenjob:sell",3)
					else
						-- print("badhe teej bannre the na xD")
						MRFW.Functions.Notify("You have nothing to sell!", "error")
						FreezeEntityPosition(PlayerPedId(),false)
					end
				end, 'packagedchicken')
