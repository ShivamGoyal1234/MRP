...
$('form .btn').click(function (e) {
    ...
    if(!inputValue || /^\d+$/.test(inputValue.value)) {
        ...
        if (activeMenu == "sell") {
            $.post('https://poly-invest/sellInvestment', JSON.stringify({job: label}))
        } else if(activeMenu == "buy") {
            $.post('https://poly-invest/buyInvestment', JSON.stringify({job: label, amount: inputValue, boughtRate: rate}))
        }
...
$('#new_bank').click(function() {
    $.post('https://poly-invest/newBanking', "{}")
})
...
function mainPage() {
    ...
    $('#general').show();
    inMenu = false
    $.post('https://poly-invest/balance', "{}")
}

function close() {
    $('#general, #waiting, #sellUI, #allUI, #buyUI, #topbar').hide();
    $('body').removeClass("active");
    $.post('https://poly-invest/close', "{}");
}