...
function AlertBoosting(netid, type)
    ...
    elseif type == "mr-dispatch" then
        exports['mr-dispatch']:CarBoosting(car)
    else
        TriggerServerEvent('jacob-carboost:notifypolice', netid)
    end
    ...
    CreateThread(function ()
        while DoesEntityExist(car) do
            ...
            if not Hacked then
                ...
                TriggerServerEvent('jacob-carboost:notifyboosting', carPos, car)
            end
        end
    end)
end
...
-- From mr-dispatch
RegisterNetEvent('jacob-carboost:notifyboosting', function (pos, veh)
...
RegisterNetEvent('jacob-carboost:notifypolice', function (car)
    local Player = MRFW.Functions.GetPlayerData()
    if Player.job.name == 'police' then
        MRFW.Functions.Notify(Lang:t('info.boosting'), 'info')
    end
end)
